...
See [Production deployment → Schema compatibility and rollback](/operate/deployment/production#schema-compatibility-and-rollback) for guidance on handling schema changes across versions.
...
- **Forward-only schema**: `tale rollback` reverts container images but does not revert Convex data. Required-field additions, renames, and type changes should follow the two-release expand-contract pattern documented in [Schema compatibility and rollback](/operate/deployment/production#schema-compatibility-and-rollback).
