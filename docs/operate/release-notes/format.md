...
- **Security**: anything touching authentication, session, secrets storage, crypto, or reachable dependency CVEs. When in doubt, classify as security AND file a [Security Advisory](/operate/security/advisories).
...
- [Security advisory process](/operate/security/advisories) — when to also file a GitHub Security Advisory.
