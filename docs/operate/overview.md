...
- AI chat assistant with multi-turn conversations, file attachments, agent selection, [arena mode](/use/chat/arena-mode) for model comparison, [canvas](/use/workspace/canvas) for content editing, and built-in tools
- [Prompt library](/use/workspace/prompt-library) for saving and sharing reusable prompt templates
- Semantic knowledge base for documents, websites, products, customers, and vendors with [document comparison](/use/workspace/document-comparison)
