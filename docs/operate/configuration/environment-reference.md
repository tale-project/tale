...
To use an external PostgreSQL instance instead of the bundled container, see [Using an external database](/operate/deployment/production#using-an-external-database).
...
When unset, all `/metrics/*` endpoints return `401`. See [Operations](/operate/observability/operations) for endpoint details.
...
See the [Authentication guide](/admin/authentication) for details on configuring trusted headers.
