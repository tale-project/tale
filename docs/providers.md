---
title: AI providers
description: Configure AI model providers, connect self-hosted models, and manage API keys.
---
...
You can also edit these files directly instead of using the UI. See [environment reference](/environment-reference) for the `TALE_CONFIG_DIR` location.
...
Any inference server that exposes an OpenAI-compatible API can be used as a provider. This includes:
...
To connect a self-hosted model:
