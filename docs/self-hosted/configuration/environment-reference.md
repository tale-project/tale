...
| `SOPS_AGE_KEY`          | No       | Age secret key for [SOPS](https://github.com/getsops/sops) encryption of `providers/*.secrets.json`. When set, provider secrets are stored encrypted; when unset, they are stored plaintext at file mode `0600`. Auto-generated by `tale init`.    |
...
To use an external PostgreSQL instance instead of the bundled container, see [Using an external database](/self-hosted/install/linux-server#using-an-external-database).
...
When unset, all `/metrics/*` endpoints return `401`. See [Operations](/self-hosted/operate/observability/operations) for endpoint details.
