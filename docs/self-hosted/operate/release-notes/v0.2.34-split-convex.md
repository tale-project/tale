---
title: v0.2.34 — Split Convex into its own service
...
See [Production deployment → Schema compatibility and rollback](/self-hosted/install/linux-server#schema-compatibility-and-rollback) for guidance on handling schema changes across versions.
...
- **Forward-only schema**: `tale rollback` reverts container images but does not revert Convex data. Required-field additions, renames, and type changes should follow the two-release expand-contract pattern documented in [Schema compatibility and rollback](/self-hosted/install/linux-server#schema-compatibility-and-rollback).
