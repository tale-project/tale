...
Voir la [référence d'environnement](/fr-CH/operate/configuration/environment-reference) pour toutes les options.
...
Les rapports vont dans `trivy-reports/`. Voir [Architecture des conteneurs](/fr-CH/operate/deployment/container-architecture) pour les détails d'image.
