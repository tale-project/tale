...
La configuration des fournisseurs IA (clés API, base URLs, modèles) passe par des fichiers dans `providers/`, pas par des variables d'environnement. Voir la page Paramètres > Providers dans l'admin UI, ou édite les JSON directement.
...
Pour utiliser une instance PostgreSQL externe au lieu du conteneur fourni, voir [Utiliser une base externe](/fr-CH/operate/deployment/production#utiliser-une-base-externe).
...
Non défini, les endpoints `/metrics/*` renvoient `401`. Voir [Operations](/fr-CH/operate/observability/operations) pour les détails.
...
Voir le [guide d'authentification](/fr-CH/admin/authentication) pour configurer les Trusted Headers.
