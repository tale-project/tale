...
L'Arena Mode te laisse comparer deux modèles IA sur le même prompt. Clique l'icône **Épées** dans la barre d'outils, sélectionne deux modèles et envoie un message. Les deux modèles répondent en parallèle dans une vue côte à côte. Enregistre un verdict pour dire quelle réponse était meilleure.

Voir [Arena mode](/fr-CH/use/chat/arena-mode) pour tous les détails.

## Canvas

Quand l'IA génère un bloc de code, un snippet HTML, un diagramme Mermaid ou du Markdown, clique **Open in Canvas** pour l'ouvrir dans un panneau latéral dédié. Canvas offre coloration syntaxique, aperçu en direct, édition et export. Tu peux modifier puis réappliquer dans la conversation.

Voir [Canvas](/fr-CH/use/workspace/canvas) pour tous les détails.

## Prompt Library

Enregistre et réutilise des modèles de prompt dans toute ton organisation. Ouvre la Prompt Library depuis la barre d'outils pour parcourir, chercher et insérer des prompts enregistrés. Tu peux aussi enregistrer un message de chat comme modèle directement depuis la conversation.

Voir [Prompt library](/fr-CH/use/workspace/prompt-library) pour tous les détails.
