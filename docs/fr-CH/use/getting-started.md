...
C'est le chemin le plus rapide pour passer de rien à une instance Tale qui tourne. Pas besoin d'être développeur — si tu sais lancer une commande dans un terminal, tu peux suivre. Pour les setups de production auto-hébergés, va plutôt sur [Déploiement en production](/fr-CH/operate/deployment/production).
...
Voir [Contributing Docker guide](/fr-CH/develop/contributing-docker) pour plus de détails.
