...
Ollama lauscht standardmässig auf `http://localhost:11434`. Bestätige mit einer Modellliste:
...
Die vollen Routing-Regeln — Anbieter-Reihenfolge, Fallbacks, wann ein Präfix nötig ist — liegen unter [Anbieter — Modelle für Agents verfügbar machen](/de-CH/self-hosted/configuration/providers#making-models-available-to-agents).
...
Hat ein anderer Anbieter geantwortet, passt entweder die Modell-ID nicht zu Ollamas Liste (Gross-/Kleinschreibung beachten), oder `supportedModels` enthält noch einen Frontier-Modell-Eintrag, der im Routing Vorrang hat.
...
- **404 auf das Modell** — die ID beachtet Gross-/Kleinschreibung und muss exakt zu dem passen, was `ollama list` ausgibt. Aus der Laufzeit kopieren, nicht aus der Modell-Karte.
