...
description: Dockerfiles ändern, Container-Tests ausführen und Images in ihren Grössen-Budgets halten.
---
...
Wenn du Tale eher betreibst als baust, decken die Installationspfade unter [Quickstart](/de-CH/self-hosted/install/quickstart) und [Linux-Server](/de-CH/self-hosted/install/linux-server) alles ab, was du brauchst — diese Seite ist Contributor-Territorium.
...
# Image-Struktur validieren (keine Secrets, OCI-Labels, Grössen-Budgets)
...
Für jedes Image prüft der Validator das OCI-`org.opencontainers.image.version`-Label, den Non-Root-Nutzer (für das Platform-Image Pflicht), die Abwesenheit von Secrets in Env oder Dateisystem, die `HEALTHCHECK`-Anweisung und das Grössen-Budget. Jeder einzelne Fehlschlag lehnt das Image ab.
...
Contributing-Docker ist der Source-Contributor-Ablauf für das Build-System, das Tales Container-Images produziert. Die Laufzeit-Architektur, in der diese Images laufen, ist unter [Container-Architektur](/de-CH/self-hosted/operate/container-architecture) dokumentiert; für den operatorseitigen Installationspfad, der die Images konsumiert, sind [Quickstart](/de-CH/self-hosted/install/quickstart) und [Linux-Server](/de-CH/self-hosted/install/linux-server) die kanonischen Referenzen.
