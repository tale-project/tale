...
Der Workflow läuft asynchron; das aufrufende System wartet nie auf den Output. Status und Schritt-Ergebnisse liegen im **Ausführungen**-Tab des Workflows — siehe [Ausführungslogs](/de-CH/platform/automations/execution-logs).
...
- **Vom aufrufenden System zu Tale**: der POST-Body und die Kopfzeilen, einschliesslich des Tokens in der URL. HTTPS schützt alles in Transit; das Token wird nicht als Kopfzeile gesendet, also bleibt es aus Standard-`Authorization`-Log-Zeilen heraus, erscheint aber in der URL jedes Access-Logs, das der Client schreibt. Behandle es entsprechend.
