...
`SITE_URL` muss der URL entsprechen, die Nutzer im Browser aufrufen, inklusive nicht-standardmässiger Ports (z. B. `https://example.com:8443`).
...
Zum Einsatz einer externen PostgreSQL-Instanz statt des mitgelieferten Containers siehe [Externe Datenbank nutzen](/de-CH/operate/deployment/production#externe-datenbank-nutzen).
...
Wenn leer, liefern alle `/metrics/*`-Endpoints `401`. Siehe [Operations](/de-CH/operate/observability/operations) für Endpoint-Details.
...
Siehe die [Authentifizierungs-Anleitung](/de-CH/admin/authentication) für Details zur Konfiguration von Trusted Headers.
