...
Tales `providerOptions` fliessen nur in den Anfrage-Body. Kopfzeile-Routing-Steuerungen (`ai-gateway-order`, `ai-gateway-only`) und Observability-Tags (`metadata`) sind aktuell nicht über die Anbieter-Konfiguration setzbar; Routing über das Modell-ID-Präfix anpinnen und Tagging im Vercel-Dashboard konfigurieren.
...
Wenn `SOPS_AGE_KEY` (oder `SOPS_AGE_KEY_FILE`) in `.env` gesetzt ist, speichert Tale Secrets [SOPS](https://github.com/getsops/sops)-verschlüsselt mit dem konfigurierten age-Empfänger. `tale init` erzeugt automatisch einen Schlüssel und verwendet diesen Modus standardmässig. Die Datei sieht auf der Platte so aus:
