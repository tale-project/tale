...
| `ENCRYPTION_SECRET_HEX` | Ja      | Verschlüsselungsschlüssel für sensible Daten, einschliesslich der in der DB gespeicherten Guardrails-Secrets (Moderations-API-Schlüssel usw.). Erzeugen mit: `openssl rand -hex 32`. Eine Rotation invalidiert alle gespeicherten Guardrails-Secrets — Admins müssen sie über die UI neu speichern. |
