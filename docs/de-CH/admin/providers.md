...
Diese Seite beschreibt das tägliche Anbieter-Management im Admin-UI. Für das Anbinden selbst gehosteter Modelle (Ollama, vLLM, LocalAI, etc.) siehe [Eigene Modelle anbinden](/de-CH/build/integrations/providers).

## Anbieter verwalten

Anbieter werden unter **Einstellungen > Providers** in der Management-UI verwaltet. Admins können:
...
Du kannst die Dateien auch direkt bearbeiten, statt die UI zu nutzen. Siehe [Environment-Referenz](/de-CH/operate/configuration/environment-reference) für den Ort von `TALE_CONFIG_DIR`.
...
3. Verschlüssle die Secrets-Datei mit SOPS oder aktualisiere den API-Schlüssel via UI unter **Einstellungen > Providers > OpenRouter**.
...
Nach dem Anlegen eines Anbieters mit Modellen musst du die Model-IDs zusätzlich in die `supportedModels`-Liste des Agents eintragen. Agent-Konfigurationen liegen unter `TALE_CONFIG_DIR/agents/`. Bearbeite die passende Agent-JSON-Datei und ergänze die exakten Model-IDs aus deinem Provider-Config (`models[*].id`):
