...
- ob eine Modell- oder Provider-Änderung ihre Workflow-Ausgaben verschiebt;
...
| 2       | `## 🤖 Model & Provider` | LLM-Modell-Wechsel/-Upgrade/-Deprecation, Provider-Config-Änderungen, die Output verändern           |
...
- **Security**: alles, was Authentifizierung, Session, Secret-Speicherung, Crypto oder erreichbare Abhängigkeits-CVEs berührt. Im Zweifel als Security klassifizieren UND ein [Security Advisory](/de-AT/operate/security/advisories) anlegen.
- **Model & Provider**: jede Änderung, die die LLM-Ausgabe bei gleichem Nutzer-Input verschieben kann — Modell-Bumps, Provider-Wechsel, Prompt-/Template-Änderungen in Default-Agents.
...
- [Security-Advisory-Prozess](/de-AT/operate/security/advisories) — wann zusätzlich ein GitHub Security Advisory erstellt wird.
