...
Choisis les modèles par défaut chat, vision et embedding utilisés quand les utilisateurs n'en choisissent pas explicitement. Les modèles viennent de tout fournisseur configuré — voir [Fournisseurs IA](/fr/admin/providers).
...
Configure combien de temps les conversations, fichiers téléversés et enregistrements d'audit sont conservés avant suppression automatique. Voir [Rétention](/fr/operate/configuration/retention) pour les valeurs par défaut au niveau environnement qui s'appliquent aux déploiements auto-hébergés.
...
Vois la consommation de tokens, la ventilation des coûts et les tendances d'usage. Filtre par équipe, utilisateur, modèle ou période. Pour une analyse plus poussée, voir [Usage analytics](/fr/admin/usage-analytics).
