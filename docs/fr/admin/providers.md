...
Cette page couvre la gestion quotidienne dans l'admin UI. Pour connecter des modèles auto-hébergés (Ollama, vLLM, LocalAI, etc.), voir [Bring your own model](/fr/build/integrations/providers).
...
Tu peux aussi éditer ces fichiers directement au lieu de l'UI. Voir la [référence d'environnement](/fr/operate/configuration/environment-reference) pour l'emplacement de `TALE_CONFIG_DIR`.
