...
- Chat IA avec conversations multi-tours, pièces jointes, sélection d'agent, [arena mode](/fr/use/chat/arena-mode) pour comparer des modèles, [canvas](/fr/use/workspace/canvas) pour éditer du contenu et outils intégrés.
- [Prompt library](/fr/use/workspace/prompt-library) pour enregistrer et partager des modèles de prompt.
- Base de connaissances sémantique pour documents, sites, Products, Customers, Vendors avec [comparaison de documents](/fr/use/workspace/document-comparison).
