...
| `tale-bifrost`        | Gateway LLM pour les agents de code en sandbox                                                             | Les agents en sandbox ne joignent aucun modèle ; le chat n'est pas affecté         |
...
**`tale-bifrost` en panne.** Les agents de code en sandbox perdent leur chemin vers un fournisseur de modèles. Le chat ordinaire — qui appelle les fournisseurs directement depuis convex, pas via Bifrost — n'est pas affecté.
