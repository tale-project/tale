...
Pour les jobs Linux ou macOS ARM64 de GitHub Actions, utilise l’action composite Tale `.github/actions/setup-cli`. Fixe l’action et `revision` au même commit Tale complet. Elle compile avec Bun 1.4.2, vérifie le binaire final, fournit `executable` et complète le `PATH`. Les builds macOS prennent en charge la préparation générale de configuration ; la pile Linux gérée exige toujours un binaire Linux correspondant. Sur un runner Linux x64, passe `linux-baseline: 'true'` quand le processeur cible n’a pas AVX2 (Intel avant Haswell, par exemple). Le binaire standard s’y arrête sur `Illegal instruction`, le binaire baseline fonctionne. Les autres runners refusent cette option.
...
La préparation vérifie d’abord chaque configuration avec les schémas propres à la CLI, et seulement ensuite télécharge et vérifie les images du runtime. Elle nomme chaque phase et chaque image au fur et à mesure. Un paquet qui déclare des champs inconnus de cette CLI est refusé en quelques secondes avec `native manifest normalization changes release semantics at <fields>` ; prépare-le alors avec une CLI au moins aussi récente que le Tale visé par le paquet. Une erreur que la CLI lève délibérément affiche sa propre cause. Toute autre erreur garde un résumé fixe, afin qu’aucun identifiant ni aucune sortie du registre n’atteigne le journal.
...
Si un autre compte détient la nouvelle adresse, ou si le compte conservé ne détient aucune des deux adresses, le déploiement s’arrête avant toute modification.
...
Chaque déploiement aligne le backend sur la déclaration. Un compte absent est créé avec une adresse vérifiée et exactement les identifiants déclarés. Un compte existant retrouve ces identifiants, et toutes ses sessions prennent fin quand ils changent. Le compte devient `admin` de l’organisation gérée via les points d’accès natifs des membres ; un `owner` n’est jamais modifié. Un journal conservé lie l’adresse à un seul identifiant de compte : si un autre compte la détient plus tard, le déploiement s’arrête. Le déploiement ne se connecte jamais avec ce compte. Change son mot de passe en modifiant le hachage déclaré, pas dans l’application.
...
Les politiques de conservation et DSAR exigent leurs workflows natifs dédiés. Interromps les envois, la synchronisation et les crawls avant de changer de modèle d’embedding. La CLI vérifie le nombre de documents et de sites web dans toute l’organisation ; elle ne verrouille pas l’import et ne migre pas les vecteurs existants. Pour une organisation avec des documents ou des sites web enregistrés, un changement de modèle exige une migration native distincte de l’index. Une modification limitée à `minSimilarity`, `maxConcurrentRequests` ou `minTokensPerSecond` laisse les vecteurs existants valables et échappe donc à cette vérification. Les paramètres d’instance exigent aussi la liste native des éditeurs autorisés. L’application autonome signale `restartRequired` pour les paramètres de démarrage ; les enregistrer ne les active pas encore. Examine les effets du plan avant de l’appliquer.
...
Pour la sélection du modèle de vision, utilise `governance` avec `key: "vision_model"` et les champs natifs `providerSlug`/`modelId`. L’embedding utilise `knowledge-embedding` avec `providerSlug`, `model`, `dimensions` et `baseUrl`, plus les réglages facultatifs que la plateforme garde à côté du modèle dans [`embedding.json`](/fr/self-hosted/configuration/data-residency#le-modele-dembedding-de-lorganisation) : `minSimilarity`, le plancher de cosinus de l’assistant pour ce modèle, et les limites du serveur `maxConcurrentRequests` et `minTokensPerSecond` ([Doser les requêtes vers un serveur d’embedding auto-hébergé](/fr/self-hosted/configuration/data-residency#capacite-du-serveur-dembedding)). Chacun suit la règle de la plateforme elle-même : une valeur le fixe, une clé omise laisse ce que le fichier contient (une valeur posée à la main survit à une release qui ne la mentionne pas), et `null` l’efface — `"minSimilarity": null`, par exemple, est la seule façon de retirer un plancher par la CLI. Pour remplacer les identifiants par défaut, déclare aussi les anciens identifiants d’environnement avec `isDefault: false` ; la CLI applique ce changement explicite en premier. Les secrets ne figurent ni dans la déclaration ni dans le reçu.
