...
La page de connexion de Tale envoie elle-même le navigateur vers l’adresse de passage dès que la requête porte l’en-tête d’identité du proxy, si bien que les membres ne voient jamais le formulaire d’identifiants ; rediriger `/log-in` du proxy directement vers l’adresse de passage évite ce détour et reste recommandé. Si un passage n’aboutit pas à une session (refus, ou cookies bloqués dans un cadre), la page affiche le formulaire avec un bouton Réessayer au lieu de boucler.
