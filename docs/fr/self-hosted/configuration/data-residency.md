...
- **Stockage de fichiers** — où vivent les fichiers téléversés (les blobs d’origine). Par défaut, ils sont dans le store d’objets fourni (le service `object-store`, sur son propre volume) ; les variables `OBJECT_STORE_*` pointent le backend vers un bucket externe compatible S3. Le basculement est greenfield : les blobs déjà écrits dans le store fourni ne sont pas copiés — fixe-le au déploiement initial, ou copie d’abord le volume dans le bucket hors bande.
