...
Avant de lancer son daemon Docker interne, la runtime choisit un `/16` privé qui ne chevauche ni ses routes IPv4 réelles ni le réseau d’organisation raccordé ensuite. Elle privilégie `172.31.0.0/16`, puis essaie d’autres plages privées. Le premier `/24` sert à `docker0` ; les réseaux Compose internes utilisent des blocs `/24` du même pool. Si les routes sont illisibles ou qu’aucun pool ne reste libre, le démarrage de la session échoue avec une erreur précise de la runtime.

Garde `sandbox`, `sandbox-egress` et `SANDBOX_RUNTIME_IMAGE` sur la même version lors d’une mise à niveau. Avant de raccorder le réseau de build d’une organisation, le spawner vérifie la protection de la session contre le transfert de paquets. Compose et les conteneurs de session générés désactivent IPv6 avec `net.ipv6.conf.all.disable_ipv6=1` et `net.ipv6.conf.default.disable_ipv6=1`. Conserve ces sysctls dans tes propres définitions de déploiement pour qu’un déploiement IPv4 ne dépende pas du pare-feu IPv6 de l’hôte.
