...
Coller un lien vidéo dans le chat récupère sa transcription pour l'agent. YouTube bloque l'accès automatisé depuis les IP de centres de données/serveurs, ce qui peut échouer sur un déploiement cloud. Chaque option ci-dessous est facultative et désactivée par défaut ; chacune rend le trafic plus légitime, mais aucune ne garantit un contournement — une IP de sortie propre est le levier le plus important. Lues par le conteneur `convex` et réévaluées à chaque ingestion, donc une modification prend effet sans redémarrage.
...
| `VIDEO_INGEST_POT_PROVIDER_URL` | non défini          | URL de base d'un fournisseur de PO tokens (le serveur HTTP bgutil) qui fournit les tokens GVS levant le mur anti-bot de YouTube pour les clients `mweb`/`web`/`tv_simply`. À héberger toi-même.                                                                                                             |
...
| `VIDEO_INGEST_PLAYER_CLIENT`    | `default,tv_simply` | Liste de repli des clients de lecture YouTube, séparés par des virgules. N'ajoute `mweb` que lorsqu'un fournisseur de PO tokens est configuré (il lui faut un token GVS).                                                                                                                                   |
...
Aucune de ces options ne garantit le succès face à la détection adaptative de YouTube. Les vidéos publiques ordinaires, les plateformes moins agressives ou un déploiement à IP résidentielle/auto-hébergé fonctionnent généralement sans elles. Voir [pièces jointes du chat](/platform/chat/attachments) pour l'usage des transcriptions ingérées.
