...
## Contrôle selon l’assignation des conversations

Par défaut, chaque membre ayant accès à la boîte de réception voit toutes les conversations de l’organisation. Sous **Paramètres > Gouvernance > Politiques et limites**, ouvre **Contrôle selon l’assignation des conversations** et active **Activé pour cette organisation** pour rendre privée une conversation assignée : une conversation assignée à une équipe n’est visible que par ses membres, et une conversation assignée à une personne n’est visible que par elle. Les conversations non assignées restent un pool partagé à l’échelle de l’organisation que chacun peut prendre en charge, et les administrateurs et propriétaires voient toujours tout.

L’interrupteur est désactivé pour les organisations existantes : rien ne change tant qu’un administrateur ne l’active pas. La visibilité suit l’[équipe](/fr/platform/admin/teams) et le membre auxquels une conversation est assignée — définis tous deux depuis le sélecteur d’assignation de l’en-tête de la conversation.

## Routage des conversations

Le courrier entrant arrive non assigné tant qu’une règle de routage ne le revendique pas. Sous **Paramètres > Gouvernance > Politiques et limites**, ouvre **Routage des conversations** et ajoute une règle associant une adresse destinataire à une équipe, une personne, ou les deux : la prochaine conversation qui arrive à cette adresse est assignée dès sa création, avant que quiconque n’ouvre la boîte de réception. Une règle correspond à l’adresse à laquelle l’expéditeur a écrit — le `À` de la conversation — sans tenir compte de la casse ; une adresse sans règle reste non assignée dans le pool partagé.

Le routage ne fait qu’assigner ; il ne réassigne jamais une conversation qui a déjà un responsable ou une équipe, de sorte qu’une réponse s’enchaînant dans un fil existant est laissée intacte. Combine-le avec **Contrôle selon l’assignation des conversations** ci-dessus pour rendre le courrier routé privé à l’équipe où il atterrit dès l’instant de son arrivée. Une règle pointant vers une équipe ou une personne supprimée depuis est ignorée — la conversation arrive quand même, simplement non assignée.
