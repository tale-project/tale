...
Les clés API sont le pont entre Tale et le code externe ; elles s’asseyent à côté des [Intégrations](/fr/platform/admin/integrations) (systèmes tiers que Tale appelle) et des [Webhooks](/fr/platform/agents/webhook-triggers) (systèmes qui appellent Tale sur événement). La lecture suivante naturelle est l’API REST elle-même — voir la référence API dans l’onglet Develop pour la surface contre laquelle une clé authentifie, et voir [Membres et rôles](/fr/platform/admin/members-and-roles) pour la carte rôle-vers-permission que chaque clé hérite.
