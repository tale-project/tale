...
**[Clés API](/fr/platform/admin/api-keys)** — Les Développeurs lisent ceci quand ils câblent un script, une tâche cron, ou un service interne à l'API REST de Tale ; la surface est partagée avec Admin sous Paramètres > Clés API.

**[Serveurs MCP](/fr/platform/integrations/mcp-servers)** — Les Développeurs lisent ceci quand ils enregistrent un processus externe protocole MCP et choisissent quels de ses tools les agents de l'org peuvent appeler.

**[Déclencheurs webhook d'agent](/fr/platform/agents/webhook-triggers)** — Les Développeurs lisent ceci quand ils veulent qu'un système externe déclenche un agent spécifique sur un POST entrant.

**[Tools d'agent](/fr/platform/agents/tools)** — Les Développeurs lisent ceci quand ils étendent le toolbelt d'un agent avec un tool personnalisé que les agents de l'org peuvent appeler.
