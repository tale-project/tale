...
Les gates d'approbation sont la façon dont un workflow met un humain entre deux étapes automatisées. La lecture suivante naturelle est [Concepts de workflow](/fr/platform/workflows/concepts) pour le modèle environnant — définitions, déclencheurs, étapes, exécutions — et [Concepts d'approbation](/fr/platform/approvals/concepts) pour l'histoire inter-sources de ce que chaque approbation porte.
