---
title: Concepts de workflow
description: Un workflow est une définition d'étapes plus un déclencheur plus un historique d'exécutions. Cette page nomme les quatre pièces et montre comment un rapport quotidien y circule.
---

Un workflow est l'unité vers laquelle Tale se tourne quand le travail est multi-étapes et que tu veux des approbations, de la planification ou des déclencheurs externes entre les étapes. Il est une définition d'étapes plus un déclencheur plus un historique d'exécutions — trois choses que tu composes pour transformer une tâche récurrente en un graphe qui s'exécute lui-même.

Cette page te donne le vocabulaire que le reste de la section workflows présuppose. Lis-la avant de construire un workflow, et reviens-y quand tu ne sais plus si un comportement appartient à une étape, au déclencheur ou à un point d'approbation.

## Les quatre pièces

**La définition** est l'ensemble ordonné d'étapes avec leurs entrées et sorties. Les étapes peuvent s'exécuter en séquence, en parallèle ou derrière une branche conditionnelle. Un workflow est versionné ; chaque enregistrement crée une nouvelle version à laquelle tu peux revenir.
...
La page de concept [Approbations dans les workflows](/fr/platform/workflows/approvals-in-workflows) couvre les états du point et les règles de routage en détail.

## Mis bout à bout — un workflow de rapport quotidien

Un workflow de rapport quotidien met les quatre pièces dans une chaîne :
...
| Utilise … quand                                              | Workflow | Agent | Cron |
| ------------------------------------------------------------ | -------- | ----- | ---- |
...
Les agents sont la bonne forme pour des conversations en un seul tir ; les workflows sont la bonne forme quand le travail a des étapes et que tu veux capturer l'entrée, la sortie et l'approbateur de chacune.

## Construis-en un

La définition, les déclencheurs, les étapes et les exécutions sont les quatre pièces dont chaque workflow Tale est fait : la définition est la recette, le déclencheur est le départ, les étapes sont les coups, l'exécution est l'enregistrement. Va vers un workflow quand le travail a des étapes ; va vers un agent quand la conversation reste dans une voix. La lecture suivante naturelle est [Workflow avec approbations](/fr/tutorials/editor/workflow-with-approvals) — elle parcourt les quatre pièces sur une instance neuve.
