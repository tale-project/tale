...
**Manuel** est un bouton. Les Membres et Éditeurs ayant accès au workflow le lancent depuis son bouton de lancement ; cliquer le bouton ouvre le formulaire d'entrée (un champ par entrée déclarée) et démarre l'exécution. Va vers manuel quand le workflow est occasionnel et qu'un humain sait quand il doit tourner.
...
Les déclencheurs sont la couche de démarrage du modèle de workflow ; les étapes qui les suivent sont le vrai travail. La lecture suivante naturelle est [Concepts de workflow](/fr/platform/workflows/concepts) pour le modèle workflow-étape-exécution dans lequel le déclencheur s'écoule, et [Approbations dans les workflows](/fr/platform/workflows/approvals-in-workflows) pour le gate qui met l'exécution en pause entre étapes.
