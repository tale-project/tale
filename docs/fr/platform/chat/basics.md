...
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rag_search` | Les connaissances de l’organisation et son travail — documents, entrées de connaissances, pages de sites web explorés, produits, contacts, sites web, tâches et projets, et les conversations de la boîte de réception — limitées à l’endroit où vit le chat (ci-dessous) |
...
**Où les outils cherchent dépend de l’endroit où vit le chat, pas de l’assistant.** Un document vit à un seul endroit — le hub de connaissances de l’organisation ou un projet, jamais les deux. Qui voit un document du hub, ce sont ses tags d’équipe qui le décident : sans tag, chaque membre ; avec, les membres de l’une des équipes listées — et les admins n’ont aucun passe-droit. Qui voit un fichier de projet, seul l’accès au projet le décide — un projet à l’échelle de l’organisation veut dire chaque membre, un projet d’équipe ses équipes propriétaire et partagées, et les propriétaires et les admins voient chaque projet ; un fichier de projet ne porte jamais de tag d’équipe. Un chat démarré dans un projet voit les fichiers de ce projet plus le hub tel que tu le vois, et son tableau est celui de ce projet ; le chat de l’organisation voit le hub seulement, et chaque tableau que tu peux lire. Chaque chat voit aussi ses propres pièces jointes et les pièces jointes des e-mails des conversations que tu peux lire. Les fichiers à la corbeille ou expirés sont invisibles partout ; un projet archivé garde ses fichiers lisibles, marqués comme archivés. La frontière tient côté serveur : un chat de projet ne va jamais puiser dans un autre projet, et une question sur les fichiers d’un projet posée dans le chat de l’organisation trouve sa réponse dans le chat du projet lui-même — posée là, elle liste les fichiers du projet à côté de ceux du hub, chacun marqué de l’endroit où il vit et de son état d’indexation.
...
- Chaque appel d’outil est une ligne d’étape — _Recherche de « … » dans l’espace de travail_, _Liste des tâches_, _Lecture de example.com_, _Lecture de lead-verify.txt_ — avec un indicateur d’activité pendant qu’il tourne et, quand il échoue, un avertissement qui en donne la raison. Une étape de document nomme le fichier même quand la lecture échoue, et la raison dit ce qui bloque : un fichier chargé sans indexation, un fichier encore en cours d’indexation, un fichier dont l’indexation a échoué. Les étapes restent visibles quand le raisonnement est replié ; elles sont la trace de ce que l’assistant est allé chercher.
