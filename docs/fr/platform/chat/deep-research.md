...
La recherche approfondie est un mode du composer qui confie une question à un agent **Chercheur** spécialisé. L'agent planifie le travail comme une liste de sous-questions, cherche sur le web ouvert avec Tavily, lit les pages les plus prometteuses, suit la progression dans une carte de tâches que tu regardes en direct, et finit avec un rapport PDF qui cite chaque source utilisée. Va vers ce mode quand la question est ouverte, que la réponse a besoin de preuves, et que tu y passerais sinon une heure avec vingt onglets de navigateur.
...
La recherche approfondie bat un chat ordinaire pour les questions où la valeur n'est pas le savoir existant du modèle mais l'assemblage d'informations récentes et sourcées. Trois signaux qu'il s'agit du bon mode :
...
Ouvre Chat. Le composer montre un bouton **Deep research** (icône télescope) à côté du sélecteur d'agents ; clique-le et le composer bascule vers l'agent Chercheur. Tape la question et envoie. Le panneau de réponse passe du streaming texte habituel à une carte **research plan** avec trois à sept entrées de tâche que l'agent a choisies comme sous-questions.

Le bouton Deep research est visible quand un Éditeur ou un rôle supérieur a lié l'intégration **Tavily** sous [Paramètres > Intégrations](/fr/platform/integrations/overview) ; sans Tavily, l'agent affiche une ligne demandant à un admin de la connecter.

## Le plan de recherche

Le plan est une liste d'entrées `pending` que l'agent a générées à partir de ta question. Pour les questions complexes, l'agent fait une pause après le premier plan et te demande de confirmer — une carte **Proceed with this plan?** apparaît avec un champ oui/non. Clique oui pour démarrer ; clique non et l'agent ne tourne pas plus loin. Les questions triviales sautent la confirmation.
...
La carte se met à jour en direct à mesure que chaque étape arrive. Tu peux regarder le raisonnement du modèle se former ; si une nouvelle sous-question émerge en cours de route, l'agent l'ajoute à la liste.

## Recherches et extractions

Tavily est le fournisseur de recherche web ouverte derrière la recherche approfondie — son API est optimisée pour les agents LLM et renvoie les résultats de recherche avec des snippets nettoyés et des scores par résultat. Deux opérations comptent :
...
- **3 recherches + 2 extractions par tâche.** Le wrapper d'intégration refuse les appels au-delà.
- **40 étapes de raisonnement au total** sur toute l'exécution.
- **25 minutes wall-clock.** Au-delà, l'agent s'arrête et synthétise avec ce qu'il a.
...
Quand chaque tâche est `done` (ou annulée, ou que le budget a tapé un mur), l'agent appelle l'outil **pdf** une fois pour produire un seul rapport structuré :
...
Pour les rapports en chinois, japonais et coréen, le jeu de polices du moteur de rendu PDF est incomplet ; dans ce cas, l'agent émet le même rapport structuré directement dans le chat et note qu'un PDF traduit en anglais est disponible sur demande.
...
- **Quota Tavily épuisé.** L'intégration renvoie `INTEGRATION_BUDGET_EXHAUSTED` et l'agent passe à la synthèse avec ce qu'il a. Le palier gratuit touche ça vers le millième appel du mois.
- **Une URL spécifique échoue à l'extraction.** La tâche concernée est marquée `failed` avec une raison ; les autres tâches continuent.
- **Ton budget se vide.** L'exécution s'arrête et l'agent synthétise. La carte montre quelles tâches ont été sautées.

## Où ça s'inscrit

La recherche approfondie est l'extrémité la plus lourde du composer chat — elle fait en dix minutes ce qu'un analyste ferait en un après-midi. Couple cette page avec [Concepts d'agent](/fr/platform/agents/concepts) (le modèle à quatre boutons sur lequel l'agent Chercheur est construit) et [Aperçu des intégrations](/fr/platform/integrations/overview) (où Tavily se trouve à côté des autres intégrations que la ceinture d'outils de l'agent peut atteindre). Si tu veux construire ton propre agent de recherche plutôt que d'utiliser celui livré, [Créer un agent](/fr/platform/agents/create) parcourt la construction d'agent de bout en bout.
