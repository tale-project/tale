...
| ----------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code | Géré ou le tien    | Peut être guidé en cours de tour — un commentaire de tâche l’atteint pendant que le travail est encore en cours. Prend le canal MCP. |
...
Les connectors connectés atteignent l’agent par un broker, pas par la boîte. Quand l’agent en appelle une, la requête repart vers Tale, qui l’exécute avec l’accès stocké et ne renvoie que le résultat : un conteneur compromis ne peut donc pas lire tes clés. Le broker ne porte que des actions de lecture : une écriture — poster un message, envoyer un mail, ouvrir un ticket — est refusée avec un motif lisible, ces écritures ne passent donc pas par le broker. Utilise pour cela un nœud connector d’automatisation. GitHub est l’exception assumée : `git` et la CLI `gh` ont besoin d’un jeton en local ; tant que l’agent a le connecteur GitHub équipé, chaque exécution reçoit un jeton restreint — injecté à chaque exécution, disparu dès sa fin.

Les bundles équipés sont préparés comme fichiers dans la sandbox et nommés dans les instructions de l’exécution. Examine leurs consignes et scripts : ils peuvent guider l’usage des outils disponibles. [Skills des agents](/fr/platform/agents/skills) explique le périmètre et la préparation. Les **Secrets** accordés sont injectés dans l’environnement de l’exécution ; consulte [Agents de projet](/fr/platform/projects/project-agents).
