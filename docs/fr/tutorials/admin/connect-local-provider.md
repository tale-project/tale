...
- **Format du fichier de clé API** — si tu édites les fichiers de fournisseurs directement, le fichier de clé API doit correspondre au mode configuré : chiffré par SOPS quand `SOPS_AGE_KEY` est défini, sinon JSON en clair. Définir la clé via l'UI écrit la bonne forme pour toi ; voir [Fournisseurs IA — Stockage des secrets de fournisseur](/fr/self-hosted/configuration/providers#stockage-des-secrets-de-fournisseur).

## Où ça s'inscrit

Connecter un fournisseur local est le chemin air-gap : l'inférence IA tourne sur du matériel que tu contrôles, sans que le trafic ne quitte le réseau. Les surfaces de configuration des fournisseurs sont documentées exhaustivement à [Fournisseurs IA](/fr/platform/admin/providers) (UI) et [Configuration des fournisseurs](/fr/self-hosted/configuration/providers) (fichiers de configuration). Une fois un fournisseur local câblé, chaque autre surface — agents, automatisations, chat — l'utilise de la même façon qu'un fournisseur hébergé ; la différence est opérationnelle, pas comportementale.
