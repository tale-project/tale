...
| Le flux de fichiers projet — chaque `POST` sous `/api/v1/projects/{id}/...` (dossiers, handoffs de chargement, liaisons de fichiers)  | 240 requêtes / min | 300    |

Le second bucket est petit à dessein : chacune de ces requêtes coûte une exécution durable entière ou un tour de modèle, pas une lecture de base. Le troisième est spacieux à dessein : un fichier coûte ici au moins deux appels — demander le handoff, lier le fichier — le budget couvre donc toute la chorégraphie. Deux appels tirent sur plus d'une voie : l'appel dossier compte aussi contre le budget général, le démarrage de tâche contre le général et celui du travail à démarrer — dimensionne sur la voie la plus étroite. Un token bucket se remplit en continu — la capacité de rafale absorbe un lot, puis le débit soutenu s'applique.
