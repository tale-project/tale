...
Arena Mode sendet dieselbe Nachricht gleichzeitig an zwei verschiedene KI-Modelle und vergleicht ihre Antworten in einer geteilten Ansicht. Nutze ihn, um Modellqualität zu bewerten, neue Modelle gegen deinen aktuellen Standard zu testen oder Präferenzdaten in deinem Team zu sammeln.

## Arena Mode aktivieren

1. Öffne eine beliebige Chat-Konversation.
2. Klicke auf das **Schwerter**-Icon in der Toolbar der Chat-Eingabe. Das Icon leuchtet, wenn Arena Mode aktiv ist.
...
Um Arena Mode zu deaktivieren, klicke erneut auf das Schwerter-Icon. Der gesamte Arena-Zustand (Modellauswahl, Threads, Verdikt) wird zurückgesetzt.

> **Hinweis:** Arena Mode benötigt mindestens zwei verfügbare Modelle. Ist nur ein Modell konfiguriert, wird der Selector ausgeblendet und der Schalter deaktiviert.
...
Du kannst im Arena Mode weiter Nachrichten senden. Jede neue Nachricht geht parallel an beide Modelle.
...
Wenn du eine Nachricht im Arena Mode sendest, tut die Plattform Folgendes:
...
sequenceDiagram
    ...
    Du->>Plattform: Nachricht im Arena Mode senden
