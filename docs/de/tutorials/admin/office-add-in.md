...
Der Schritt hat funktioniert, wenn der Settings-Dialog ohne Fehler schließt und das „Test"-Feature des Panels oder die erste Anfrage Inhalt zurückgibt.
...
- **Von Tale zum Modell-Anbieter**: der Prompt, den der Agent sendet, genau wie es eine Chat-UI-Anfrage täte. Um diese Strecke im Netz zu halten, kombiniere dieses Tutorial mit [Lokalen Anbieter verbinden](/de/tutorials/admin/connect-local-provider).
...
- **Modell nicht gefunden** — die Modell-ID beachtet Groß-/Kleinschreibung und muss exakt zu `GET /api/v1/models` passen. Aus der Antwort kopieren, nicht aus dem **KI-Anbieter**-UI-Label.
...
Das Office-Add-in-Tutorial routet Microsoft-365-Verkehr zu einem Tale-Agent, ohne den Bearbeitungs-Workflow des Nutzers zu ändern. Dieselbe OpenAI-kompatible API-Oberfläche, die das Add-in nutzt, ist voll dokumentiert in der [API-Referenz](/de/develop/api-reference); der Agent, den das Add-in ruft, wird über [Den ersten Agent end-to-end bauen](/de/tutorials/editor/first-agent-end-to-end) gebaut; der Pro-Konversations-Audit-Trail liegt in der Chat-Historie unter [Konversationen](/de/platform/workspace/conversations).
