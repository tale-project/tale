...
Verwaltete Agentenläufe zählen für die Person, die sie gestartet hat. Ein Lauf, den du aus einer Aufgabe, einem Kommentar, über die REST-API oder den MCP-Endpoint startest, verbraucht deine persönlichen und Team-Grenzen; ein mit einem API-Schlüssel gestarteter Lauf zählt zusätzlich für diesen Schlüssel. Läufe, die ein Zeitplan, ein Webhook oder ein Ereignis gestartet hat, haben keine Person dahinter: Für sie gelten nur die Organisationslimits, und die [Nutzungsanalyse](/de/platform/admin/governance/usage-analytics) führt sie unter **Automatisierungen (Trigger)**.
