...
Standardmäßig sieht jedes Mitglied mit Inbox-Zugriff jede Konversation der Organisation. Unter **Einstellungen > Governance > Richtlinien & Grenzen** öffnest du **Konversationszugriff** und aktivierst **Zugewiesene Konversationen einschränken**, um eine zugewiesene Konversation privat zu machen: eine einem Team zugewiesene ist nur für dessen Mitglieder sichtbar, eine einer Person zugewiesene nur für diese Person. Nicht zugewiesene Konversationen bleiben ein gemeinsamer organisationsweiter Pool, den jede und jeder übernehmen kann, und Admins sowie Inhaber sehen immer alles.

Der Schalter ist für bestehende Organisationen aus – es ändert sich also nichts, bis er von einem Admin eingeschaltet wird. Die Sichtbarkeit folgt dem [Team](/de/platform/admin/teams) und dem Mitglied, dem eine Konversation zugewiesen ist – beides wird über die Zuweisungs-Auswahl in der Kopfzeile der Konversation gesetzt. Kombiniere ihn mit der Automatisierung **Eingehende Konversationen nach Adresse zuweisen**, um eingehende Post gleich beim Eintreffen direkt in die Inbox des passenden Teams einzusortieren.
