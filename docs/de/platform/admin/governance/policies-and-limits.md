...
## Konversationssteuerung nach Zuständigkeit

Standardmäßig sieht jedes Mitglied mit Inbox-Zugriff jede Konversation der Organisation. Unter **Einstellungen > Governance > Richtlinien & Grenzen** öffnest du **Konversationssteuerung nach Zuständigkeit** und aktivierst **Für diese Organisation aktiviert**, um eine zugewiesene Konversation privat zu machen: eine einem Team zugewiesene ist nur für dessen Mitglieder sichtbar, eine einer Person zugewiesene nur für diese Person. Nicht zugewiesene Konversationen bleiben ein gemeinsamer organisationsweiter Pool, den jede und jeder übernehmen kann, und Admins sowie Inhaber sehen immer alles.

Der Schalter ist für bestehende Organisationen aus – es ändert sich also nichts, bis er von einem Admin eingeschaltet wird. Die Sichtbarkeit folgt dem [Team](/de/platform/admin/teams) und dem Mitglied, dem eine Konversation zugewiesen ist – beides wird über die Zuweisungs-Auswahl in der Kopfzeile der Konversation gesetzt.

## Konversations-Routing

Eingehende Post landet unzugewiesen, sofern keine Routing-Regel sie beansprucht. Unter **Einstellungen > Governance > Richtlinien & Grenzen** öffnest du **Konversations-Routing** und legst eine Regel an, die eine Empfängeradresse einem Team, einer Person oder beiden zuordnet: Die nächste Konversation, die an dieser Adresse eintrifft, wird im Moment ihrer Erstellung zugewiesen, bevor jemand die Inbox öffnet. Eine Regel trifft auf die Adresse zu, an die die absendende Person geschrieben hat — das `An` der Konversation — und zwar unabhängig von Groß- und Kleinschreibung; eine Adresse ohne Regel bleibt unzugewiesen im gemeinsamen Pool.

Routing weist nur zu; eine Konversation, die bereits eine Inhaberin oder ein Team hat, wird nie neu zugewiesen — eine Antwort, die sich in einen bestehenden Thread einreiht, bleibt also unberührt. Kombiniere es mit **Konversationssteuerung nach Zuständigkeit** oben, um weitergeleitete Post im Moment des Eintreffens privat für das Team zu machen, bei dem sie landet. Eine Regel, die auf ein zwischenzeitlich gelöschtes Team oder eine gelöschte Person zeigt, wird übersprungen — die Konversation trifft trotzdem ein, nur unzugewiesen.
