...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `rag_search` | Das Wissen der Organisation und ihre Arbeit — Dokumente, Wissenseinträge, gecrawlte Website-Seiten, Produkte, Kontakte, Websites, Aufgaben und Projekte sowie die Konversationen des Posteingangs —, begrenzt auf den Ort, an dem der Chat lebt (siehe unten) |
...
**Wo die Tools suchen, entscheidet der Ort des Chats, nicht der Assistent.** Ein Dokument lebt an genau einem Ort — in der Wissensdatenbank der Organisation oder in einem Projekt, nie in beiden. Wer ein Dokument der Wissensdatenbank sieht, entscheiden seine Team-Tags: ohne Tags jedes Mitglied, mit Tags die Mitglieder eines der genannten Teams — und Admins bekommen keine Ausnahme. Wer eine Projektdatei sieht, entscheidet allein der Projektzugriff — ein org-weites Projekt heißt jedes Mitglied, ein Team-Projekt seine besitzenden und geteilten Teams, und Inhaber und Admins sehen jedes Projekt; eine Projektdatei trägt nie Team-Tags. Ein Chat, der in einem Projekt gestartet wurde, sieht die Dateien dieses Projekts plus die Wissensdatenbank, so wie du sie siehst, und sein Board ist das dieses Projekts; der Organisations-Chat sieht nur die Wissensdatenbank — und jedes Board, das du lesen darfst. Jeder Chat sieht außerdem seine eigenen Anhänge und die E-Mail-Anhänge der Konversationen, die du lesen darfst. Gelöschte und abgelaufene Dateien sind überall unsichtbar; ein archiviertes Projekt hält seine Dateien lesbar, als archiviert gekennzeichnet. Die Grenze hält der Server: Ein Projekt-Chat greift nie in ein anderes Projekt hinein, und eine Frage nach den Dateien eines Projekts beantwortet der Chat des Projekts selbst, nicht der Organisations-Chat — dort gestellt, listet sie die Dateien des Projekts neben denen der Wissensdatenbank, jede mit dem Vermerk, wo sie lebt und ob sie indexiert ist.
...
- Jeder Tool-Aufruf ist eine Schrittzeile — _Durchsucht den Workspace nach "…"_, _Listet Aufgaben auf_, _Liest example.com_, _Liest lead-verify.txt_ — mit einem Spinner, solange er läuft, und einer Warnung samt Grund, wenn er scheitert. Ein Dokument-Schritt nennt die Datei auch dann, wenn das Lesen scheitert, und der Grund sagt, was im Weg steht: eine ohne Indexierung hochgeladene Datei, eine, die noch indexiert wird, eine, deren Indexierung fehlgeschlagen ist. Die Schritte bleiben sichtbar, wenn das Nachdenken eingeklappt ist; sie sind das Protokoll dessen, wonach der Assistent gegriffen hat.
