...
GitHub ist die Ausnahme, bei der auch ein Token in die Sandbox gelangt, weil `git` und das `gh`-CLI es lokal brauchen: Verbinde GitHub unter [Integrationen](/platform/integrations/overview) und binde es an den Agenten, dann erhält die Sitzung ein begrenztes Token, mit dem der Agent in deinem Namen klonen, pushen und Pull Requests öffnen kann. Alle Anmeldedaten — das GitHub-Token in der Sandbox ebenso wie die vermittelten — sind auf die Sitzung beschränkt, werden bei jedem Aufruf protokolliert und beim Ende der Sitzung widerrufen.
