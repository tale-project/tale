...
- Die Dienste `rag` und `crawler` lesen ihre gemeinsame Anbieter-Konfiguration jetzt aus `convex-data:/app/platform-config:ro` statt aus `platform-data`.
...
Siehe [Produktions-Deployment → Schema-Kompatibilität und Rollback](/de/self-hosted/install/linux-server#schema-kompatibilität-und-rollback) für den Umgang mit Schema-Änderungen über Versionen hinweg.
...
- **Nur-Vorwärts-Schema**: `tale rollback` tauscht Container-Images, aber nicht Convex-Daten. Pflichtfeld-Erweiterungen, Umbenennungen und Typ-Änderungen folgen dem Zwei-Release-Expand-Contract-Muster aus [Schema-Kompatibilität und Rollback](/de/self-hosted/install/linux-server#schema-kompatibilität-und-rollback).
