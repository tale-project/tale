...
Die Anmeldeseite von Tale leitet den Browser selbst auf die Übergabeadresse weiter, sobald die Anfrage den Identitäts-Header des Proxys trägt, sodass Mitglieder das Anmeldeformular nie sehen; leitet der Proxy `/log-in` direkt auf die Übergabeadresse, entfällt dieser Umweg — das bleibt empfohlen. Endet eine Übergabe ohne Sitzung (abgewiesen oder Cookies in einem Frame blockiert), zeigt die Seite das Formular mit einer Schaltfläche zum erneuten Versuch, statt in einer Schleife weiterzuleiten.
