...
- **Dateispeicher** — wo hochgeladene Dateien (die Original-Blobs) liegen. Standardmäßig im mitgelieferten Object-Store (dem Service `object-store`, auf eigenem Volume); die `OBJECT_STORE_*`-Variablen richten das Backend stattdessen auf einen externen S3-kompatiblen Bucket aus. Der Wechsel ist Greenfield: Blobs, die schon im mitgelieferten Store liegen, werden nicht kopiert — setze ihn bei der ersten Installation, oder kopiere das Volume vorab außerhalb von Tale in den Bucket.
