...
| `TALE_ALLOW_PRIVATE_CRAWL_HOSTS`  | unset                    | Erlaubt, dass ein Crawl-Ziel (`POST /api/v1/websites` und jeder Abruf des Crawlers) — und die `imageUrl` eines Produkts, die als String derselben Host-Regel unterliegt und nie abgerufen wird — einen Loopback-, Link-Local- oder Privatnetz-Host nennt (RFC 1918, CGNAT, ULA, `.internal`, `.local`, einteilige Namen). Nicht gesetzt, antwortet ein solches Ziel mit **400** `WEBSITE_DOMAIN_NOT_CRAWLABLE` (`INVALID_BODY` beim Produkt) — der Crawler wählt aus dem eigenen Netz des Deployments heraus. Setz die Variable nur auf `1`, wenn das Deployment sein eigenes Intranet crawlt; die Cloud-Metadaten-Endpoints bleiben in jedem Fall gesperrt. |
