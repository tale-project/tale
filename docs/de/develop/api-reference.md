...
Jede API-Antwort enthält `X-Request-Id`. Zur Zuordnung in Logs kannst du bis zu 255 Zeichen aus Buchstaben, Ziffern, `_`, `-` und `=` senden. Ein ungültiger Wert wird durch eine neue UUID ersetzt; die Antwort nennt die tatsächlich verwendete Kennung. Bei `429`, `500`, `413` und `414` steht `requestId` zusätzlich in der JSON-Hülle.
...
Steuerzeichen unter 0x20 außer Tabulator sowie DEL in Headerwerten werden vor der Plattform abgelehnt: HTTP/1.1 liefert eine reine Textantwort mit `400`; HTTP/2 setzt den Stream zurück oder schließt eine Verbindung mit Body.
...
| Skills | `/api/v1/skills/...`<br>Skill-Bundles der Organisation auflisten, lesen, anlegen oder ändern und löschen; jede Datei eines Bundles lesen. Jeder Skill nennt seine Version (`etag`, `updatedAt`), und ein Speichern lässt sich mit `If-Match` absichern. |
...
`null` leert ein optionales Feld. Bei `PATCH` wird auch ein leerer String als `null` behandelt. Ein leeres Pflichtfeld, etwa der Produktname, ergibt **400** `INVALID_BODY`. Beim Anlegen und Massenimport gelten leere Strings dagegen als ausgelassene Felder. Der Massenimport verlangt mindestens einen Kontakt; `contacts: []` ergibt **400** `INVALID_BODY`.
...
Sende beim Aktualisieren oder Löschen das zuletzt gelesene `etag` als `If-Match`. Hat sich die Datei geändert oder existiert sie nicht mehr, folgt **412** `SKILL_STALE` mit dem aktuellen Tag in `data.etag` (`null`, wenn keine Datei existiert). Es wird nichts geschrieben. Lade den aktuellen Inhalt und führe die Änderungen zusammen, bevor du erneut speicherst.
...
`GET /api/v1/skills/{slug}/files/{path}` liest jede Datei des Bundles — `SKILL.md` eingeschlossen — als rohe Bytes, benannt über `Content-Disposition`, mit `path` genau so, wie `files[].path` es listet (`/` roh oder als `%2F`); ein Pfad, den die Liste nie tragen würde, ergibt **404**, `SKILL_FILE_NOT_FOUND`, und ein Bundle, das die Dateischicht ablehnt — ein eingeschleuster Symlink, eine Datei über der Staging-Grenze von 4 MiB — **422**, `SKILL_MALFORMED`.
...
`GET /api/v1/conversations/sync` nennt außerdem die gebundene `externalContactId` und `contactStatus`: `active`, `trashed` oder `missing`. Die Bindung behält die ursprüngliche Kontakt-ID. Ein gelöschter Kontakt liegt im Papierkorb; seine E-Mail und externe ID werden wieder frei. Ein neuer Kontakt mit denselben Kennungen übernimmt jedoch niemals den alten Konversationsverlauf.

Ein neuerer Inhalts-Snapshot für einen Kontakt im Papierkorb ergibt `409 CONVERSATION_CONTACT_TRASHED`. Stelle den Kontakt in der App wieder her, bevor du weiteren Inhalt sendest, oder beende die Spiegelung mit `deleted: true` und einer gleichen oder höheren Version. Ältere Snapshots bleiben ignoriert, und für Wiederholungen derselben Version gelten weiterhin die Versionsregeln oben. Das Löschen macht also nicht jede Wiederholung zu einem Fehler.
...
Ein `PATCH` ersetzt den aktiven Eintrag durch eine neue Version und indexiert unter derselben `documentId` erneut. Sind `topic` und `content` nach dem Trimmen unverändert, entsteht keine Version; die Antwort nennt die vorhandene Eintrags-ID. Ein Löschen verschiebt das zugehörige Dokument in den Papierkorb.
...
`GET .../triggers` liest die Bindung zurück — als `triggers`, eine Liste mit höchstens einem Eintrag, der eine Plural in dieser Familie — samt ihrer Gesundheit: `lastFiredAt` ist der letzte Zeitpunkt, zu dem diese Bindung **einen Lauf gestartet** hat — `lastRunId` nennt ihn, und beide bleiben `null`, bis es so weit war —, während `lastSkippedAt` und `lastSkipReason` das letzte Mal festhalten, dass sie fällig war und nichts gestartet hat: `not_deployed` (nichts ist live — deploy eine Version), `unusable_cron` (der Ausdruck oder die Zone ließ sich nicht lesen; der Scheduler lässt die Bindung in Ruhe, bis sie bearbeitet wird) oder `start_refused` (das `inputs`-Schema der Live-Version hat die Eingabe des Laufs abgelehnt).
...
| --- | --- |
| `queued` | Angenommen, wartet auf einen Worker; weiter abfragen |
...
Für inkrementelle Updates sende die bisherige `textLength` als `since` und `reasoningLength` als `reasoningSince`. `textOffset` und `reasoningOffset` zeigen den Beginn der zurückgegebenen Ausschnitte. Liegt ein Offset unter deinem gesendeten Wert, hat eine abgeschlossene Tool-Runde diesen Ausgabestrom zurückgesetzt. Ersetze dann den betreffenden lokalen Puffer.
...
`parts` ist eine geordnete Liste mit dem Unterscheidungsfeld `type`: `text`, `reasoning`, `attachment`, `tool-call`, `tool-result`, `approval` oder `human-input`. OpenAPI beschreibt jede Variante. Behandle künftig unbekannte Varianten als undurchsichtige Daten, statt die gesamte Nachricht abzulehnen.
...
`query` ist erforderlich und wird vor der Prüfung getrimmt. Optional sind `limit` (1–50, Standard 10) und `minSimilarity` (0–1). Tale sucht zunächst in einem größeren Kandidatenpool, prüft die aktuelle Sichtbarkeit jedes Dokuments und begrenzt erst nach dem Zusammenführen die Ergebnismenge. `limit: 1` liefert so die beste lesbare Passage.
...
| `similarity` | Kosinuswert der Vektorsuche auf der modellabhängigen Skala 0–1; `null` bei reinen Stichworttreffern. Für eine Schwelle nutzbar, aber keine kalibrierte Wahrscheinlichkeit. |
...
Jeder Treffer enthält die Passage und ihre `source`. `diagnostics.cached` und `diagnostics.reranked` sind für zusätzliche semantische Caches oder Reranker vorgesehen; in der ausgelieferten Implementierung bleiben beide `false`. Dokumenttreffer enthalten neben der Blob-`ref` auch `source.documentId`:
...
| **431** | die Anfrage-Kopfzeilen insgesamt übersteigen das 64-KiB-Budget des Edge; unter HTTP/1.1 kommt die Antwort ohne Umschlag (und erst nach ein paar KiB Spielraum — eine URL oder Kopfzeile knapp über dem Budget erreicht die Plattform noch und wird nach deren Regeln beurteilt, eine 66-KiB-URL antwortet **414**), unter HTTP/2, wo das Budget exakt gilt, wird die Verbindung geschlossen. |
