...
Eine erfolgreiche Antwort ist eine benannte Liste: `{ "automations": [ { "name": "billing/dunning", "latestVersion": 3, "deployedVersion": 2 } ] }`. Die Listenform hängt von der Familie ab: die meisten antworten mit einem benannten Array wie diesem, während die Wissens- und Chat-Ressourcen — Kontakte, Produkte, Dokumente, Wissenseinträge, Threads, Websites — mit einem `{ "page": [...], "isDone": ..., "continueCursor": ... }`-Umschlag antworten. Wo ein Umschlag paginiert, gib `continueCursor` als `?cursor=` zurück und begrenze die Seite mit `?limit=`: Kontakte, Produkte, Dokumente, Wissenseinträge, Threads und Websites paginieren alle so bis zur letzten Seite (`isDone: true` mit leerem `continueCursor`). Lauflisten paginieren genauso, nur unter ihrem eigenen Schlüssel — `{ "runs": [...], "isDone": ..., "continueCursor": ... }`, die neuesten zuerst, `?limit=` (1..200, Standard 50; Werte außerhalb werden auf den Bereich begrenzt) pro Seite — und `GET /api/v1/runs` listet Läufe über alle Automatisierungen hinweg. Der Maschinenzugang unter Projekte reist noch leichter — sein Abschnitt zeigt jene Formen.
...
| Dokumente                 | `/api/v1/documents/...`                                                                                                                    | Dokumente der Wissensdatenbank: CRUD (ein `PATCH` antwortet mit dem aktualisierten Dokument) plus `POST .../retry-indexing`; jedes dateigestützte Dokument trägt seinen `indexing`-Zustand. Projektdateien tauchen hier nie auf — sie leben unter Projekte.            |
...
| Browser-Sessions          | `/api/v1/browser-sessions/...`                                                                                                             | Der vorgewärmte Cookie-Pool hinter der [Video-Ingestion](/de/self-hosted/configuration/video-ingestion): maskierte Liste, `POST .../import` und `DELETE .../{id}` für Operatoren auf der Allowlist — eine Session lebt standardmäßig 14 Tage und höchstens 180.        |
...
| Gespräche                 | `/api/v1/conversations/...`                                                                                                                | Externe Gespräche als versionierte Snapshots in den Posteingang spiegeln, die Snapshot-Quittung einer Quelle lesen, native Antworten abholen und ihre Zustellung bestätigen oder als fehlgeschlagen melden; genaue Schemas stehen unter `/docs` der laufenden Instanz. |
...
`GET .../triggers` liest die Bindung zurück: `lastFiredAt` ist der letzte Zeitpunkt, zu dem diese Bindung gefeuert hat, und ein Umbinden auf eine andere Art setzt diese Uhr neu. `enabled: false` pausiert einen Trigger, ohne ihn zu verlieren; `DELETE .../triggers` entfernt ihn — und bei einem Webhook widerruft es die URL.
...
Polle `GET /api/v1/projects/{id}/runs/{runId}?fields=status,finishedAt`, bis `status` `queued`/`running`/`waiting` verlässt — mit den benannten Schlüsseln bleibt der Poll eine Zeile statt des ganzen Laufs, und der `ETag` der Antwort als `If-None-Match` macht aus einem unveränderten Poll eine **304** ohne Body ([Caching](#caching-kompression-und-gezieltes-lesen)); dann lies den Lauf vollständig: Er trägt `output`, den `trace` pro Knoten und die `effects`, die er erzeugt hat. `POST /api/v1/projects/{id}/runs/{runId}/cancel` stoppt einen Lauf an seiner nächsten Knotengrenze — was ein Knoten schon getan hat, wird nicht rückgängig gemacht.
...
curl -sS --compressed "https://your-host.example.com/api/v1/projects/<projectId>/threads/<threadId>/generation" \
  -H "Authorization: Bearer $TALE_API_KEY" \
  -H "X-Organization-Slug: <org-slug>"
# → 200 { "status": "queued", "messageId": "..." } … dann { "status": "streaming", "messageId": "...", "text": "Das Quartal…", "reasoning": "", "cancelRequested": false, "updatedAt": 1774... } … dann { "status": "idle" }
```

`{"status": "idle"}` bedeutet, dass gerade kein Turn läuft. Lies die Antwort mit `GET /api/v1/projects/{id}/threads/{threadId}/messages`. `queued` heißt, das Senden ist angenommen und wartet auf einen Worker; `streaming` heißt, das Modell streamt seine Antwort an den Server, und `text` und `reasoning` tragen, was bisher angekommen ist — einen Push-Kanal gibt es auf dieser Oberfläche nicht, polle also alle zwei bis fünf Sekunden mit einem Client-Timeout von mindestens dreißig Sekunden. Behalte die `messageId`, die die 202 nennt — das ist die Assistenten-Nachricht, in der die Antwort landet. Geht dir die 202 verloren (eine abgerissene Verbindung nach dem Senden), polle `.../generation` — `queued` oder `streaming` heißt, der Turn läuft; `idle` heißt, lies die Nachrichten und finde die Antwort über diese `messageId` oder über die neueste `user`-Zeile mit deinem `content`.

Jeder Turn läuft über den eingebauten Arbeitsbereich-Assistenten: Seine Anweisungen, seine Sicherheitsregeln und seine drei Recherche-Tools reisen mit jeder Anfrage mit — rund 3.000 Prompt-Tokens, gezählt in `usage.inputTokens` —, und wer ein Arbeitsergebnis verlangt (ein Dokument, einen Bericht), wird mit Absicht zu den Aufgaben umgeleitet. Das ist ein Gespräch mit dem Arbeitsbereich, kein nackter Modellaufruf. Zwei optionale Felder begrenzen einen Turn: `reasoningEffort` wählt die Denktiefe auf derselben fünfstufigen Skala wie die App (`low`, `medium`, `high`, `extra`, `max`; ein Modell ohne `capabilities.reasoning` ignoriert es), und `maxOutputTokens` deckelt die Antwort — es darf das eigene `maxOutputTokens` des Modells aus `GET /api/v1/models` nicht übersteigen, sonst ergibt das Senden **400**, `INVALID_BODY`, mit der Obergrenze; ein denkendes Modell hält sein Denkbudget unter dem Deckel. Listen, Threaddetails, Nachrichten und Statusabfragen zeigen nur die eigenen Threads des Schlüsselbesitzers in diesem Projekt. Die Threads anderer Benutzer bleiben unsichtbar, auch im gemeinsamen Projekt. `GET /api/v1/projects/{id}/threads` listet deine Threads; `GET /api/v1/projects/{id}/threads/{threadId}` liest einen davon.

`content` wird vor der Prüfung getrimmt — ein leerer Prompt ergibt **400**, statt einen Turn zu verbrauchen; `locale` ist ein BCP-47-Tag (`de`, `en-GB`) und nennt die Sprache, in der der Assistent antwortet. Jede Nachricht trägt einen `status`: Solange ein Turn läuft, steht seine Assistenten-Zeile bereits als `pending` mit leeren `parts` auf der Seite — die Zeile, die `.../generation` als `messageId` nennt — und wird am Ende des Turns `complete`, nach einem Stopp `cancelled` (mit der Teilausgabe, die schon gestreamt war) oder `failed` mit `error` und `errorCode`. `usage` trägt die Token-Zähler und die Kosten: `reasoningTokens` ist der Anteil von `outputTokens`, der aufs Denken ging, `cachedInputTokens` der Anteil von `inputTokens`, der aus dem Cache des Anbieters kam, und `costEstimateCents` (US-Cent mit Nachkommastellen) ist die Katalogschätzung, die das Nutzungsbuch der Organisation für denselben Turn verbucht — sie fehlt, wenn der Katalog für das Modell keinen Preis kennt. `usage` fehlt bei einem Turn, der scheiterte, bevor der Anbieter Zähler gemeldet hat. `parts` ist eine geordnete Liste, unterschieden nach `type` — `text`, `reasoning`, `attachment`, `tool-call`, `tool-result`, `approval`, `human-input` —, und das OpenAPI-Dokument typisiert jede Art. Ein `reasoning`-Teil ist das Denken des Modells und kann die eigenen Anweisungen des Assistenten wörtlich zitieren: Zeig ihn als das, was er ist, nie als die Antwort. Das Vokabular ist additiv — eine Art, die du nicht kennst, gibst du opak wieder.

Für persönliche Chats ohne Projekt verwendest du `/api/v1/threads` sowie die zugehörigen Detail-, Nachrichten- und Statuspfade. Projektthreads sind dort nicht erreichbar. Ein falsches Projekt in der URL ergibt **404**. Beide Chatarten verwenden den eingebauten Assistenten; `projectId`, `agentSlug` oder `agentId` beim Anlegen oder Senden ergibt **400**. Projektleser einschließlich Mitgliedern dürfen Threads anlegen und Nachrichten senden. Ein archiviertes Projekt verweigert diese Aufrufe mit **403**. Ein archivierter Thread verweigert eine Nachricht mit **409**, `CHAT_THREAD_ARCHIVED`, ein Sandbox-Thread mit **409**, `CHAT_THREAD_NOT_DIRECT`, und ein Thread, dessen Turn noch läuft, mit **409**, `CHAT_TURN_IN_PROGRESS` — den letzten wiederholst du, sobald die Abfrage idle meldet, die anderen beiden nie.

Den Lebenszyklus steuerst du über dieselben URLs. `PATCH .../threads/{threadId}` mit `{ "archived": true }` archiviert einen Thread, `false` holt ihn zurück, und `{ "title": "Q3 review" }` benennt ihn um (sende mindestens eines von beiden); einen ohne Titel angelegten Thread benennt der Assistent nach seiner ersten Nachricht, und das `title` des Threads trägt den Namen in beiden Fällen. Archivieren stempelt `archivedAt` auf den Thread und lässt `updatedAt` in Ruhe — `updatedAt` ist die letzte Nachrichtenaktivität, ein Abgleich, der darauf schaut, muss also `archived` und `archivedAt` lesen, um ein Archivieren oder Zurückholen zu sehen. `DELETE .../threads/{threadId}` verschiebt ihn in den Papierkorb (**409**, `CHAT_TURN_IN_PROGRESS`, solange ein Turn läuft); `DELETE .../threads/{threadId}/generation` bittet den laufenden Turn zu stoppen — **202** `{ "status": "cancelling" }`, danach pollst du bis idle; die gestoppte Antwort landet als `status: "cancelled"` mit dem, was schon gestreamt war; läuft nichts, **404**, `CHAT_TURN_NOT_RUNNING`. Ein archiviertes Projekt verweigert alle drei mit **403**.
...
- **400** — fehlerhafte Anfrage: fehlendes Pflichtfeld, falscher Typ, unbekannter Schlüssel, nicht parsebarer Body, ein Body, der kein UTF-8 ist, ein String mit NUL-Zeichen, eine ganze Zahl jenseits von 2^53 − 1 — die Antwort trägt `code: "INVALID_BODY"` und listet unter `data.issues` jedes Problem mit Feld (`price`, `contacts.2.email`; ein unbekannter Schlüssel ist ein eigenes Problem unter seinem Namen) und Grund, also korrigier, was sie benennt. Ein Body wird als JSON gelesen, egal was `Content-Type` sagt; ein 415 gibt es an dieser Tür nie; ein `cursor`, den die Liste nie geliefert hat (`INVALID_CURSOR`), ein `limit`, das keine ganze Zahl ist (`INVALID_LIMIT` — Werte außerhalb des Bereichs werden stattdessen auf ihn begrenzt), oder ein anderer Query-Parameter, den die Route ablehnt (`INVALID_QUERY`) — nichts davon wird als erste Seite gelesen; eine Lauf-Eingabe, die das `inputs`-Schema der Automatisierung ablehnt (`AUTOMATION_INPUT_INVALID`, wieder mit `data.issues`), oder ein Trigger, der nie feuern könnte (`AUTOMATION_TRIGGER_INVALID`); ein Kontakt-Patch, der das letzte Identitätsfeld leeren würde (`CONTACT_IDENTITY_REQUIRED`); oder ein Multi-Org-Schlüssel, der seine Organisation nicht benannt hat (`ORG_SLUG_REQUIRED`).
- **401** — fehlender oder ungültiger API-Schlüssel (`UNAUTHORIZED`), mit einer `WWW-Authenticate: Bearer`-Challenge.
- **403** — die Rolle (`ROLE_FORBIDDEN`) oder Projektbearbeitungsrechte fehlen, die gewünschte Änderung betrifft ein archiviertes Projekt oder eine archivierte Aufgabe (`PROJECT_ARCHIVED`, `TASK_ARCHIVED`), die Automatisierung darf in diesem Projekt nicht laufen, oder `X-Organization-Slug` benennt eine Organisation, in der der Schlüsselbesitzer kein Mitglied ist (`ORG_FORBIDDEN`).
- **404** — die Ressource fehlt, ist für den Schlüsselbesitzer unsichtbar, gehört einem anderen Threadbenutzer oder liegt in einem anderen Projekt als dem der URL; jede Familie nennt ihren eigenen Code (`PROJECT_NOT_FOUND`, `THREAD_NOT_FOUND`, …), ein `X-Organization-Slug`, der keine Organisation benennt, antwortet `ORG_SLUG_INVALID`, und eine unbekannte Route antwortet `NOT_FOUND`.
- **405** — die Route existiert, aber nicht für dieses Verb (`METHOD_NOT_ALLOWED`); `Allow` nennt die Verben, die sie bedient.
- **409** — der Zustand verhindert die Aktion: keine bereitgestellte Version, eine gebundene Automatisierung ohne Projekt-URL, ein beim Löschen noch laufender Lauf (`RUN_ACTIVE`), ein `Idempotency-Key`, der mit anderem Body wiederverwendet wurde (`IDEMPOTENCY_KEY_REUSED`), ein archivierter Thread oder laufender Turn, ein doppeltes Thema, eine doppelte E-Mail oder `externalItemId`, ein abgelöster Wissenseintrag (`KNOWLEDGE_ENTRY_SUPERSEDED`), ein veraltetes `expectedUpdatedAt` (`CONTACT_STALE`, `PRODUCT_STALE`, `DOCUMENT_STALE`), ein Dokument, hinter dem ein aktiver Wissenseintrag steht (`DOCUMENT_HAS_KNOWLEDGE_ENTRY` — lösche oder ändere stattdessen den Eintrag), oder eine Suche ohne Embedding-Modell.
- **412** — eine Vorbedingung ist gescheitert: `If-None-Match: *` auf einem Skill-Slug, der schon ein Bundle hat (`SKILL_EXISTS`); nichts wurde geschrieben.
- **413** — der Body ist zu groß (`BODY_TOO_LARGE`): jeder JSON-Body an seiner Grenze (1 MiB, sofern die Operation nichts anderes sagt — die Grenzen stehen oben), der Webhook-Trigger an seiner Grenze von 256 KiB (262.144 Bytes). Eine hochgeladene Datei, die die Größen- oder Typ-Policy verletzt, wird beim Binden stattdessen mit **400** und einem Reason-Code abgewiesen.
- **422** — ein Skill-Body, den die Dateischicht nicht lesen kann (`SKILL_MALFORMED`).
...
- **431** — die Anfrage-URL übersteigt 32 KiB; die Antwort kommt vom Edge, ohne Umschlag.
...
Der aktuelle REST-Präfix lautet `/api/v1/`. Das OpenAPI-Dokument unter `/openapi.json` beschreibt die Routen sowie Anfrage- und Antwortschemas der laufenden Instanz, mit `servers` auf genau diese Instanz gesetzt; `/docs` rendert es. Verwende es als Vertrag für deinen Client.
