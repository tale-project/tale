...
Der Workflow läuft asynchron; der Anrufer wartet nie auf den Output. Status und Schritt-Ergebnisse liegen im **Ausführungen**-Tab des Workflows — siehe [Ausführungslogs](/de/platform/automations/execution-logs).
...
- **Vom Anrufer zu Tale**: der POST-Body und die Header, einschließlich des Tokens in der URL. HTTPS schützt alles in Transit; das Token wird nicht als Header gesendet, also bleibt es aus Standard-`Authorization`-Log-Zeilen heraus, erscheint aber in der URL jedes Access-Logs, das der Anrufer schreibt. Behandle es entsprechend.
