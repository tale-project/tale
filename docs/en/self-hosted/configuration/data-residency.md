...
Data residency settings select where Tale stores application data, knowledge content and uploaded files. Deployment defaults provide the initial storage connections; organization-specific connections can place one organization’s knowledge corpus or files on separate infrastructure.

Decide which data must move before changing a connection. Pointing new writes at a new store and migrating existing data are different tasks; follow the relevant database or object-storage procedure below.
...
- **Knowledge database** — the knowledge corpus: document metadata, the extracted chunk text, embeddings, the BM25 index, the semantic cache, and the crawled web pages. It ships as the bundled `knowledge-db` container (`tale_knowledge`, with the `private_knowledge` and `public_web` schemas) and is the store most residency requirements care about, because it holds your document content. `KNOWLEDGE_DATABASE_URL` points the backend at a managed Postgres of your own instead. The database can start empty of tables — the backend creates its schemas on first use — but it must already have `pgvector` installed, because Tale never installs extensions on a database it does not own.
- **File storage** — where uploaded files (the original blobs) live. By default they sit in the bundled object store (the `object-store` service, on its own volume). The `OBJECT_STORE_*` variables point this at any S3-compatible bucket instead — AWS S3, MinIO, Cloudflare R2, Wasabi. They are read on every start, and the backend keeps `$TALE_CONFIG_DIR/default/object-storage/connection.json` (plus `connection.secrets.json` for the access keys, SOPS-encrypted when a key is configured) in step with them, so repointing the store or rotating its credentials is an `.env` change plus a restart. Switching is greenfield: blobs already written to the old store are not copied, so move them out of band first — and read [Backups and restore](/self-hosted/operate/backups-and-restore), because a repointed default takes the blobs out of `tale backup`'s snapshots.
- **Application database** — the operational store behind agents, runs, and the audit log (the bundled `db` container, the `tale_app` database). `DATABASE_URL` relocates it; the database name defaults to `tale_app` (override with `APP_DB_NAME`). It needs no extensions and no superuser — just a database and a role that may create schemas.

Set deployment-wide storage variables in the deployment’s `.env`. Apply changes with `tale deploy`, or recreate the affected backend services in your own deployment workflow. `docker compose restart` retains the old container environment. Verify the active connection and coordinate data migration before changing a database or bucket: a new URL does not move existing data. The [environment reference](/self-hosted/configuration/environment-reference) lists the variables and defaults. The former `dataStores` block in `deployment.yml` is ignored; configure the environment instead.

The backend says what it did with the object-store variables in its boot log — `seeded` on a deployment that had no connection, `reconciled` when they changed, `ignored` when the file is yours to manage, `skipped` when no credential pair is set. If you would rather manage `connection.json` by hand, set `"managedBy": "operator"` in it and the backend will never touch it again.

> **Two constraints on any external Postgres.** Neither database may sit behind a **transaction-mode** connection pooler — the job queue holds `LISTEN` connections, the migrator holds a session-scoped advisory lock, and queries use prepared statements, so point Tale at the database's own port rather than at PgBouncer in transaction mode, Supabase's pooler port, or a pinning RDS Proxy. And if you ask for `sslmode=verify-ca` or `verify-full`, set `POSTGRES_CA_FILE` to a PEM bundle containing your provider's root: Amazon RDS and most managed providers sign with roots that are not in the certificate store Node ships, and without the bundle the connection is refused at boot rather than silently downgraded.
