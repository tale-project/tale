...
| `OBJECT_STORE_ACCESS_KEY`      | unset                         | **Required.** S3 access key, and the store's root user — both sides read the same value. The shipped compose passes `tale`; the process itself has no default, and the backend skips the blob store when either key is missing. |
...
| `OBJECT_STORE_PUBLIC_ENDPOINT` | `${SITE_URL}` (set by the CLI) | Where the browser reaches the store. The proxy publishes it at `/<bucket>/*` and forwards presigned URLs verbatim, so uploads and downloads run browser↔store directly. Read at the first boot only — see below. |
...
The first boot seeds the deployment-default connection from these variables into the config volume (`default/object-storage/connection.json`) and never overwrites a default that already exists — an operator who repointed the store keeps their edit. So a stack that booted without `OBJECT_STORE_PUBLIC_ENDPOINT` hands browsers presigned URLs on the internal hostname, and setting the variable afterwards does not change that: add `"publicEndpoint"` to the seeded connection file and restart the backend. `tale deploy` writes the variable for you, so this is the hand-rolled-compose case ([Run Compose yourself](/self-hosted/install/own-compose)).

## Audit log signing

The audit hash chain is made tamper-evident by an HMAC-SHA256 signature over its retention and PII-scrub checkpoints (SOC 2 CC7.2, ISO 27001); the daily integrity cron verifies it. A second key pseudonymises the personal data a failed sign-in leaves in the chain.
