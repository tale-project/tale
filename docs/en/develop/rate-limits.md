...
The second bucket is deliberately small: each of those requests costs a whole durable run or a model turn, not a database read. The third is deliberately roomy: one file costs at least two calls here — mint the handoff, bind the file — so the lane is budgeted for the whole choreography. Every request also counts against the general budget — it is the door — so a starting-work or upload POST draws from two lanes at once, and the tighter one governs; plan against it. A token bucket refills continuously — the burst capacity absorbs a batch, then the sustained rate applies.
