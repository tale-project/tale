...
The body requires `query` and also accepts `limit` (1–50) and `minSimilarity` (0–1). A missing embedding model answers **409**, `EMBEDDING_NOT_CONFIGURED`; so does an embedding provider that refuses for account reasons — a spent balance, or a plan that excludes the model — as **409**, `EMBEDDING_CREDIT_EXHAUSTED`. Neither is a rate limit: no wait lifts them, an admin has to act. Any other provider failure answers **503**, `EMBEDDING_UPSTREAM_ERROR` — retry that one with backoff. To search visible non-project Hub and team documents or registered websites instead, use `POST /api/v1/knowledge/search` with `corpus` set to `"documents"`, `"web"` or `"all"` (the default). That URL excludes project files and email attachments. Both URLs find file-backed documents only — a document created with inline `content` never indexes.
