...
| Repository work — bugs, features, refactors, PRs                           | A **Sandbox agent** with the right dispatch: tale-daemon (`runtime`) for git workspaces, durable sandbox when configured, or accept that sandbox-only agents still run the platform loop on the board until you add those fields |
