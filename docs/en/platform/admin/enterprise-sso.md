...
3. Configure the proxy to send its sign-ins to the **Hand-off URL** with the key in the key header and the identity headers listed under **Header names**. Tale's sign-in page hands off to that address by itself when it sees the identity headers; pointing the proxy's `/log-in` there as well saves the extra round trip.
