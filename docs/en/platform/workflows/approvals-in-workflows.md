...
Approval gates are how a workflow puts a human between two automated steps. The natural next read is [Workflow concepts](/platform/workflows/concepts) for the surrounding model — definitions, triggers, steps, executions — and [Approval concepts](/platform/approvals/concepts) for the cross-trigger story of what every approval carries.
