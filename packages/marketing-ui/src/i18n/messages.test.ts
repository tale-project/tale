 ...
 *
 * Every string a `@tale/marketing-ui` component renders on its own lives
 * here, so this catalog is gated exactly like a service's: locale parity, no
 * orphan keys, and no `t('literal')` in `src/` that resolves to nothing (the
 * raw-key bug class).
 */
...
defineI18nTests({
  ...
  allowlistDisplayPath: 'packages/marketing-ui/src/i18n/keys-dynamic.yml',
  modes: {
