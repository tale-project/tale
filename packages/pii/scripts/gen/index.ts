 ...
 *
 * Usage:
 *   bun packages/pii/scripts/gen/index.ts [--lang en,de,fr,...] [--seed 42]
 *   bun packages/pii/scripts/gen/index.ts verify
 *
 * The default run regenerates every locale that has a dataset directory
 * under `packages/pii/datasets/`. `--lang` narrows to a subset. `verify`
 * regenerates to a temp tree and diffs against the committed fixtures —
 * a non-empty diff fails the CI.
 *
 * Offline guarantee: this CLI reads only from `packages/pii/datasets/`
