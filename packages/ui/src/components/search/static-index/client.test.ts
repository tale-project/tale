...
const EN_INDEX = '/search-index-en.json';
const DE_INDEX = '/search-index-de.json';
...
describe('loadIndex', () => {
  ...
  it('fetches the index JSON and hydrates a MiniSearch instance', async () => {
    const fetchSpy = stubFetch((url) => {
      expect(url).toBe(EN_INDEX);
      return makeIndexResponse(SAMPLE_DOCS);
    });
    const ms = await loadIndex(EN_INDEX);
    expect(fetchSpy).toHaveBeenCalledTimes(1);
    expect(ms.search('configuration').length).toBeGreaterThan(0);
  });

  it('fetches exactly the URL it is given, deploy base included', async () => {
    const fetchSpy = stubFetch((url) => {
      expect(url).toBe('https://cdn.example.com/docs/search-index-en.json');
      return makeIndexResponse(SAMPLE_DOCS);
    });
    await loadIndex('https://cdn.example.com/docs/search-index-en.json');
    expect(fetchSpy).toHaveBeenCalledTimes(1);
  });

  it('returns the cached instance for repeat calls on the same index', async () => {
    const fetchSpy = stubFetch(() => makeIndexResponse(SAMPLE_DOCS));
    const a = await loadIndex(EN_INDEX);
    const b = await loadIndex(EN_INDEX);
    expect(a).toBe(b);
    expect(fetchSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it('throws when the fetch returns a non-2xx', async () => {
    ...
    await expect(loadIndex(EN_INDEX)).rejects.toThrow(/failed to load .*: 404/);
  });
  ...
  it('throws a legible error when a 200 returns HTML (SPA fallback)', async () => {
    ...
    await expect(loadIndex(EN_INDEX)).rejects.toThrow(
      /non-JSON \(text\/html\)/,
    );
  });
  ...
  it('is race-safe across indexes: concurrent calls resolve to distinct instances', async () => {
    stubFetch((url) => {
  ...
  it('dedupes concurrent calls for the same index onto one fetch', async () => {
    const fetchSpy = stubFetch(() => makeIndexResponse(SAMPLE_DOCS));
  ...
  it('resetSearchCache clears every index', async () => {
    const fetchSpy = stubFetch(() => makeIndexResponse(SAMPLE_DOCS));
    await loadIndex(EN_INDEX);
    resetSearchCache();
    await loadIndex(EN_INDEX);
...
describe('search', () => {
  ...
  it('returns [] for an empty / whitespace query', async () => {
    expect(await search(EN_INDEX, '')).toEqual([]);
    expect(await search(EN_INDEX, '   ')).toEqual([]);
  });

  it('returns results for a single-token query', async () => {
    const rows = await search(EN_INDEX, 'configuration');
  ...
  it('highlights *what was matched* — prefix expansion surfaces "configuration" for query "config"', async () => {
    const rows = await search(EN_INDEX, 'config');
    const top = rows[0];
  ...
  it('falls back to OR when AND yields a thin result set for multi-token', async () => {
    ...
    const rows = await search(EN_INDEX, 'windows configuration');
    expect(rows.length).toBeGreaterThan(0);
  });
  ...
  it('boosts proximity: co-located terms beat spread-out terms', async () => {
    ...
    const rows = await search(EN_INDEX, 'configure rag');
