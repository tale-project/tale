...
> {
  ...
  required?: boolean;
  /**
   * Explains *why* the checkbox is disabled, surfaced in a tooltip on hover
   * AND focus and to screen readers. Only takes effect while the checkbox is
   * `disabled`; ignored otherwise, so callers can pass it unconditionally.
   * The control stays focusable with `aria-disabled` and activation is
   * blocked instead of the native attribute making it unreachable.
   */
