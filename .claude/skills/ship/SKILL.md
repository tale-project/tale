...
The last mile, done right. `ship` is the routine that turns a finished change into a mergeable PR
without leaving cross-cutting work undone. It composes the other workflow skills — don't duplicate
their detail, run them.
...
4. **Verify behaviour** ([`verify`](../verify/SKILL.md)) — run `/verify` for anything a user can see or call. A green gate is necessary, not sufficient.
...
Don't open the PR to "see if CI passes" in place of running the gate locally — that wastes CI and
reviewer time. Don't bundle unrelated changes into one PR. Don't mark a box done you didn't do.
