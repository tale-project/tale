...
under [`skills/pptx/scripts/`](../../../skills/pptx/scripts/) — the source of truth that
`bun run skills:sync` copies to `builtin-configs/skills/pptx/` (the shipped bundle; never hand-edit it).
...
Editing or adding any `.py` file — in practice the pptx skill scripts above. Edit the source under
`skills/pptx/`, then run `bun run skills:sync` to regenerate the `builtin-configs/` copy (CI's
`skills:check` fails on drift). Touching the sandbox _launch_ path (`entrypoint.sh`,
...
Path) -> set[str]:` ([`clean.py:27`](../../../skills/pptx/scripts/clean.py)). Use
...
"schemas"`, `.rglob("*.xml")`, `.relative_to(…)`, `.read_text(encoding="utf-8")`
  ([`office/validators/base.py:98`](../../../skills/pptx/scripts/office/validators/base.py)).
...
- **Never a bare `except:`, never silently swallow.** Catch the specific exception and _act_ — collect
  ...
  [`base.py:145`](../../../skills/pptx/scripts/office/validators/base.py) is
...
- **Context managers for resources** so they always close: `with open(schema_path, "rb") as …:`,
  ...
  ([`base.py:763`,`805`,`808`](../../../skills/pptx/scripts/office/validators/base.py)).
...
- **f-strings for interpolation**, never `%` or `str.format`. Send diagnostics to `stderr`
  ...
  ([`clean.py:279`](../../../skills/pptx/scripts/clean.py)).
- **Parse untrusted XML with `defusedxml`** to block entity-expansion (billion-laughs) attacks —
  `import defusedxml.minidom` ([`clean.py:21`](../../../skills/pptx/scripts/clean.py)).
...
directly.")` ([`base.py:854`](../../../skills/pptx/scripts/office/validators/base.py)).
...
([`clean.py`](../../../skills/pptx/scripts/clean.py) style):
