...
Convex runs as its own service; function source is pushed on startup and `api.d.ts` is committed
(regen with `bunx convex codegen`). Filenames are **snake_case** here (the one exception to the repo's
dash-case rule). Data-model changes go through the [`convex-migrations`](../convex-migrations/SKILL.md)
guide; per-query/per-request performance lives in [`performance`](../performance/SKILL.md).
...
- **Use the RLS wrappers, not raw `query`/`mutation`.** `queryWithRLS` / `mutationWithRLS` (import
  from [`../lib/rls`](../../../services/platform/convex/lib/rls/)) inject a row-level-security-wrapped
  `ctx.db` and a request-scoped auth context. Raw `query`/`mutation` bypass access control.
...
- **Validate args with `v` and declare `returns`.** Every function validates its `args` and its
  ...
  `json.ts`) and per-domain `validators.ts`.
- **Auth cost matters.** In read queries prefer `getAuthUserIdentity` (0 DB reads, JWT only) over
  `authComponent.getAuthUser` (2 DB reads). See [`convex/lib/rls/auth/`](../../../services/platform/convex/lib/rls/auth/).
- **No `@deprecated` tombstones.** Delete the dead function and update its callers; git is the record.
- **Never `import 'node:*'` in V8 code.** File I/O and Node APIs live in `'use node'` modules; pass
  their _output_ into V8 functions — see the boundary section below.
...
Pagination — use the native `.paginate()` with `paginationOptsValidator`; for heavily-filtered scans
use `paginateWithFilter` from [`convex/lib/pagination/`](../../../services/platform/convex/lib/pagination/helpers.ts)
(it tracks last-_scanned_ id so a tail row isn't lost). Don't hand-roll cursors.
...
inward; the migration framework's `NodeMigrationHelpers` is the canonical seam. Symptom of getting
it wrong: `bun run dev` dies in Convex bundling with `Could not resolve "node:*"`.

```ts
// ❌ V8 mutation importing fs transitively → bundling failure
import { readConfig } from './config-loader'; // a 'use node' module

// ✅ Node action reads, V8 receives data. (sync.ts opens with the `'use node'`
//    directive on its own first line — kept out of this snippet so it isn't reflowed.)
...
HTTP routes (Hono) live in [`convex/http.ts`](../../../services/platform/convex/http.ts): validate
every request body, never log secrets, and keep auth checks on the handler.
