...
async function main() {
  ...
  // Release validation must resolve the same native connector node types even
  // when the installed binary has no checkout. Keep this executable catalogue
  // separate from the reference files that `init` writes into a project.
  const connectors = new Map<string, string>();
  const connectorRoot = join(REPO_ROOT, 'configs/platform/system/connectors');
  if (!existsSync(connectorRoot))
    throw new Error('Native connector catalogue missing');
  await collectFiles(connectorRoot, connectorRoot, 'connectors', connectors);
  ...
  const output = [
    '// AUTO-GENERATED by scripts/generate-embedded.ts — do not edit',
    '',
    `export const EMBEDDED_CLI_BUILD = ${JSON.stringify(cliBuild())} as const;`,
    '',
    ...
    '',
    `export const EMBEDDED_CONNECTORS: Record<string, string> = ${JSON.stringify(Object.fromEntries([...connectors].filter(([name]) => name.endsWith('/connector.yml'))), null, 2)};`,
    '',
  ].join('\n');
...
/** A bundle records the source used to build its executable. Exported source
 * archives may have no Git metadata; they still build normal CLI commands but
 * cannot claim a commit-verified deployment executable. */
function cliBuild(): { revision: string | null; clean: boolean } {
  try {
