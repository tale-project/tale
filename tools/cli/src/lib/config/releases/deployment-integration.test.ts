...
test('managed deployment consumes the same source stage and preserves native receipt replay', async () => {
  ...
  const first = await provisionDeploymentConfigs(bundle, context, dependencies);
