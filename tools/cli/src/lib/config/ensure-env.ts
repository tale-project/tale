...
function generateEnvContent(config: EnvConfig): string {
  ...
  lines.push(
    ...
    '# Live browser view (read-only mirror in the chat UI). Default OFF. When set',
    '# to 1, the spawner launches session containers with a headed Chromium +',
    '# x11vnc mirror (TALE_BROWSER_CDP) and the platform attaches Playwright MCP',
    "# over CDP, so the agent's browser is streamed read-only into the web page.",
    ...
    '# so they stay in lockstep. Off = the agent still uses a headless browser,',
    '# just with no live preview.',
    '# SANDBOX_BROWSER_VIEW=1',
    '',
    '# ============================================================================',
