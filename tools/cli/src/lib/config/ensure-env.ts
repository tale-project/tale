...
function generateEnvContent(config: EnvConfig): string {
  ...
  lines.push(
    ...
    '#   - SOPS_AGE_KEY_FILE set → key sourced from a file (Vault, K8s Secret',
    '#                             mount, systemd LoadCredential). Encrypted.',
    '#   - Both unset            → plaintext JSON at mode 0600. Use only if disk',
    '#                             is encrypted at rest, or you provision the',
    '#                             *.secrets.json files via external tooling.',
