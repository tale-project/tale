...
function generateEnvContent(config: EnvConfig): string {
  const lines: string[] = [
    ...
    `HOST=${config.host}`,
    `SITE_URL=${config.siteUrl}`,
    '# Serve this same deployment on more domains (comma- or whitespace-',
    '# separated origins). Each becomes a full entry point: the proxy serves and',
    '# certifies it, sign-in works on it, and links keep a visitor on the domain',
    '# they arrived on. SITE_URL stays canonical for e-mail links, SAML entityID,',
    "# SCIM meta.location and passkeys. Point each domain's DNS here and register",
    '# its own OAuth/SSO callback URL (Settings lists them per domain).',
