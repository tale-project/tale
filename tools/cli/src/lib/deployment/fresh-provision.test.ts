...
async function fixture() {
  ...
  await prepareDeploymentConfig({
    ...
    descriptorPath: path.relative(source.root, source.descriptorPath),
      ...
      configs: [
        {
          ...
          client: 'north-labs',
          descriptor: path.relative(source.root, source.descriptorPath),
...
test('fresh managed command freezes source and holds one native lock through identity, inference and config callbacks', async () => {
...
test('fresh command rejects input drift before native state, and concurrent provisioning before identity', async () => {
  const f = await fixture();
  let identities = 0;
  ...
  await withLock(stateDirectory, 'synthetic competing operation', async () => {
    await expect(
      provisionManagedBundle(
        f.directory,
        f.input,
...
test('fresh owner compilation deploys exact native bytes into a create-once project and retains replay receipts', async () => {
  const f = await fixture();
  ...
  const dependencies = {
    ...
    deploy: async (options: DeployOptions) => {
      ...
      expect(release.manifest.sourceCommit).toBe(f.source.options.sourceCommit);
  ...
  expect(second[0]).toMatchObject({ ...(first[0] as object), unchanged: true });
