...
under [`builtin-configs/skills/pptx/scripts/`](../../../builtin-configs/skills/pptx/scripts/) — a
product skill shipped to org agents from there (embedded in the CLI binary, seeded per-org at chat time).
...
Editing or adding any `.py` file — in practice the pptx skill scripts above, under
`builtin-configs/skills/pptx/`. CI's `skills:check` verifies every script a `SKILL.md` references
actually exists. Touching the sandbox _launch_ path (`entrypoint.sh`,
...
Path) -> set[str]:` ([`clean.py:27`](../../../builtin-configs/skills/pptx/scripts/clean.py)). Use
...
"schemas"`, `.rglob("*.xml")`, `.relative_to(…)`, `.read_text(encoding="utf-8")`
  ([`office/validators/base.py:98`](../../../builtin-configs/skills/pptx/scripts/office/validators/base.py)).
...
- **Never a bare `except:`, never silently swallow.** Catch the specific exception and _act_ — collect
  ...
  [`base.py:145`](../../../builtin-configs/skills/pptx/scripts/office/validators/base.py) is
...
- **Context managers for resources** so they always close: `with open(schema_path, "rb") as …:`,
  ...
  ([`base.py:763`,`805`,`808`](../../../builtin-configs/skills/pptx/scripts/office/validators/base.py)).
...
- **f-strings for interpolation**, never `%` or `str.format`. Send diagnostics to `stderr`
  ...
  ([`clean.py:279`](../../../builtin-configs/skills/pptx/scripts/clean.py)).
- **Parse untrusted XML with `defusedxml`** to block entity-expansion (billion-laughs) attacks —
  `import defusedxml.minidom` ([`clean.py:21`](../../../builtin-configs/skills/pptx/scripts/clean.py)).
...
directly.")` ([`base.py:854`](../../../builtin-configs/skills/pptx/scripts/office/validators/base.py)).
...
([`clean.py`](../../../builtin-configs/skills/pptx/scripts/clean.py) style):
