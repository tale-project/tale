...
assertion rarely says which rule broke or where the contract lives — this companion names every check,
what it catches, and the fix. Writing/voice rules live in [SKILL.md](SKILL.md); cross-locale
terminology lives in [`write-translations`](../write-translations/SKILL.md).
...
**Content** — each page reads like a real page (the shape contract from [SKILL.md](SKILL.md)):
...
5. **Prose shape** (`structure-opening`, `structure-closing`) — last, because it's the deepest _content_ fix; rewrite to the page-shape contract in [SKILL.md](SKILL.md).
...
- **Terminology, voice, loanwords, pronouns, German grammar, ICU.** These moved out of this suite into
  ...
  the voice rules and how to fix a terminology failure, read [`write-translations`](../write-translations/SKILL.md)
