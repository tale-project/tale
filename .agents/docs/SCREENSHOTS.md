...
Capture product screenshots through Playwright, reusing the existing harness at `services/platform/e2e` so the capture runs against a real, seeded instance with the same auth and fixtures the tests use. Drive the app to the exact state, set a fixed viewport so images stay consistent across captures, and screenshot the specific element or region rather than the whole tab — a cropped panel reads better and stays under the size ceiling.
...
The broader docs contract lives in [AGENTS.md](AGENTS.md); the per-locale rules for translated alt text and callouts live in the companion [`translation`](../translation/AGENTS.md) skill. Read those for voice and locale parity; read this file before you reach for a capture tool.
