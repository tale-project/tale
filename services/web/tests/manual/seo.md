...
| F9  | JSON-LD structured data | `curl -s {base}/`, `{base}/pricing`, `{base}/changelog`, `{base}/platform/chat`; extract every `<script type="application/ld+json">` block | `/` and `/pricing` embed the **Organization** + **SoftwareApplication** nodes (`lib/seo/organization.ts`, `lib/seo/software-application.ts` — Ruler GmbH, `@id: …/#software`); `/changelog` embeds a **BreadcrumbList** + an **ItemList** of releases; platform pages embed a **BreadcrumbList** (Tale → Platform overview → page) + **FAQPage**; every block parses as valid JSON |
