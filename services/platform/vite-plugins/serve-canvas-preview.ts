...
export function serveCanvasPreview(): Plugin {
  ...
  // Registered on both the dev server (`configureServer`) and the prod-build
  // preview server (`configurePreviewServer`, the E2E serving path) — both
  // expose `config.publicDir` and a connect `middlewares` stack. No
  // `apply: 'serve'` so the preview hook can fire.
  const register = (server: ViteDevServer | PreviewServer) => {
    ...
    const canvasLibsHandler: Connect.NextHandleFunction = (req, res, next) => {
