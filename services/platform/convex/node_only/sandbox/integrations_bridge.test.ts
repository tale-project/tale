// Coverage for the integrations-bridge dispatch actions — the server side of
// `tale-integrations-mcp`. Runs against the REAL shipped connector catalog
...
// mocked at ctx.runAction — its own behavior is covered by the integrations
...
async function getActions(): Promise<{
  ...
  const mod = await import('./integrations_bridge');
  return {
    dispatch: (mod.dispatchBridgeIntegration as unknown as { handler: Handler })
      .handler,
    status: (mod.bridgeIntegrationStatus as unknown as { handler: Handler })
      .handler,
  };
}
...
describe('dispatchBridgeIntegration', () => {
  beforeEach(() => vi.clearAllMocks());

  it('refuses an unshipped integration without dispatching', async () => {
    const ctx = createCtx({});
  ...
  it("surfaces the dispatcher's coded refusal with its hint", async () => {
    const runAction = vi.fn().mockRejectedValue(
      new ConvexError({
        ...
        hint: 'Connect one under Settings → Integrations.',
      }),
    );
    const ctx = createCtx({ runAction });
    const { dispatch } = await getActions();
    ...
    expect(result.message).toContain('Settings → Integrations');
  });
});

describe('bridgeIntegrationStatus', () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it('says plainly when nothing is equipped', async () => {
    ...
    expect(String(result.note)).toContain('No integrations are equipped');
  });
  ...
  it('reports usable / blocked per granted slug', async () => {
    ...
    const bySlug = new Map(
      result.integrations.map((entry) => [entry.slug, entry]),
    );
    ...
    expect(bySlug.get('not-shipped')).toMatchObject({
      usable: false,
      blockers: [{ code: 'unknown_integration' }],
    });
  });
});
