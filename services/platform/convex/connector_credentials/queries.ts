/**
 * Read surface of the connector-credential domain.
 *
...
function toMasked(row: Doc<'connectorCredentials'>) {
  return {
    id: row._id,
...
export const getCredentialInternal = internalQuery({
  args: { credentialId: v.id('connectorCredentials') },
...
export const resolveCredentialRefInternal = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const asId = ctx.db.normalizeId('connectorCredentials', ref);
    if (asId !== null) {
