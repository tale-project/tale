/**
 * Update contact metadata fields (business logic)
 *
 * This mutation safely updates nested metadata fields in a contact record.
 * It uses lodash for safe deep merging to avoid overwriting existing nested data.
 *
 * IMPORTANT: This function requires a valid contactId. It will throw an error
 * if the contact is not found, preventing accidental bulk updates.
 */

import { ConvexError } from 'convex/values';
import merge from 'lodash/merge';
import set from 'lodash/set';

import type { Id } from '../_generated/dataModel';
import type { MutationCtx } from '../_generated/server';
import { toConvexJsonRecord } from '../lib/type_cast_helpers';

interface UpdateContactMetadataResult {
  success: boolean;
  contactId: Id<'contacts'>;
}

export async function updateContactMetadata(
  ctx: MutationCtx,
  contactId: Id<'contacts'>,
  metadataUpdates: Record<string, unknown>,
): Promise<UpdateContactMetadataResult> {
  // Get the existing contact
  const contact = await ctx.db.get(contactId);
  if (!contact) {
    throw new ConvexError({
      code: 'CONTACT_NOT_FOUND',
      message: `Contact not found: ${contactId}`,
    });
  }

  // Get existing metadata or initialize empty object
  const existingMetadata = contact.metadata ?? {};

  // Process metadata updates
  // Support both flat keys and dot-notation keys
  const updatedMetadata: Record<string, unknown> = { ...existingMetadata };

  for (const [key, value] of Object.entries(metadataUpdates)) {
    if (key.includes('.')) {
      // Use lodash.set for dot-notation keys (e.g., "churn.survey.sent")
      set(updatedMetadata, key, value);
    } else {
      // For top-level keys, use merge to preserve nested objects
      if (
        typeof value === 'object' &&
        value !== null &&
        !Array.isArray(value) &&
        typeof updatedMetadata[key] === 'object' &&
        updatedMetadata[key] !== null &&
        !Array.isArray(updatedMetadata[key])
      ) {
        // Deep merge objects
        updatedMetadata[key] = merge({}, updatedMetadata[key], value);
      } else {
        // Direct assignment for primitives, arrays, or when replacing
        updatedMetadata[key] = value;
      }
    }
  }

  // Update the contact with the new metadata
  await ctx.db.patch(contactId, {
    metadata: toConvexJsonRecord(updatedMetadata),
  });

  return {
    success: true,
    contactId,
  };
}
