...
import { findRelatedConversation } from './helpers/find_related_conversation';
...
export async function createConversationFromSentEmail(
  ...
  if (existing) {
    ...
    for (const email of emailsArray) {
      ...
      // Determine customer email (reuse logic from below)
      ...
      for (const threadEmail of emailsArray) {
        if (threadEmail.from?.[0]?.address)
          allAddresses.add(threadEmail.from[0].address.toLowerCase());
        if (threadEmail.to?.[0]?.address)
          allAddresses.add(threadEmail.to[0].address.toLowerCase());
      }
  ...
  // Determine the customer email from the thread
    ...
    const created = await ctx.runMutation(
      internal.conversations.createConversationWithMessage,
      {
        ...
        initialMessage: buildInitialMessage(
          rootEmail,
          rootIsFromCustomer,
          'delivered', // All emails synced from mailbox should be "delivered"
        ),
      },
    );
