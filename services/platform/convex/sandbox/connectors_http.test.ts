// Coverage for the /api/connectors/{execute,status} HTTP handlers — the
...
async function getHandlers(): Promise<{
  ...
  const mod = await import('./connectors_http');
  return {
    execute: mod.connectorsExecuteHandler as unknown as HttpHandler,
    status: mod.connectorsStatusHandler as unknown as HttpHandler,
  };
}
...
const AUTH = {
  ...
  sessionId: 'session_1',
  connectorGrants: ['tavily'],
  toolGrants: [],
  userId: 'user_1',
};

function postJson(body: unknown): Request {
  return new Request('http://convex/api/connectors/execute', {
    method: 'POST',
...
describe('connectorsExecuteHandler', () => {
  ...
  it('answers invalid_args for a non-JSON body', async () => {
    ...
    const res = await execute(
      ctx,
      new Request('http://convex/api/connectors/execute', {
        method: 'POST',
        body: 'not json',
      }),
    );
...
describe('connectorsStatusHandler', () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it('401s an unauthenticated call', async () => {
    ...
    const res = await status(
      ctx,
      new Request('http://convex/api/connectors/status', { method: 'POST' }),
    );
  ...
  it("reports the token's grant set", async () => {
    mockAuthSessionToken.mockResolvedValue(AUTH);
    const runAction = vi.fn().mockResolvedValue({ connectors: [] });
    const ctx = { runAction };
    const { status } = await getHandlers();
