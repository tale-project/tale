/**
 * The in-sandbox connectors dispatch surface —
 * `POST /api/connectors/{execute,status}` — the platform end of the baked
 * `tale-connectors-mcp` bridge.
 *
...
export const connectorsExecuteHandler = httpAction(async (ctx, request) => {
  ...
  const result: unknown = await ctx.runAction(
    internal.node_only.sandbox.connectors_bridge.dispatchBridgeConnector,
    {
...
export const connectorsStatusHandler = httpAction(async (ctx, request) => {
  const auth = await authSessionToken(ctx, request);
  if (auth === null) {
    return json(401, { status: 'error', message: 'Unauthorized.' });
  }
  const result: unknown = await ctx.runAction(
    internal.node_only.sandbox.connectors_bridge.bridgeConnectorStatus,
    {
      organizationId: auth.organizationId,
