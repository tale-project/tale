...
/**
 * Per-agent setup readiness for an automation's bundled agents — the agent half of the
 * install-wizard / readiness checklist (the integration half is
 * `getAutomationInstallState`). Orchestrates three existing reads and the pure
 * classifier: `listAutomationAgents` (the automation's agent configs incl. `metadata.requires.env`),
...
export const getAutomationAgentReadiness = action({
  ...
  handler: async (ctx, args): Promise<unknown> => {
    ...
    const rawAgents = (await ctx.runAction(
      api.agents.file_actions.listAutomationAgents,
      {
