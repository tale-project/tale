...
const DIR = 'migrations/versions/v0_4_0/34_chat_memories_table';
...
describe('0.4.0/34_chat_memories_table (reference)', () => {
  it('up leaves rows untouched; down empties the table (idempotent)', async () => {
    ...
    await t.run(async (ctx) => {
      for (let pass = 0; pass < 2; pass++) {
        for (const doc of await ctx.db.query('memories').collect()) {
    ...
    // row.
    await t.run(async (ctx) => {
      const seeded = await ctx.db.query('memories').collect();
      for (let pass = 0; pass < 2; pass++) {
        for (const doc of seeded) {
          await migration.down(ctx, doc as never);
        }
      }
      expect(await ctx.db.query('memories').collect()).toHaveLength(0);
    });
  });
});
