...
const DIR = 'migrations/versions/v0_4_0/10_drop_slack_event_dedup';
...
defineMigrationTest({
  ...
  async expectUp(world) {
    const rows = await world.run((ctx) =>
      ctx.db.query('slackEventDedup').collect(),
    );
    ...
    expect(
      snaps.map((s) => (s.payload as { eventId: string }).eventId).sort(),
    ).toEqual(['Ev0WORLD001', 'Ev0WORLD002']);
  },
});
