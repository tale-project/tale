...
const DIR = 'migrations/versions/v0_4_0/26_automation_workflows_table';
...
describe('0.4.0/26_automation_workflows_table (reference)', () => {
  it('up leaves rows untouched; down empties the table (idempotent)', async () => {
    ...
    await t.run(async (ctx) => {
      for (let pass = 0; pass < 2; pass++) {
        for (const doc of await ctx.db.query('workflows').collect()) {
          await migration.up(ctx, doc as never);
        }
      }
      const rows = await ctx.db.query('workflows').collect();
      expect(rows).toHaveLength(1);
      expect(rows[0].name).toBe('billing/dunning');
    });
    ...
    // row.
    await t.run(async (ctx) => {
      const seeded = await ctx.db.query('workflows').collect();
      for (let pass = 0; pass < 2; pass++) {
        for (const doc of seeded) {
          await migration.down(ctx, doc as never);
        }
      }
      expect(await ctx.db.query('workflows').collect()).toHaveLength(0);
    });
  });
});
