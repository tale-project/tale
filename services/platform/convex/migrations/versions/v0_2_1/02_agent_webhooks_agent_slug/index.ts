/**
 * Reference migration: rename `agentWebhooks.agentFileName` → `agentSlug`.
 *
 * Per-row, idempotent, shape-guarded so a re-run is a no-op. The runner never
 * executes a `reference` migration; this transform exists so the documented
 * history stays under round-trip test. The test calls `up`/`down` directly.
 */
