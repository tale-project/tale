/**
 * 0.3.4 / 37 — remap wfSchedules rows onto their automations.
 *
 * Sibling of 36_remap_wf_installations for the `wfSchedules` table: each retired
...
export const migration = defineDbMigration({
  ...
  description:
    'Patches every wfSchedules row whose workflowSlug is a retired standalone slug to its automation slug per the shared cutover map; down applies the inverse map, restoring rows byte-for-byte.',
  destructive: false,
  snapshot: 'none',
  subjects: { tables: ['wfSchedules'] },
  table: 'wfSchedules',
