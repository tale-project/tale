 ...
 * outlook/reply-emails / gmail/reply-emails / imap-smtp/reply-emails apps, so an
 ...
 * renamed action-style (`outlook-inbox` → `reply-outlook-emails`) and then
 * FILED under their provider (`outlook/reply-emails` — an automation slug is
 * the path it lives at) while the branch was still unreleased; this historical
 * migration is re-seedable, so the constant is edited in place to the current
 * slugs rather than pinned to the old names.
 */
export const INTEGRATION_TO_EMAIL_APP: Readonly<Record<string, string>> = {
  outlook: 'outlook/reply-emails',
  gmail: 'gmail/reply-emails',
  imap_smtp: 'imap-smtp/reply-emails',
};
...
export const migration = defineNodeMigration({
  ...
  description:
    ...
    '(outlook/reply-emails / gmail/reply-emails / imap-smtp/reply-emails) via installAutomationInternal with ' +
