/**
 * 0.3.4 / 40 — remap wfDefaultProvisions rows onto their automations.
 *
 * Sibling of 36_remap_wf_installations for the `wfDefaultProvisions` table: each retired
...
export const migration = defineDbMigration({
  ...
  description:
    'Patches every wfDefaultProvisions row whose workflowSlug is a retired standalone slug to its automation slug so the automation provisioner honors prior provisioning and org opt-outs; down applies the inverse map, restoring rows byte-for-byte.',
  destructive: false,
  snapshot: 'none',
  subjects: { tables: ['wfDefaultProvisions'] },
  table: 'wfDefaultProvisions',
  ...
  async up(ctx, doc) {
    ...
    await ctx.db.patch(doc._id as Id<'wfDefaultProvisions'>, {
