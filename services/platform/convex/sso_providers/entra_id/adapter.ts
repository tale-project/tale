...
import { MICROSOFT_LOGIN_BASE, MICROSOFT_GRAPH_BASE, ONEDRIVE_SCOPES, extractTenantId } from './constants';
...
function buildAuthorizeUrl(config: SsoProviderConfig, params: AuthorizeUrlParams): URL {
	const tenantId = extractTenantId(config.issuer);
	const authUrl = new URL(`${MICROSOFT_LOGIN_BASE}/${tenantId}/oauth2/v2.0/authorize`);
...
async function exchangeCodeForTokens(config: SsoProviderConfig, params: TokenExchangeParams): Promise<SsoTokens> {
	const tenantId = extractTenantId(config.issuer);
...
async function getGroups(accessToken: string): Promise<SsoGroup[]> {
	const response = await fetch(`${MICROSOFT_GRAPH_BASE}/me/memberOf?$select=id,displayName`, {
		headers: { Authorization: `Bearer ${accessToken}` },
	});
	...
	const data = await response.json();
	return (data.value || [])
		.filter((member: { '@odata.type'?: string }) => member['@odata.type'] === '#microsoft.graph.group')
...
async function validateConfig(
	...
	try {
		...
		if (!discoveryResponse.ok) {
			return { valid: false, error: `Invalid Issuer URL (HTTP ${discoveryResponse.status})` };
		}
...
function mapToRole(rules: RoleMappingRule[], defaultRole: PlatformRole, userInfo: SsoUserInfo): PlatformRole {
