...
describe('approvals gate — the autonomy-tier lattice', () => {
  it('a3 refuses a write outright, names the tier, and records nothing', async () => {
    const t = convexTest(schema, modules);
    // Even an explicit org auto_approve rule cannot loosen the declared tier.
    await t.run(async (ctx) => {
      await ctx.db.insert('configCache', {
        organizationId: ORG_A,
        domain: 'governance',
    ...
    // No card, no record: nobody can grant an operation the tier forbids.
    expect(await approvalsFor(t, 'run_a3:file')).toHaveLength(0);
  });

  it('a3 leaves reads alone', async () => {
    const t = convexTest(schema, modules);
    const decision = await evaluate(t, {
      organizationId: ORG_A,
      source: 'automation',
      resourceKey: 'run_a3:read',
      action: 'search',
      effect: 'read',
      autonomyTier: 'a3',
    });
    expect(decision).toEqual({ decision: 'allow' });
    expect(await approvalsFor(t, 'run_a3:read')).toHaveLength(0);
  });

  it('a2 tightens an org auto_approve rule on an outbound write back to asking', async () => {
    const t = convexTest(schema, modules);
    await t.run(async (ctx) => {
      await ctx.db.insert('configCache', {
        organizationId: ORG_A,
    ...
    // Without the tier the rule lets it straight through…
    expect(
      await evaluate(t, {
        organizationId: ORG_A,
        source: 'automation',
        resourceKey: 'run_notier:file',
      }),
    ).toEqual({ decision: 'allow' });
    // …with a2 the same write waits on a human.
    const gated = await evaluate(t, {
      organizationId: ORG_A,
      source: 'automation',
      resourceKey: 'run_a2:file',
      autonomyTier: 'a2',
    });
    expect(gated.decision).toBe('needs-approval');
    expect(await approvalsFor(t, 'run_a2:file')).toHaveLength(1);
  });

  it("a2 leaves a platform-internal write auto — the agent's working surface", async () => {
    const t = convexTest(schema, modules);
    const decision = await evaluate(t, {
      organizationId: ORG_A,
      source: 'automation',
      resourceKey: 'run_a2:mark_started',
      connector: 'task',
      action: 'update_status',
      platformInternal: true,
      autonomyTier: 'a2',
    ...
    expect(decision).toEqual({ decision: 'allow' });
    expect(await approvalsFor(t, 'run_a2:mark_started')).toHaveLength(0);
  });

  it('a2 never loosens: an org rule requiring approval on an internal write still asks', async () => {
    const t = convexTest(schema, modules);
    await t.run(async (ctx) => {
      await ctx.db.insert('configCache', {
        organizationId: ORG_A,
    ...
    const decision = await evaluate(t, {
      organizationId: ORG_A,
      source: 'automation',
      resourceKey: 'run_a2:tightened_internal',
      connector: 'task',
      action: 'update_status',
      platformInternal: true,
      autonomyTier: 'a2',
  ...
  it('a1 behaves exactly like an unset tier', async () => {
    const t = convexTest(schema, modules);
    // Outbound write: both ask.
    const unset = await evaluate(t, {
      organizationId: ORG_A,
      source: 'connector',
      resourceKey: 'op_unset_base',
    });
    const a1 = await evaluate(t, {
      organizationId: ORG_A,
      source: 'connector',
      resourceKey: 'op_a1_base',
      autonomyTier: 'a1',
    });
    expect(a1.decision).toBe(unset.decision);
    // With an auto_approve rule: both run straight through.
    await t.run(async (ctx) => {
      await ctx.db.insert('configCache', {
        organizationId: ORG_A,
        domain: 'governance',
