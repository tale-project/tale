...
/**
 * Read-only snapshot of every retention-related env var the resolver
 * currently considers, plus its observed `process.env` value (or null
 * if unset). Backs the admin "Environment" page modeled after
 * Backstage's plugin-config-schema.
 *
 * Each entry includes the binding source — `'metadata'` when the env
 * name is derived from the JSON file's `_metadata.envPrefix` block,
 * `'none'` when the category has no `_metadata.envPrefix` (no env
 * binding at all). Operators can see at a glance whether their
 * customizations are wired.
 *
 * Auth: any org member (bounds are operator-set, not org-secret).
 */
export const getRetentionEnvSnapshotAction = action({
  args: { organizationId: v.string() },
  returns: v.array(
    v.object({
      category: v.string(),
      kind: v.union(v.literal('min'), v.literal('max'), v.literal('default')),
      envName: v.string(),
      source: v.union(v.literal('metadata'), v.literal('none')),
      currentValue: v.union(v.null(), v.string()),
      applied: v.boolean(),
    }),
  ),
  handler: async (ctx, args) => {
    ...
    // process.env is available in V8 actions (Convex syncs env via
    // `convex env set` from the platform's docker-entrypoint.sh).
    // Defensive try/catch on each read because Convex enforces a 40-char
    // limit on env-var names and an oversize name would throw.
    const readEnvSafe = (name: string): string | null => {
      try {
    ...
    for (const b of bounds) {
      for (const kind of ['min', 'max', 'default'] as const) {
        const binding =
          kind === 'min' ? b.minEnv : kind === 'max' ? b.maxEnv : b.defaultEnv;
        out.push({
          category: b.category,
          kind,
          envName: binding.envName,
          source: binding.source,
          currentValue: binding.envName ? readEnvSafe(binding.envName) : null,
