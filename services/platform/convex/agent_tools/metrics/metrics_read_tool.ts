 ...
 *
 * Read-only task metrics so analyst agents can ground their work
 * in real numbers — how much the team produced, how agents are performing, and
 * what's queued or stuck — and act on it (reassign or decompose).
 * Reads the shared daily aggregate under hard scan caps
...
const metricsReadArgs = z.discriminatedUnion('operation', [
  z.object({
    operation: z.literal('daily_summary'),
    windowHours: z
      .number()
      .optional()
      .describe(
...
export const metricsReadTool: ToolDefinition = {
  ...
  availability: 'any',
  tool: createTool({
    description: `Read task metrics for the organization to analyze performance and decide what to do next.
...
Use this to spot bottlenecks (rising pendingReviews/queuedRuns/openBreakers), track throughput and spend, and justify reassigning or decomposing work. For per-entity counts (customers, products, vendors) use the matching *_read 'count'/'list' operation.`,
    ...
    execute: async (ctx: ToolCtx, args) => {
      ...
      // Single rolled-up "is the org busy?" figure: the volume-bearing
      ...
      return { operation: 'daily_summary', ...summary, activity };
    },
  }),
} as const;
