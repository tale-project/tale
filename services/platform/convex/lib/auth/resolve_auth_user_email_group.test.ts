...
describe('migration.up batch wrapper', () => {
  beforeEach(() => {
    vi.resetModules();
  });
  ...
  it('runs the v0.2.91 component migration batch over the memory store', async () => {
    ...
    seedUser(
      store,
      signupUser({ _id: 'canonical', email: 'merge-me@example.com' }),
    );
    seedUser(
      store,
      signupUser({
    ...
    expect(listUsers(store)[0]?.email).toBe('merge-me@example.com');
    expect(listUsers(store)[0]?.emailVerified).toBe(true);
  });
});
