...
import { listWebsitesPaginated } from './list_websites_paginated';
...
describe('listWebsitesPaginated', () => {
  ...
  it('returns pagination result', async () => {
    const docs = [
      { _id: 'w_1', domain: 'example.com', status: 'active' },
      { _id: 'w_2', domain: 'test.com', status: 'error' },
    ];
    const { ctx, paginateResult } = createMockQueryBuilder(docs);

    const result = await listWebsitesPaginated(ctx as unknown as QueryCtx, {
      paginationOpts: DEFAULT_PAGINATION_OPTS,
      organizationId: 'org_1',
    });
