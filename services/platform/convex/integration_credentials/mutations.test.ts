/**
 * Transactional invariants of the integration-credential CRUD, driven
...
describe('setDefaultCredential (public)', () => {
  it('swaps the default for an org admin', async () => {
    ...
    await t
      ...
      .mutation(api.integration_credentials.mutations.setDefaultCredential, {
      ...
      await catchCode(() =>
        t
          ...
          .mutation(
            ...
            { organizationId: orgId, credentialId: bId },
          ),
      ),
    ...
    expect(
      await catchCode(() =>
        t
          ...
          .mutation(
            api.integration_credentials.mutations.setDefaultCredential,
            { organizationId: orgId, credentialId: foreignId },
          ),
      ),
    ).toBe('CREDENTIAL_NOT_FOUND');
    ...
    expect(
      await catchCode(() =>
        t
          ...
          .mutation(
            api.integration_credentials.mutations.setDefaultCredential,
            { organizationId: orgId, credentialId: id },
          ),
      ),
...
describe('deleteCredential (public)', () => {
  it('deleting the default promotes the OLDEST remaining active row', async () => {
    ...
    await t
      .withIdentity({ subject: 'user_admin' })
      .mutation(api.integration_credentials.mutations.deleteCredential, {
        organizationId: orgId,
        credentialId: aId,
      });
