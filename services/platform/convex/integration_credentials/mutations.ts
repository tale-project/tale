/**
 * Transactional core of the integration-credential CRUD.
 *
...
async function clearOtherDefaults(
  ...
  organizationId: string,
  connectorSlug: string,
  keepId: Id<'integrationCredentials'> | null,
): Promise<void> {
...
export const insertCredentialInternal = internalMutation({
  ...
  returns: v.id('integrationCredentials'),
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    return await ctx.db.insert('integrationCredentials', {
...
export const patchCredentialInternal = internalMutation({
  args: {
    ...
    status: v.optional(integrationCredentialStatusValidator),
