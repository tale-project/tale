...
/**
 * Credential resolution — the ONE internal seam that turns an integration
 * credential row into usable secret material, for the workflow engine's
 * integration nodes and the chat tool surface. `'use node'` by necessity
...
export interface ResolveIntegrationCredentialArgs {
...
export interface ResolvedIntegrationCredential {
  readonly credentialId: Id<'integrationCredentials'>;
  readonly connectorSlug: string;
  readonly authMethod: IntegrationAuthMethodName;
...
async function loadRow(
  ctx: ActionCtx,
  args: ResolveIntegrationCredentialArgs,
  ...
  if (args.credentialRef !== undefined) {
    throw credentialError(
      ...
      `No credential "${args.credentialRef}" is configured for "${args.connectorSlug}" — check the name, or add it in Settings → Integrations.`,
    );
  }
  throw credentialError(
    'CREDENTIAL_NONE_CONFIGURED',
    `No default credential is configured for "${args.connectorSlug}" — add one in Settings → Integrations, or name a credential explicitly.`,
  );
}
...
function assertUsable(row: CredentialRow): void {
  if (row.status === 'disabled') {
    throw credentialError(
      'CREDENTIAL_DISABLED',
      `Credential "${row.name}" is disabled — enable it in Settings → Integrations, or name another credential.`,
    );
  }
  if (row.status === 'needs-reauth') {
    ...
    throw credentialError(
      ...
      `Credential "${row.name}" lost its authorization${detail} — reconnect "${row.connectorSlug}" in Settings → Integrations to restore access.`,
    );
  }
}
...
function openEnvelope(row: CredentialRow): IntegrationSecretPayload {
  ...
  try {
    ...
    if (err instanceof KeyRotatedError) {
      throw credentialError(
        ...
        `Credential "${row.name}" was encrypted under a previous ENCRYPTION_SECRET_HEX and cannot be decrypted — re-enter it in Settings → Integrations.`,
      );
    }
    throw err;
  }
  try {
    ...
    if (err instanceof SecretPayloadError || err instanceof SyntaxError) {
      throw credentialError(
        ...
        `Credential "${row.name}" does not carry a usable ${row.authMethod} payload — re-enter it in Settings → Integrations.`,
      );
    }
    throw err;
  }
...
export async function resolveIntegrationCredential(
  ...
  const connector = findIntegrationConnector(row.connectorSlug);
