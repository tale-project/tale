 ...
 * `/api/connectors/hostcall` route, which verified the one-run capability
...
import { createLiveHost } from '../../lib/connectors/live-host';
...
export const performConnectorHostCall = internalAction({
  args: {
  ...
  handler: async (ctx, args): Promise<HostcallOutcome> => {
    const connector = findConnector(args.connectorSlug);
    if (!connector) {
      return {
        error: {
    ...
    if (!isHostcallMethod(args.method)) {
      return {
        error: {
          code: 'BAD_METHOD',
          message: `"${args.method}" is not an HTTP verb the connector host offers`,
        },
      };
    }
