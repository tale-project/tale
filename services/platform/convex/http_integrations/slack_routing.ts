...
export const resolveTeamRoute = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    if (rows.length > 1) {
      console.error(
        `[integrations:slack] refusing an ambiguous workspace route: team ${args.teamId} maps to ${rows.length} organizations`,
      );
      return null;
    }
    return {
...
export const claimTeamRoute = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (foreign.length > 0) {
      console.warn(
        `[integrations:slack] refusing to re-point workspace ${args.teamId}: already connected to another organization`,
      );
...
export const deliverInboundEvent = internalAction({
  ...
  handler: async (_ctx, args) => {
    console.info('[integrations:slack] inbound event accepted', {
      organizationId: args.organizationId,
      teamId: args.teamId,
