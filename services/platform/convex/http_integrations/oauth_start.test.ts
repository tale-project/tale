...
function startRequest(params: Record<string, string>): Request {
  const url = new URL(`${SITE_URL}/api/integrations/oauth2/start`);
...
describe('oauth2StartHandler', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('INTEGRATION_OAUTH_GMAIL_CLIENT_ID', 'gmail-client-id');
    vi.stubEnv('INTEGRATION_OAUTH_GMAIL_CLIENT_SECRET', 'gmail-client-secret');
    vi.stubEnv('INTEGRATION_OAUTH_OUTLOOK_CLIENT_ID', 'outlook-client-id');
    vi.stubEnv(
      'INTEGRATION_OAUTH_OUTLOOK_CLIENT_SECRET',
  ...
  it('ignores a spoofed Host header when building the redirect URI', async () => {
    const { ctx, pending } = harness('admin');
    const url = new URL(
      'https://attacker.example/api/integrations/oauth2/start',
    );
  ...
  it('refuses to start when the deployment has no OAuth app for the connector', async () => {
    vi.stubEnv('INTEGRATION_OAUTH_SLACK_CLIENT_ID', '');
    vi.stubEnv('INTEGRATION_OAUTH_SLACK_CLIENT_SECRET', '');
    const { ctx, pending } = harness('admin');
