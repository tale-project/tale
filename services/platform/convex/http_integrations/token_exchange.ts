...
export async function exchangeAuthorizationCode(
  ...
  try {
    ...
    console.error('[integrations:oauth2] connector token URL is not a URL');
    return { ok: false, reason: 'vendor_unreachable' };
  }
  if (parsedUrl.protocol !== 'https:') {
    console.error(
      `[integrations:oauth2] refusing a non-https token endpoint (${parsedUrl.protocol})`,
    );
    return { ok: false, reason: 'vendor_unreachable' };
  }
  ...
  try {
    ...
    console.error(
      '[integrations:oauth2] token endpoint unreachable:',
  ...
  try {
    ...
    console.error(
      `[integrations:oauth2] token endpoint returned a non-JSON body (status ${response.status})`,
    );
    return {
      ok: false,
