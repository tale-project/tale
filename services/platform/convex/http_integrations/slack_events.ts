...
async function throttleAndRefuse(
  ...
  try {
    ...
    console.error(
      '[integrations:slack] rate-limit check failed; refusing the request anyway',
...
export async function slackEventsHandler(
  ...
  if (!signingSecret) {
    ...
    console.error(
      '[integrations:slack] INTEGRATION_SLACK_SIGNING_SECRET is unset — refusing inbound events',
    );
    return textResponse('Slack integration is not configured', 503);
  }
  ...
  if (!verification.ok) {
    console.warn(
      `[integrations:slack] rejected a delivery: signature ${verification.reason}`,
    );
    return throttleAndRefuse(ctx, req);
  }
  ...
  try {
    ...
    console.warn('[integrations:slack] discarding a signed non-JSON delivery');
  ...
  if (!teamId) {
    ...
    // exists to avoid.
    console.warn('[integrations:slack] discarding a delivery with no team_id');
  ...
  if (!route) {
    console.warn(
      `[integrations:slack] no organization is connected to workspace ${teamId}`,
    );
    return textResponse('Unknown workspace', 404);
  }
