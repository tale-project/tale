/**
 * The OAuth2 authorization-code flow for integration connectors: `start` sends
...
function errorPage(
  ...
  return renderIntegrationErrorPage(
    kind,
    organizationId ? resolveIntegrationSettingsUrl(organizationId) : null,
  );
}

/**
 * GET /api/integrations/oauth2/start?connector=<slug>&organizationId=<id>
 *
...
export async function oauth2StartHandler(
  ...
  if (!redirectUri) {
    console.error(
      '[integrations:oauth2] SITE_URL is unset — refusing to derive an OAuth redirect URI from the request',
    );
  ...
  if (!user) {
    return plainText('Sign in to connect an integration.', 401);
  }
  ...
  // — connecting an integration IS one, just spelled as a consent flow.
  ...
  if (!app) {
    ...
    console.error(
      `[integrations:oauth2] no OAuth app configured for "${connectorSlug}": set ${prefix}CLIENT_ID and ${prefix}CLIENT_SECRET`,
    );
  ...
  try {
    ...
    console.error(
      `[integrations:oauth2] connector "${connectorSlug}" has an unusable authorize URL:`,
...
export async function oauth2CallbackHandler(
  ...
  if (!pending.ok) {
    console.warn(
      `[integrations:oauth2] refused a callback with a ${pending.reason} state`,
    );
    return errorPage('invalid_state');
  }
  ...
  if (vendorError || !code) {
    console.info(
      `[integrations:oauth2] "${connectorSlug}" consent did not complete for organization ${organizationId}`,
    );
  ...
  if (!exchange.ok) {
    console.warn(
      `[integrations:oauth2] "${connectorSlug}" token exchange failed for organization ${organizationId}: ${exchange.reason}${
  ...
  try {
    ...
    console.error(
      `[integrations:oauth2] storing the "${connectorSlug}" credential for organization ${organizationId} failed via ${STORE_OAUTH2_CREDENTIAL_PATH} (${
  ...
  const settingsUrl = resolveIntegrationSettingsUrl(organizationId);
  if (!settingsUrl) {
