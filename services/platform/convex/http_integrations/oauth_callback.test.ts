...
const EXPECTED_REDIRECT_URI = `${SITE_URL}/api/integrations/oauth2/callback`;
const SETTINGS_PATH = '/dashboard/org_cb_test/settings/integrations';
const ORG = 'org_cb_test';
const OTHER_ORG = 'org_cb_other';
const USER = 'user_cb_test';
const CALLBACK_PATH = '/api/integrations/oauth2/callback';
...
const insertFixtureCredentialRef = makeFunctionReference<
  ...
  string
>('integration_credentials/actions:insertFixtureCredential');

/**
 * Stand-in for the credentials domain. Records the call and writes a row into
 * the REAL `integrationCredentials` table, so "exactly one credential, scoped
...
const insertFixtureCredential = internalMutation({
  ...
  handler: async (ctx, args) => {
    const now = Date.now();
    const id = await ctx.db.insert('integrationCredentials', {
...
modules['integration_credentials/actions.ts'] = () =>
...
async function credentialsOf(
  ...
  const rows = await t.run(async (ctx) =>
    ctx.db.query('integrationCredentials').collect(),
  );
...
describe('OAuth2 callback', { timeout: 60_000 }, () => {
  beforeEach(() => {
    ...
    vi.stubEnv('INTEGRATION_OAUTH_SLACK_CLIENT_ID', 'slack-client-id');
    vi.stubEnv('INTEGRATION_OAUTH_SLACK_CLIENT_SECRET', 'slack-client-secret');
  });
  ...
  it('claims the Slack workspace route for that organization', async () => {
    ...
    const credential = await t.run(async (ctx) =>
      ctx.db.query('integrationCredentials').first(),
    );
  ...
  it('rejects an expired state and burns it', async () => {
    ...
    expect(
      await t.run(async (ctx) =>
        ctx.db.query('integrationOauthStates').collect(),
      ),
    ).toHaveLength(0);
  });
