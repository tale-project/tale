...
export async function generateAgentResponse(
  ...
  if (streamId) {
    await ctx.runMutation(internal.streaming.mutations.startStream, { streamId });
  }
  ...
  try {
    ...
    if (streamId && trimmedResponseText) {
      await ctx.runMutation(internal.streaming.mutations.appendToStream, {
        streamId,
        text: trimmedResponseText,
      });
      await ctx.runMutation(internal.streaming.mutations.completeStream, { streamId });
    }
    ...
    if (streamId) {
      try {
        await ctx.runMutation(internal.streaming.mutations.errorStream, { streamId });
      } catch (streamError) {
