...
const { resolveIntegrationCredential } =
  await import('../integration_credentials/resolve_credential');
const resolveMock = vi.mocked(resolveIntegrationCredential);

const { runIntegrationAction } = await import('./execute_action');
...
const handler = (runIntegrationAction as unknown as ActionShape).handler;
...
describe('runIntegrationAction', () => {
  ...
  it('writes an audit row for every invocation', async () => {
    ...
    expect(ctx.runMutation).toHaveBeenCalledWith('createAuditLog', {
      ...
      actorId: 'user_1',
      actorType: 'user',
      action: 'integration.tavily.search',
  ...
  it('runs live through the credential seam and the mediated host', async () => {
    resolveMock.mockResolvedValue({
      credentialId: 'cred_1' as Id<'integrationCredentials'>,
