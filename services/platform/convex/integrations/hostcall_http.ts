/**
 * `POST /api/integrations/hostcall` — the platform end of a live connector
...
export const integrationsHostcallHandler = httpAction(async (ctx, request) => {
  ...
  const result: unknown = await ctx.runAction(
    internal.integrations.hostcall_action.performIntegrationHostCall,
    {
