...
/**
 * The platform's door to the integration dispatcher.
 *
 * Everything that decides an integration call lives in `lib/integrations/`
 ...
 */
function assembleIntegrationHost(ctx: ActionCtx): void {
...
/**
 * Who a file written by an integration belongs to. The write is the
 ...
 */
const INTEGRATION_ACTOR = 'system';

/** The live tree; `.trash` is not an integration surface. */
  ...
  return {
    ...
    async write({ organizationId, segments, bytes, contentType }) {
      ...
      try {
        ...
        await ctx
          ...
          .catch((cause: unknown) =>
            console.warn(
              '[integrations] webdav.write: reclaiming the unused blob failed',
              cause,
            ),
          );
    ...
    async remove({ organizationId, segments }): Promise<boolean> {
      ...
      await ctx
        ...
        .catch((cause: unknown) =>
          console.warn(
            '[integrations] webdav.delete: lock cleanup failed',
            cause,
          ),
        );
      return true;
...
function credentialResolver(ctx: ActionCtx): CredentialResolver {
  return {
    resolve: (organizationId, connectorSlug, credentialRef) =>
      resolveIntegrationCredential(ctx, {
        organizationId,
        connectorSlug,
 ...
 * audit chain under the `integration` category — mandatory for the `system`
 * caller, whose whole justification for skipping approvals is that it leaves a
 * trail naming the reason.
 */
function auditSink(ctx: ActionCtx): IntegrationAuditSink {
  return {
    record: async (entry) => {
      await ctx.runMutation(
        internal.audit_logs.internal_mutations.createAuditLog,
        {
          ...
          action: `integration.${entry.nodeType}`,
 ...
 * side effect.
 */
export const runIntegrationAction = internalAction({
  args: {
