/**
 * List products using Convex native .paginate() for use with usePaginatedQuery.
 *
...
const FILTER_INDEXES: FilterIndex[] = [
  ...
  { field: 'category', index: 'by_organizationId_and_category' },
];
...
function buildBaseQuery(
  ...
  if (primary) {
    const tableQuery = ctx.db.query('products');
    const indexFn = (q: {
      eq: (
        field: string,
...
export async function listProductsPaginated(
  ctx: QueryCtx,
  args: ListProductsPaginatedArgs,
