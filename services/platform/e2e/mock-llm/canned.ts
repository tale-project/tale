/**
 * Deterministic content streamed by the mock LLM server, shared between
 * `server.ts` (producer) and the chat specs (assertions) so the two can never
 * drift.
 *
 * The default reply is `CANNED_REPLY`. The remaining constants drive the
 * keyword-gated "scenario" branches in `server.ts`: a user message containing
 * one of `MOCK_TRIGGERS` makes the mock emit a richer stream (reasoning,
 * structured next-steps, or a tool call) instead of the plain canned reply.
 * Messages with no trigger keyword always get `CANNED_REPLY` verbatim, so the
 * default-path specs (chat / chat-threads / chat-advanced / …) are unaffected.
 */
