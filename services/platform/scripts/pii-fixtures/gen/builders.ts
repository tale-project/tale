...
import { createScrubber, type Scrubber } from '../../../lib/pii';
import { loadLocale } from '../../../lib/pii/locales';
import type { Rng } from './rng';
import { FIXTURE_SEEDS } from './seeds';

/**
 * Locale-keyed fixture seeds. Previously inlined as
 * `LocaleConfig.fixtureSeeds`, now in a dedicated `./seeds/<code>.ts`
 * tree so the runtime locale registry stays lean (these are only used
 * by the fixture generator and aren't shipped to production).
 */
function seedsFor(code: string) {
  return FIXTURE_SEEDS[code];
}
import type {
  AddressEntry,
  CityEntry,
...
function buildAddressPositives(seed: LocaleSeed, count: number): FixtureCase[] {
  ...
  // seeds file ships `wrappers`, English-fallback otherwise. The
  // native wrappers carry their own `{}` placeholder; we replace it with
  // the synthesized address string.
  const seedWrappers = seedsFor(seed.locale)?.wrappers;
  ...
  // 0) Hand-curated locale-native addresses (seeds `addresses`) —
  //    the highest quality positives for non-Latin locales where
  //    OSM-derived synthesis would produce gibberish.
  const seedAddresses = seedsFor(seed.locale)?.addresses;
  if (seedAddresses) {
...
function buildPhonePositives(seed: LocaleSeed, count: number): FixtureCase[] {
  ...
  const wrappers = seedsFor(seed.locale)?.wrappers ?? [
...
function buildNationalIdPositives(seed: LocaleSeed): FixtureCase[] {
  ...
  const wrappers = seedsFor(seed.locale)?.wrappers ?? ['On file: {}'];
...
function buildProseNegatives(seed: LocaleSeed, count: number): FixtureCase[] {
  const out: FixtureCase[] = [];
  // Pool order: dataset prose first (Tatoeba-derived) then the locale's
  // hand-curated seed prose. Both are filtered for PII-free-ness by the
  // refresh / synthesis layer, so we trust them and don't re-run the
  // scrubber here.
  const seedProse = seedsFor(seed.locale)?.prose ?? [];
