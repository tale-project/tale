...
The stack boots with `TALE_CONFIG_DIR` pointed at [`fixtures/config`](fixtures/config) and every third-party API redirected to the [`lib/mocks`](../../lib/mocks/README.md) gateway (port 4141, booted by `playwright.config.ts`): a canned chat SSE override plus Prism-mocked AI endpoints (embeddings / image / transcription / TTS) and the connector APIs (`TALE_MOCK_CONNECTORS_BASE`). No live LLM, no API keys, no cost. **Under the AI-backend rewrite the org-seeding half of this is dormant** — the fixture agent/provider/prompt/workflow no longer reach a new org (see the note above), so the gateway currently serves only what the specs reach without an org credential.
