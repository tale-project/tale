...
Bring the stack up and sign in per [SETUP.md](SETUP.md). Have a small PDF/DOCX/TXT
to hand for the upload + preview tests. Create throwaway records and delete them
after.
...
| B3  | Unsupported upload | F1 upload a non-document type (e.g. `.exe`) or a file over the size cap | Destructive toast **Unsupported file type** (`documents.upload.unsupportedFileType`) or **File too large** (`documents.upload.fileTooLarge`); file not staged.   |
