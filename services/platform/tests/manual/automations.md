...
| F5  | Create menu              | **Create automation** (`automations.list.createButton`)                                                                                                                                                                   | A dropdown with three lanes: **From a goal** (`automations.createMenu.fromGoal`), **Blank (trigger + agent)** (`automations.createMenu.blank`), and **Upload package** (`automations.upload.trigger`); absent entirely without the developer capability                                                                                                                                                                                                                              |
...
| A3  | List and menus       | List rows are links with visible focus rings and accessible names; the **Create automation** menu and every panel action (Deploy, Save trigger, Save projects) are keyboard operable, with disabled reasons exposed, not silent      |
