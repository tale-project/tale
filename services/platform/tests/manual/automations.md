...
> org list with its create menu (author from a goal / upload a pack), the
...
| --- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| F1  | List renders             | `/dashboard/{org}/automations`                                                                                                                                                                                   | Heading **Automations** (`automations.title`) + description (`automations.list.description`); with the developer capability a **New automation** button (`automations.builder.new`); rows sorted by name, each a link card showing display name, the raw slug beneath it, and a version-count badge (`automations.list.versionCount`). With zero automations: EmptyState (`automations.list.empty.title` + `automations.list.empty.description`) |
...
| F5  | Create menu              | **New automation** (`automations.builder.new`)                                                                                                                                                                   | A dropdown with exactly two lanes: **From a goal** (`automations.createMenu.fromGoal`) and **Upload package** (`automations.upload.trigger`); absent entirely without the developer capability                                                                                                                                                                                                                                                   |
...
| F8  | Upload — yml lane        | Create menu → **Upload package** → dialog (`automations.upload.title`) → pick the Prerequisites `workflow.yml` in **Package files** (`automations.upload.filesLabel`) → **Upload** (`automations.upload.submit`) | Toast `automations.upload.uploaded` names the pack and version ("deploy it when ready"); after reload the list shows `qa/manual-probe` as **Not deployed** with 1 version; an optional manifest (automation.yml) may ride along                                                                                                                                                                                                                  |
...
| F12 | Detail workbench renders | `/dashboard/{org}/automations/{slug}` for a seeded pack                                                                                                                                                          | Header: display name + description, on-screen version badge (`automations.versions.versionLabel`), **Live** badge when deployed, **Test run**, **Run live**; body: canvas + node inspector two-column grid, then **Trigger** (`automations.trigger.title`), **Projects** (`automations.bindings.title`), **Versions** (`automations.versions.title`) and **Runs** (`automations.runs.title`) panels                                              |
...
| F14 | Node inspector           | Click a node; then click empty canvas                                                                                                                                                                            | Inspector (**Node**, `automations.editor.title`) shows the node's typed fields (e.g. **Input**, `automations.editor.fields.input`); with nothing selected it reads `automations.editor.noSelection`; read-only without the developer capability                                                                                                                                                                                                  |
...
| F16 | Version switching        | In **Versions**, click an older version's label; repeat with unsaved edits on the canvas                                                                                                                         | Clean: the canvas redraws that stored version (selected row marked current). Dirty: confirm dialog (`automations.detail.switchVersion.title`) — **Discard and switch** (`automations.detail.switchVersion.confirm`) drops the draft; Cancel keeps it                                                                                                                                                                                             |
| F17 | Deploy                   | Versions panel → **Deploy** (`automations.versions.deploy`) on a version whose tests did not fail                                                                                                                | The green **Live** badge (`automations.versions.deployed`) moves to that version; header + list row show **Live: v{n}** after reload; versions saved with passing/failing acceptance tests carry `automations.versions.testsPassed` / `automations.versions.testsFailed` badges — deploying a failed one is refused (`automations.versions.deployRefused` alert)                                                                                 |
| F18 | Test run (mock)          | On `qa/manual-probe` (undeployed is fine) → **Test run** (`automations.detail.runMock`)                                                                                                                          | A run starts on the version on screen; **Show last run** (`automations.detail.showLastRun`) toggles per-node status overlays on the canvas (`automations.runs.nodeStatus.*`), and the inspector gains an **In this run** section (`automations.editor.runTitle`) with **Resolved input** / **Output** (`automations.editor.resolvedInput` / `automations.editor.output`)                                                                         |
| F19 | Runs panel               | Workbench **Runs** panel after F18                                                                                                                                                                               | The run rows show status badge, version, and starter (`automations.runs.startedBy`); an automation that never ran reads `automations.runs.empty`; **Open the last run** (`automations.detail.openLastRun`) navigates to the run route                                                                                                                                                                                                            |
...
| ID  | Check                | Expected                                                                                                                                                                                                                          |
| --- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | Canvas semantics     | The canvas is a labelled region (**Automation canvas**, `automations.canvas.ariaLabel`); every node box is a real `<button>` that is keyboard reachable and expands/controls the inspector region — selection never needs a mouse |
