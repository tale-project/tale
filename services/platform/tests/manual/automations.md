...
> badge, not on text. The e2e suite budgets ~90 s for a real seeded run. Adding
> a step on the canvas opens the step-create dialog; verify current behaviour
> rather than assuming a stub. **Known list gap (see I-1 below):** on a freshly
...
| F4  | Add step            | Editor canvas → **Add step** (`automations.steps.toolbar.addStep`, the **+** toolbar button)                                                                                                                                                                                              | The step-create dialog opens (or the current behaviour is observed and recorded); a new step node appears on the canvas if confirmed                                                                                                                                           |
