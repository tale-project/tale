...
| F13 | Schedule trigger    | `/automations/{slug}/triggers` → **Schedules** (`automations.triggers.schedules.title`) → **Add schedule** (`automations.triggers.schedules.createButton`) → dialog **Create schedule** (`automations.triggers.schedules.form.title`) → pick preset **Every 5 minutes** (`automations.triggers.schedules.form.presets.every5Minutes`) or type a **Cron expression** (`automations.triggers.schedules.form.cronExpression`, placeholder `0 * * * *`, help text `automations.triggers.schedules.form.cronHelp`); optional **Workflow variables** JSON (`automations.triggers.schedules.form.variablesLabel`; non-JSON input → **Variables must be valid JSON object**, `automations.triggers.schedules.form.variablesInvalid`) → create | Toast **Schedule created** (`automations.triggers.schedules.toast.created`); a row shows the cron under the **Cron expression** column (`automations.triggers.schedules.columns.cronExpression`) and survives reload of the Triggers tab; toggling **Active** toasts **Schedule enabled** / **Schedule disabled** (`automations.triggers.schedules.toast.enabled` / `…toast.disabled`); delete via the **Delete schedule** dialog (`automations.triggers.schedules.deleteTitle`) removes the row. Persistence only — asserting the schedule actually *fires* requires waiting out a cron interval live; not covered here. |
| F14 | Event trigger       | Triggers → **Events** (`automations.triggers.events.title`) → **Add event trigger** (`automations.triggers.events.createButton`) → **Event type** select (`automations.triggers.events.form.eventType`, placeholder **Select an event type**, `automations.triggers.events.form.eventTypePlaceholder`) → pick **Task created** (event ids/labels come from `convex/workflows/triggers/event_types.ts` — English-only, not in en.json) → create                                                                                                                                                                                                                                                                                     | Toast **Event trigger created** (`automations.triggers.events.toast.created`); the trigger row survives reload of the Triggers tab. Then create a task in a project → a new run appears on the **Executions** sub-tab (`automations.executions.title`) — firing hermetically is unverified, check live. Adding a second trigger for the same event → error **This automation already has a trigger for that event.** (`automations.triggers.errors.duplicateSubscription`)                                                                                                                                             |
| F15 | Per-node run status | After F9: **Executions** sub-tab → open the completed run                                                                                                                                                                                                                                 | Canvas banner **Viewing run {id}** (`automations.steps.execution.viewingRun`); each node shows a status badge with accessible name **Execution status: {status}** (`automations.steps.execution.nodeBadgeLabel`; status labels e.g. **Completed**, `automations.steps.execution.status.completed`); **Hide run results** (`automations.steps.execution.clear`) removes the banner and badges, restoring the plain canvas                                                                                                                                                                                                |
| F16 | Catalog search      | `/automations/catalog` → type in **Search automations** (`automations.search.placeholder`); then a no-match query. Env-gated: the hermetic mode-A fixture ships only `workflows/test.json` and no template catalog, so the expected mode-A landing is the empty grid — the narrowing/install assertions need an environment with catalog templates                                                                                                                                                                                  | The template grid narrows to matching templates; a no-match query shows **No automations match your search.** (`automations.search.noResults`); installing a template toasts **Automation installed** (`automations.catalog.installed`)                                                                                                                                                                                                                                                                                                                                                                                |
| F17 | Webhook URL reveal  | Extends F6: after **Add webhook**, the reveal dialog opens                                                                                                                                                                                                                                | The created **Webhook URL** (`automations.triggers.webhooks.webhookUrl`) is revealed once in a dialog titled **Webhook created** (`automations.triggers.webhooks.createdTitle`) with the warning **Save this webhook URL. The token in the URL acts as the authentication credential.** (`automations.triggers.webhooks.urlWarning`); the copy control toasts **Copied** (`automations.triggers.common.copied`). Whether the Webhooks table masks the URL after the dialog closes is unverified — check live and record what is observed                                                                                |
