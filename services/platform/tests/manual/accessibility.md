...
| DataTable page | `/dashboard/{org}/agents`                  | redirects to `…/agents/all`; `<table>` (A10)       |
...
| A10 | Tables         | `/dashboard/{org}/agents` + `/documents`; query the `<table>`                                                                                          | Every `<th>` has `scope="col"` (`TableHead` defaults it — live: 5/5 agents, 8/8 documents); selected rows set `aria-selected="true"`; a `<caption>` (may be `sr-only`) is present **when the table is given one** (the `caption` prop on DataTable)                                      |
