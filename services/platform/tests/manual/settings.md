...
| F20                  | ✅ automated   | `token-sources.spec.ts`  |
...
| F20 | Token sources      | Token sources (`navigation.tokenSources`) → **New token source** (`settings.tokenSources.new`) → slug (`settings.tokenSources.slug`) + display name (`settings.tokenSources.displayName`) + endpoint (`settings.tokenSources.endpoint`) → **Save** (`settings.tokenSources.save`); then row **···** (`settings.tokenSources.actions`) → **Delete** (`settings.tokenSources.delete`) | `settings.tokenSources.saved`; row appears in the list; **Delete** removes it (`settings.tokenSources.deleted`). Bind on an agent's Environment tab (Type → `envEditor.typeTokenSource`) for BYO agents only |
| F21 | Enterprise SSO     | Enterprise SSO (`navigation.enterpriseSso`) → pick **Protocol** (`integrations.enterpriseSso.protocolLabel`) → fill OIDC issuer/client or SAML IdP fields; **Test connection** (`integrations.enterpriseSso.test`); **Save** (`integrations.enterpriseSso.save`); in **SCIM provisioning** click **Generate token** (`integrations.enterpriseSso.scim.generate`) and copy it once   | `integrations.enterpriseSso.saved`; status shows connected; SCIM token revealed once with a copy button + base URL. Admin-only (`orgSettings`)                                                               |
...
| A5  | Token-source form | Every field + Select (method/auth/selection) labelled; broker-secret field is `type=password` (auto-checked by `token-sources-manager.test.tsx`) |
