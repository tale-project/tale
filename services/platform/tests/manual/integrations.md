...
> setup guide and Slack notification config, and config-package upload. MCP
...
minimal sample package first — e.g. zip a copy of
`builtin-configs/integrations/tavily` (`config.json` + `connector.js` + icon);
whether a builtin package re-uploads cleanly under its own slug is unverified —
check live.
...
| F9  | IMAP/SMTP mailbox panel   | `?tab=all&slug=imap_smtp` opens the **IMAP / SMTP Mailbox** panel → credential fields **Username** / **Password** (start-cased secret bindings; password masked), the **SMTP (sending)** section (`settings.integrations.manageDialog.smtpSection`) with **SMTP username** / **SMTP password** (`settings.integrations.manageDialog.smtpUsername` / `…manageDialog.smtpPassword`) + hint (`…manageDialog.smtpHint`), the **Configuration** section (`settings.integrations.manageDialog.configuration`) with the connector's connection fields (`imapHost`/`imapPort`/`imapSecure`/`smtpHost`/`smtpPort`/`smtpSecure`/`fromAddress` — labels are start-cased keys, e.g. **Imap Host**; exact visible labels: check live), and the **Configuration guide** section (`settings.integrations.manageDialog.setupGuide`) | Render is mode A; a **successful** connect is **mode B only** (real mailbox credentials — Connect runs a _real_ IMAP+SMTP probe via a node action, the `:4141` mock gateway cannot answer it; see Prerequisites). Mode-A boundary: fill **Username**/**Password** plus a garbage host → **Connect IMAP / SMTP Mailbox** (`settings.integrations.panel.connectName`) fails with the inline red `<output>` error region (the probe's message, or **Connection test failed**, `settings.integrations.connectionTestFailed`) — **not** a toast; after reload the integration is still absent from the **Connected** tab |
| F10 | Slack setup guide         | `?tab=all&slug=slack` (mode A, render-only; OAuth2 method selected, no OAuth2 credentials saved)                                                                                                                                                                                                                                                                                                 | **Set up your Slack app** (`settings.integrations.slackSetup.title`) renders with **App manifest** (`settings.integrations.slackSetup.manifest`), **Event Subscriptions Request URL** (`settings.integrations.slackSetup.requestUrl`, help `…slackSetup.requestUrlHelp`), **OAuth redirect URL** (`settings.integrations.slackSetup.redirectUrl`, help `…slackSetup.redirectUrlHelp`), and the **Open Slack apps** link (`settings.integrations.slackSetup.openSlackApps`)                                                                                                                                        |
