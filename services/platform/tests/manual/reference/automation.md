...
| [settings](../suites/settings.md) | `SET-F30`, `SET-F37`, `SET-B12`; governance `GOV-F13`, `GOV-B7` | 🔶 component + adapter | `app/features/settings/sandboxes/sandbox-quota-editor.test.tsx` (three limits, preservation, validation, Save/Discard, reader permissions), `sandbox-capacity.test.tsx` (measured resources, unknown state, runtime inventory, refresh), `sandboxes-settings.test.tsx` (writer-only workspace queries and metadata visibility, including cached rows), `app/lib/backend/settings-sandbox.test.ts` (org-scoped fetch and quota cache invalidation); real infrastructure readings and persistence after reload remain manual |
