...
| [settings](../suites/settings.md) | Adaptive inner Docker networks, IPv6 startup guards and conservative Kubernetes capacity | ✅ automated | `services/sandbox-runtime/daemon/src/inner-docker-network.test.ts`, `build-cache-startup.test.ts`, `env-store.test.ts` (observed/planned routes, matching daemon/NAT ranges, protected startup environment); `services/sandbox/src/session/docker-session-args.test.ts`, `egress-firewall.test.ts`, `capacity.test.ts` (IPv6 sysctls, guard fallback, unknown pods counted without hiding capacity); `tools/cli/src/lib/compose/services/compose-parity.test.ts` |
