...
| [projects](../suites/projects.md) | REST chat lifecycle: door-side model resolution (`CHAT_MODEL_UNKNOWN` / `CHAT_MODEL_AMBIGUOUS`), model facts and the default marker on `GET /models`, message `status` (a stop settles as `cancelled`) and `usage` (`costEstimateCents` included), the 202’s pre-minted `messageId` and the `queued` poll state until the worker opens the turn, streaming progress on the poll, `reasoningEffort` / `maxOutputTokens` on a send (a cap above the model’s ceiling refused), archive with `archivedAt` / rename / delete / cancel routes, blank-prompt and locale refusals | ✅ automated | `backend/rest/v1-threads.test.ts`, `backend/rest/v1-threads.contract.test.ts`, `backend/rest/v1-threads.scope.test.ts`, `backend/domains/chat/rest-turn.test.ts`, `backend/domains/chat/store.test.ts`, `lib/chat/turn.test.ts`, `lib/chat/effort.test.ts`, `scripts/openapi/spec.test.ts` |
...
| [projects](../suites/projects.md) | REST file content honours `If-None-Match` / `If-Modified-Since`: the preconditions travel to the store with the `Range`, its 304 comes back bodiless with the validators, and the lane answers `private, no-cache` | ✅ automated | `backend/domains/files/service.content.test.ts`, `backend/rest/v1-projects.test.ts` |
