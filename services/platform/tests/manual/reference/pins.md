...
| `NAV-F1` | The rail's first tile is **Home** (`navigation.home`), not Chat, and there is no Projects or Inbox tile: chats, projects with their tasks, and the inbox fold into Home, whose tile opens the last chat and lights on every Home route through `isHomePath` rather than its own href. New chat is the panel header's action (and a click on Home from inside Home). A rail click always commits the section's default entry, whatever the user did there before — `sidebar-nav.test.tsx` and `use-navigation-items.test.ts` hold those rules, `navigation.spec.ts` drives the clicks; this box judges the labels, order, the gliding pill and the active styling. |
| `NAV-F31` | The panel folds only on a chat, a task page and an open conversation, because only their headers carry the toggle that brings it back; a project page or the inbox index has none, so a stored fold is ignored there on purpose — otherwise the panel could be hidden with no way to show it. The boot script applies the same rule before paint (`home-paths.test.ts` `isPanelCollapsible`, `boot-chat-script.test.ts`). |
...
| `RESP-F2` / `TASK-F25` | The mobile More sheet (retired with the four-tab bar) was a `Sheet` opened from state — no Radix trigger — and `Sheet` never ran `useRestoreFocus`, so Escape dropped the document on `<body>`. The primitive now restores the opener (`sheet.test.tsx`). The task page's phone details sheet is opened from state the same way, so `TASK-F25` now judges the **Show details** button regaining focus. |
| `CONV-F4` / `CONV-F12` / `CONV-F16` / `CONV-F20` / `CONV-F25` | Desktop and phone keep the facets in different places on purpose: the Home panel's Inbox view stands beside every Home route (a chat, a task, a project), so its facets cannot ride the inbox route's URL — it remembers them per device instead; the phone's list is the inbox page itself and keeps `?read=` / `?assignee=` / `?channel=` / `?mailbox=`. Both run on `useInboxList`, so the facet semantics must not differ. The team and channel chips are drawn by the phone list's rows only — the panel's rows share one anatomy with chats and tasks — which is why `CONV-F12` / `CONV-F19` / `CONV-F21` judge them at 390 px. |
