...
>
> **Update — private-app upload landed**: the hub now ships an **Upload app**
> entry point (F14), so much of this surface is exercisable in mode A after
> all: zip `builtin-configs/apps/issue-desk` and upload it, and the install
> wizard, membership hub, details page, hub search, and per-project config
> (F4–F11, F14–F17) become reachable without a populated builtin catalog.
> Only cases needing real workflow runs or large collections (a populated DAG
> in F12, F18–F19) remain env-gated.
...
| F14 | Private-app upload           | (mode A) Prep: zip `builtin-configs/apps/issue-desk` as the sample bundle (project-scoped, requires the `github` integration — which connects hermetically). `/dashboard/{org}/apps` → **Upload app** (`apps.upload.uploadApp`) → dialog **Upload private app** (`apps.upload.dialogTitle`) → drop the `.zip` on **Upload app bundle** (`apps.upload.dropZoneLabel`) or click **Select folder** (`apps.upload.selectFolder`) → **Upload app** (`apps.upload.submit`). Then re-upload the same slug → **Replace existing app?** (`apps.upload.replaceTitle`) → **Replace** (`apps.upload.replaceConfirm`) | Preview shows scope badge **Project app** / **Org app** (`apps.upload.scopeProject` / `apps.upload.scopeOrg`) and **Workflows** / **Agents** / **Requires integrations** / **Bundle files** counts (`apps.upload.workflows`, `apps.upload.agents`, `apps.upload.requiresIntegrations`, `apps.upload.bundleFiles`; file count via `apps.upload.fileCount`). Submit → toast **App uploaded** (`apps.upload.uploadSuccess`) + hint (`apps.upload.installHint`); after reload the app card is on the hub. Re-upload → **App replaced** (`apps.upload.replaceSuccess`) |
| F15 | Hub search                   | (mode A, after F14) `/dashboard/{org}/apps` → type into **Search apps…** (`apps.searchPlaceholder`) a matching term, then a non-matching one                                                                                                                                                    | Matching term narrows the grid to matching cards; non-matching term → EmptyState **No apps match your search.** (`apps.searchNoResults`)                                                                          |
...
| F17 | Per-project config scoping   | (mode A — the wizard's github step hermeticity is unverified, check live) Install the project-scoped app into TWO projects (wizard, F5 path) → in each project's app page open **Configure** (`apps.config.configure`) → **Configuration** (`apps.config.title`) → set different values (e.g. repository) → **Save** (`apps.config.save`)                                                        | Toast **Configuration saved** (`apps.config.saved`); reload each project — the values are independent per project. An unconfigured project shows **Configuration needed** (`apps.config.setupTitle`)              |
...
| 2   | F1/F2   | `/dashboard/{org}/apps`, `/apps/{slug}`       | med      | No in-UI app catalog/discovery surface: the hub lists only _installed_ apps and the detail page 404s for an absent slug, so a fresh org can't find or install any app from the UI. **PARTIALLY RESOLVED:** private upload landed — a fresh org CAN add an app via **Upload app** (`apps.upload.uploadApp`, F14); discovery of builtin apps is still absent. | `apps/S1-hub.png`, `apps/S2-appdetail.png` |
