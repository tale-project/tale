...
> `export-chat-dialog.test.tsx` — so F10/F13/F15 are 🔶 component-only (vitest, not
> Playwright), **not** automated by an e2e spec. Per-spec scope: `chat-threads`
> starts/reopens/deletes a thread by **id** and lists the seeded prompt in the
> library; `chat-advanced` = edit-branch, stop, regenerate, multi-turn render;
> `chat-features` = thumbs-feedback latch + save-draft→library→delete;
> `chat-depth` = picker lists seeded agent + shows model, share
...
| F24 | Image-generation output | Mode B with an image-generation-capable model/agent: ask it to generate an image | A **Generate image** tool card (`chat.tools.generateImage`) runs; the generated image renders large in the assistant bubble with an **Edit this image** overlay (`chat.imageEdit.editThis`); clicking it flips the composer to edit mode — placeholder **Describe the edit…** (`chat.imageEdit.placeholder`), banner **Editing last image** (`chat.imageEdit.editingLatest`) |
| F25 | @-mention KB documents | Type `@` in the composer (mode A if hermetic indexing completes, else mode B — verify live) | A popover **Knowledge base** (`composer.kbMention.title`) opens; empty states read **No indexed documents yet** (`composer.kbMention.emptyNoQuery`) / **No matching indexed documents** (`composer.kbMention.empty`); selecting a document inserts a chip with **Remove reference to {title}** (`composer.kbMention.removeMention`); at the cap a notice renders (`composer.kbMention.limitReached`); the sent message references the document |
| F26 | Prompt library auto-open on empty composer | Mode A: with an EMPTY composer, click the save control; then type text and re-check | Empty: the save control is a single **Prompt library** button (`chat.promptLibrary`) that opens the library directly; with text: it becomes the **Save options** menu (`chat.savePromptMenu`) |
| F27 | Model capability info popover | Mode A: open the model picker → on a model row click **Model details** (`chat.modelSelector.viewInfo`) | A popover shows rows **Provider / Type / Context / Max output / Input cost / Output cost / Reasoning / Prompt caching / Tools / Vision** (`chat.modelSelector.info.*`); clicking the info control does **not** select the model — the picker trigger keeps its prior value. (The rendered row set depends on catalog metadata — verify live) |
| F28 | Model fallback notice | Mode B: pin a specific model, make it unavailable at the provider, then send (hard to stage — requires breaking the pinned model, e.g. revoking its provider key) | An inline warning **{from} was unavailable — switched to {to}** (`chat.modelFallbackNotice`) renders and the turn completes on the fallback model |
| F29 | Budget banner | Set a low **Budget rules** rule (see [governance.md](governance.md) F4), then chat past the threshold (mode A if mock usage lands in the ledger, else mode B — verify live) | At the threshold a warning banner (`chat.budgetWarning`) with **Dismiss** (`chat.budgetWarningDismiss`) renders; once exceeded, sending is blocked with `chat.budgetExceededDetail` / `chat.budgetExceededDefault` and the toast **Usage limit reached** (`chat.toast.budgetExceeded`) fires |
| F30 | Plan/Act toggle | Mode B + sandbox, external agent: in the composer radiogroup **Plan or act mode** (`chat.planMode.label`) pick **Plan** (`chat.planMode.plan`); send a task | The Plan turn ends in a **Proposed plan** card (`planApproval.cardTitle`); **Approve & execute** (`planApproval.approve`) flips the thread to **Act** (`chat.planMode.act`) and executes the plan |
| F31 | Voice output / TTS | Mode B with a TTS-capable provider: toggle composer **Voice mode** (`chat.voice.voiceModeLabel`, `aria-pressed`; tooltips **Read replies aloud** `chat.voice.voiceModeEnable` / **Stop reading replies aloud** `chat.voice.voiceModeDisable`; unconfigured shows tooltip `chat.voice.voiceOutputErrorConfig`); send | The reply is spoken — **Speaking** indicator (`chat.voice.voiceOutputSpeaking`) + **Stop voice output** (`chat.voice.voiceOutputStop`); the per-thread override survives a reload. (Dictation input stays V1–V2) |
| F32 | Inline to-do card | Mode B: give the agent a multi-step task it tracks with to-dos | A **To-do list** tool card (`chat.tools.updateTodos`) lists the items with live statuses. (The panel-side Research-plan pane is covered by [workspace.md](workspace.md) — no double coverage here) |
| F33 | Citations + source cards | Mode B: ask about an uploaded document (see TL2) so the answer carries a RAG citation | Inline `[n]` buttons render (**Source {number}**, `chat.citations.source`); the popover shows **Page {page}** (`chat.citations.page`), **View in document** (`chat.citations.viewDocument`), **Visit page** (`chat.citations.visitPage`); the source-card row collapses beyond 3 — **Show all {count} sources** (`chat.citations.showAllSources`) / **Hide sources** (`chat.citations.hideSources`) |
| F34 | Attachments reach sandboxed external agents | Mode B + sandbox: attach a small file, ask the external agent to summarize it | The answer uses the file's content; the workspace explorer shows the delivered file (cross-ref [workspace.md](workspace.md) F13) |
...
| AT4 | Too many files     | Attach 11 files                           | Rejected — the composer caps at 10 attachments per message; a **Too many files** toast (`chat.tooManyFiles`, description `chat.tooManyFilesDescription`) reports the rejected count |
...
| B2  | Markdown/LaTeX edge cases | Mode B (mode A's reply is fixed): ask for nested lists, a wide table, code fences, and LaTeX             | Wide tables paginate (paginated markdown table); LaTeX renders; no layout break                                                                                                                                    |
| B3  | Paste image              | Mode A: paste an image from the clipboard into the composer                                              | An image token/chip appears (**View image** prefix, `chat.viewImage`); after send, the inline image renders in the sent bubble                                                                                     |
...
| B5  | Rapid send/stop/send     | Mode A: send → click **Stop generating** (`chat.stopGenerating`) immediately → send again                | No duplicate turn is created; the composer/Send state stays consistent                                                                                                                                             |
| B6  | Empty-composer states    | Empty or whitespace-only composer (mode A)                                                               | **Send message** (`chat.send`) is disabled; Enter no-ops; the save control collapses to the direct **Prompt library** button (`chat.promptLibrary`; ties to F26)                                                                         |
