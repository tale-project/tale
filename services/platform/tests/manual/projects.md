...
| F4  | Files                | Files tab (rail label **Knowledge**, `projects.navigation.files`; page heading **Files**, `projects.files.title`) → **Add file** (`projects.files.addButton`) → attach a document; row actions **Preview file** (`projects.files.previewAction`) and **Remove from project** (`projects.files.detachAction`)                                                                                                                  | Attach shows toast "Document added to project" (`projects.files.attachSuccess`) and the file row is visible by name; the row's RAG badge shows **Queued** (`projects.files.ragStatusQueued`) then **Failed** (`projects.files.ragStatusFailed`) on the hermetic stack (same indexing seam as knowledge.md F1); **Preview file** opens the document preview; after **Remove from project** (toast "Document removed from project", `projects.files.detachSuccess`) + reload the row is gone |
| F5  | Threads / agents     | Threads tab → **New chat in this project** → send `hello`, wait for Send to re-enable; Agents tab (rail label **Agents & models**, `projects.navigation.agents`) → **Add agent** (`projects.agents.addAgent`) under **Recommended** / **Restricted** mode (`projects.agents.modeRecommended` / `projects.agents.modeRestricted`) → **Save** (`common.actions.save`)                                                           | The new thread is listed in the Threads tab after reload; toast "Saved" (`projects.agents.saveSuccess`); the added agent row persists in the Agents tab after reload                                                                                                                                                                                                                                                                                                                     |
...
| F13 | Review flow          | Precondition: a task with a pending review. There is NO human "request review" control — a review exists only while an agent-created task review is pending, i.e. after an agent run finishes on the task (whether mode A can generate one is unverified — check live; otherwise env-gated). With the **Needs review** card (`tasks.review.needsReview`) in the detail sheet: click **Approve** (`tasks.review.approve`); or **Request changes** (`tasks.review.requestChanges`) → type feedback → **Send feedback** (`tasks.review.sendFeedback`) | Approve: toast "Task approved and completed" (`tasks.review.approvedToast`); after reload the task status is **Done** (`tasks.status.done`). Request changes: **Send feedback** stays disabled until feedback is typed; on submit, toast "Changes requested — the agent is on it" (`tasks.review.changesRequestedToast`)                                                                                                                                                                    |
...
| F18 | Task comments & mentions | Mode A (an agent actually replying is env-dependent — the mock stack may not run the mentioned agent): task detail sheet → **Comments** (`tasks.detail.comments`; empty state "No comments yet." `tasks.detail.noComments`) → type text containing `@` → the mention picker **Mention a member or agent** (`tasks.mentionPicker.title`; empty state **No matches** `tasks.mentionPicker.empty`) inserts a handle; an agent handle shows the preview chip "{slug} will respond" (`tasks.mentionPreview.willRespond`) or the queued variant "{slug} will respond (queued — at its concurrency limit)" (`tasks.mentionPreview.willQueue`) → **Comment** (`tasks.actions.comment`) | The comment renders in the thread and survives reload; deleting it asks **Delete this comment?** (`tasks.comment.deleteConfirm`) and after confirming + reload the comment is gone                                                                                                                                                                                                                                                                                                        |
...
```
Area: Projects & tasks
Functional: ___/18   Boundary: ___/4   A11y: ___/4   Perf: ___/2
