...
| F6  | Create-org wizard | Fresh user → `/dashboard/create-organization`; fill **Organization name** (`settings.organization.organizationName`) → **Next** (`common.actions.next`) → **Skip** (`common.actions.skip`) → **Go to dashboard** (`onboarding.finish.goToDashboard`)                                                                          | URL becomes `/dashboard/{org}` (a 16+ char id); the new org's chat loads                                                                                                                         |
...
| B6  | Reserved org name | Create-org wizard → **Organization name** = `default` → **Next**       | Submit is **rejected server-side** with `"…default" is reserved by the platform` (`settings.organization.nameReserved`); org is NOT created. `default` is the only reserved slug |
...
```
Area: Auth & account
Functional: ___/14   Boundary: ___/6   A11y: ___/5   Perf: ___/3
