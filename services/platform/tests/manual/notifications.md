...
> sidebar's footer + its popover panel). Review decisions happen on the task
> itself (Status → **Done** approves; a comment **@-mention** sends the work
> back), not inline in the bell — the bell deep-links to the task sheet.
...
| F13     | 🔶 backend     | — (no e2e; `convex/automations/human_asks.test.ts`: fan-out scope, prefs, fold rewrite, dismissal) |
| B1–B3   | ⛔ manual-only | —                                                   |
| A1–A3   | ⛔ manual-only | —                                                   |
...
| F7  | Review deep-link   | On a `task_review_requested` or review-reminder row, click the row body | URL opens the task in its project (`?task=` search param); the task sits at **In review** and its board card wears the **Waiting on you** chip (`tasks.review.waitingOnYou`); the notification row becomes read |
| F7b | Approve in context | From F7's task sheet, set sidebar **Status** (`tasks.fields.status`) to **Done** (`tasks.status.done`) | Pending review notifications for that task clear from the bell without manual mark-read; the task status reflects the approval after reload and the status change is attributed to you in Activity |
| F7c | Send back          | From F7's task sheet, post a comment that **@-mentions** the assignee agent with concrete feedback | The comment renders in the thread, a rework run starts from it, the task leaves **In review**, and the review notifications clear from the bell |
...
| F13 | Agent asked a question | Park an automation run on `ask_human` with a task subject ([approvals.md](approvals.md) F5/F7); as any member who can **see the task's project**, open the bell | A row titled **Agent needs your answer** (`inbox.agentQuestionAsked`) with the body **"{name} paused with a question on "{title}": {question}"** (`inbox.agentQuestionAskedBody`) — written for every project-visible member (team-scoped projects: team members + org admins; the **escalation** preference group mutes it, and it emails as an actionable type). The row deep-links to the task sheet where the answer card renders. A second question from the same turn **rewrites the unread row in place** (no stacking); once anyone answers — or the ask expires/cancels — the unread rows flip to read **without** manual mark-read and a still-pending email is cancelled |
...
| B3  | Stale review deep-link | On a review row whose review was already decided elsewhere, click the row body | No crash; the task sheet opens normally and no **Waiting on** chip remains on the board card — the row simply becomes read |
