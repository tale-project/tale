...
> agents. Team binding, the visible-in-chat switch, Delegation (organigram side
> panel, F9), Webhooks (**Create webhook**, F11), and Environment
> (`envEditor.save`, F20) save via their own mutations (separate toasts), NOT
> the Save bar.
...
| F8  | Skills              | Skills tab → **Bound skills** section (`settings.agents.form.sectionSkillBindings`) → flip a per-row bind switch (aria label **"Bind {slug}"**, `settings.agents.form.skillBindingsToggleAria`) → **Save**                                                                                                                                                                                                                                                                  | Counter **"{count}/{max} bound"** (`settings.agents.form.skillBindingsCounter`, max = 10 per `MAX_SKILL_BINDINGS_PER_AGENT` in `lib/shared/schemas/agents.ts`) increments; the binding survives reload. In an org with no skills the section shows **"No skills exist in this organization yet."** (`settings.agents.form.skillBindingsNoSkillsInOrgDescription`) + CTA **"Create one in Skills settings"** (`settings.agents.form.skillBindingsNoSkillsInOrgCta`). Cap behaviour → B5 |
| F9  | Delegation          | Delegation tab — there is **no** per-agent delegate list; the tab renders the organigram canvas focused on this agent (header **"Delegation"**, `settings.agents.delegation.title`, + description, `settings.agents.delegation.description`) → select the agent's node → side panel **Reports to** (`organigram.panel.reportsTo`) / **Delegates to** (`organigram.panel.delegatesTo`) → add an edge                                                                          | Toast **"Delegation saved"** (`organigram.saved`); after reload the new edge renders on the chart — and also on `/dashboard/{org}/agents/overview`                                                                  |
...
| F11 | Webhook             | Webhooks tab → **Create webhook** (`settings.agents.webhook.createButton`) — webhook URLs are generated, never typed                                                                                                                                                                                                                                                                                                                                                        | **"Webhook created"** reveal (`settings.agents.webhook.createdTitle`) shows the **Webhook URL** (`settings.agents.webhook.webhookUrl`) + the one-time warning (`settings.agents.webhook.urlWarning`); **Copy webhook URL** (`settings.agents.webhook.copyUrl`) toasts **"URL copied"** (`settings.agents.webhook.toast.urlCopied`); after reload the webhook is a row in the table under the **URL** column (`settings.agents.webhook.columns.url`). Publish gate: see the note below this table |
...
| F17 | Agent type switch   | Throwaway agent → General tab → **Agent type** group (`settings.agents.form.agentType.sectionTitle`) with **Internal agent** / **External agent** / **Image generation** (`settings.agents.form.agentType.internalLabel` / `…externalLabel` / `…imageLabel`) → pick **External agent** → confirm dialog **"Change agent type?"** (`settings.agents.form.agentType.switchTitle`) → **Change type** (`settings.agents.form.agentType.switchConfirm`) → **Save**               | After reload **External agent** is selected; the **External runtime** select (`settings.agents.form.agentKind.label`, options **Claude Code** / **OpenCode** — `settings.agents.form.agentKind.claudeCode` / `…agentKind.openCode`) is visible; the Tools tab shows the external description (`settings.agents.form.sectionToolsExternalDescription`); Skills + Knowledge tabs are gone from the editor nav |
| F18 | Folder breadcrumb   | List → **Create agent** → **Blank** → **Name** `qa/helper` (the Name help text, `settings.agents.form.nameHelp`, documents the `/` folder convention) → **Continue** → the editor opens                                                                                                                                                                                                                                                                                     | The editor breadcrumb shows the folder segment `qa` between **Agents** (`settings.agents.title`) and the agent's display name; clicking the `qa` crumb navigates to `/agents/all?folder=qa` (the List drilled into that folder)                                                                                                                                                                              |
...
| F20 | Environment & secrets | Environment tab (**"Environment & secrets"**, `settings.agents.env.title`) → **Add variable** (`envEditor.add`) → fill **NAME** (`envEditor.keyPlaceholder`) + **value** (`envEditor.valuePlaceholder`); on further rows toggle **Secret** (`envEditor.secret`) and set **Type** (`envEditor.valueType`) to **Token source** (`envEditor.typeTokenSource`) → **Save** (`envEditor.save`)                                                                                  | Toast **"Environment saved"** (`envEditor.saved`); after reload the rows persist and the secret's value is masked. Sub-asserts: an invalid NAME (e.g. `1BAD`) shows the inline error `envEditor.badKey`; a duplicate NAME shows `envEditor.dupKey`                                                                                                                                                           |

> **F11 publish gate (verify live)**: an unpublished agent replaces the create
> flow with **"Publish this agent to enable webhook access."**
> (`settings.agents.webhook.notPublished`). Which control "publish" maps to
> needs a live check — most likely the **Visible in chat** switch on the
> General tab (`settings.agents.general.visibleInChat`). If **Create webhook**
> is missing, toggle that switch on, reload the Webhooks tab, and record which
> control actually gates it.
...
```
Area: Agents
Functional: ___/20   Boundary: ___/5   A11y: ___/4   Perf: ___/3
