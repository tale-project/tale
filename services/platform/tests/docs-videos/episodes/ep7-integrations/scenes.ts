 ...
 * the cleanup registry has no integration type, so the coordinator sweeps
...
function connectButton(rt: SceneRuntime) {
  return detailPanel(rt).getByRole('button', {
    name: rt
      .t('settings.integrations.panel.connectName')
      .replace('{name}', 'GitHub'),
  });
}
...
export async function warmup(
  ...
  const routes = [
    `/dashboard/${ctx.orgId}/settings/integrations?tab=all`,
  ...
  await page.goto(`/dashboard/${ctx.orgId}/settings/integrations?tab=all`, {
    waitUntil: 'load',
  });
...
export const SCENES: readonly SceneChoreography[] = [
  {
    ...
    run: async (rt) => {
      ...
      // of your integrations" — the surface must be VISIBLE while the
  ...
  },
  {
    ...
    run: async (rt) => {
      ...
      await cue(7.0);
      await cursor.hover(
        tavily.getByText(rt.t('settings.integrations.badge.connected')).first(),
      );
    },
  },
  {
    ...
    run: async (rt) => {
      const { cursor, cue } = rt;
      const allTab = catalogTab(rt, 'settings.integrations.tabs.all');
  ...
  },
  {
    ...
    run: async (rt) => {
      ...
      const active = panel
        .getByText(rt.t('settings.integrations.upload.active'))
        .first();
      if (await active.isVisible().catch(() => false)) {
        console.warn(
          '[ep7] GitHub is already connected — degraded fallback take. ' +
            'Sweep the connection (panel → Disconnect → Delete integration) and retake.',
        );
        await cue(6.6);
  ...
  },
  {
    ...
    run: async (rt) => {
      ...
      const badge = github
        .getByText(rt.t('settings.integrations.badge.connected'))
        .first();
  ...
  },
  {
    ...
    run: async (rt) => {
      const { page, cursor, cue } = rt;
      const connectedTab = catalogTab(
        rt,
        'settings.integrations.tabs.connected',
      );
      await cue(2.4);
  ...
  },
  {
    ...
    run: async (rt) => {
      ...
      await spaNavigate(
        page,
        `/dashboard/${ctx.orgId}/settings/integrations?tab=connected`,
      );
      const github = connectorCard(rt, /GitHub/);
