 ...
 *
 * Usage:
 *   bunx tsx tests/stress/scenarios/loop-contention.ts
 *
...
async function run() {
  ...
  const rawLoopIterations = config.inputOverrides?.loopIterations;
  const loopIterations =
    typeof rawLoopIterations === 'number' ? rawLoopIterations : 50;
      ...
      batch.map(async (id) => {
        ...
        try {
          const execution = await client.query(
            api.workflow_executions.queries.getRawExecution,
            { executionId },
          );
          if (!execution) return;
  ...
  const reportPath = `tests/stress/report-loop-${Date.now()}.json`;
