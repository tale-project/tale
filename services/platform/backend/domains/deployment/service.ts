...
/** Ask the opt-in controller sidecar to restart services (HMAC-signed). */
export async function requestRestart(args: {
  ...
  if (!url || !token) {
    return {
      ...
      ok: false,
      error:
        'The restart controller is not enabled. Restart manually: `docker compose restart` (or `tale deploy`).',
    };
  }
