 ...
 * at read time — for the origin the viewer is on — and drops the URL for a chip
 * whose bytes were never captured, so the client shows no broken download
 * affordance.
 */
...
describe('presignMessageAttachments', () => {
  ...
  it('mints a fresh download URL from a materialized storageId, for the viewer’s origin', async () => {
