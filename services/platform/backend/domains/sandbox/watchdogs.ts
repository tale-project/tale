 ...
 *    statuses flip to `expired` (freeing their slots), their gateway
 *    virtual keys are revoked, and the parked-run wake fires for their
 *    orgs. The spawner's own reaper collects the container on its TTL —
 *    the row must not wait for it.
