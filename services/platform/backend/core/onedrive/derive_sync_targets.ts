 ...
 *
 * SharePoint items are excluded: the sync engine reads `/me/drive` (personal
 * OneDrive) only, so a SharePoint selection stays a one-time import.
 */
export function deriveSyncTargets(items: ImportItem[]): SyncTarget[] {
  const targets = new Map<string, SyncTarget>();

  for (const item of items) {
    if (item.sourceType === 'sharepoint') continue;
