...
describe('deriveSyncTargets', () => {
  ...
  // The sync engine reads `/me/drive` only — a SharePoint selection must not
  // register a config it can never service.
  it('skips SharePoint items', () => {
    const targets = deriveSyncTargets([
      folderFile('a', { sourceType: 'sharepoint', siteId: 's', driveId: 'd' }),
      {
        id: 'sp-file',
        name: 'sp.pdf',
        size: 5,
