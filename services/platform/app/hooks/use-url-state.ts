...
export function useUrlState<T extends UrlStateDefinitions>(
  ...
  const state = useMemo(() => {
    ...
    for (const [key, definition] of Object.entries(definitions)) {
      ...
      const urlValue = search[urlKey] ?? null;
    ...
    return result;
  }, [search, definitions, initialValues]);

  // Build new search params, preserving unmanaged keys
  const buildSearch = useCallback(
    ...
    [search, definitions, managedKeys, preserveKeys, state],
  );
  ...
  const setState = useCallback(
    ...
    [buildSearch, location.pathname, navigate],
  );
