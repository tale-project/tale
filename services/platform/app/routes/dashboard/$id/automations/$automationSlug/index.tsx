...
import {
  resolvesToAutomation,
  resolvesToWorkflow,
  workflowSlugMatches,
} from '@/app/features/automations/utils/resolve-legacy-automation';
...
export const Route = createFileRoute(
  ...
  // (installed or catalog) today; an UNRESOLVED slug falls back to the
  // standalone workflow route ONLY when that workflow actually exists (a
  // pre-rename workflow bookmark) — a slug that is neither renders this
  // page's own not-found surface. This beforeLoad is the fast path (warm
  // cache, no paint); on a COLD load the WebSocket auth handshake hasn't
  // resolved yet, so it fails open ("assume real") and the component below
  // re-runs the decision once the queries actually settle.
  beforeLoad: async ({ context, params }) => {
    ...
    if (
      !isReal &&
      (await resolvesToWorkflow(context, params.id, params.automationSlug))
    ) {
      throw redirect({
...
function AutomationIndexRoute() {
  ...
  } = useListWorkflows(organizationId, 'all');
  ...
  if (!resolved) {
    ...
    if (!isAuthenticated || isLoading || catalogLoading || workflowsLoading) {
      return <SkeletonText lines={6} />;
    }
    // Settled and genuinely unresolved: the D3 legacy-workflow fallback the
    // fail-open `beforeLoad` skipped on this cold load — taken only when the
    // slug names an EXISTING workflow. A query ERROR (or a slug that is
    // neither) renders the page's own not-found/error surface instead —
    // never misroute on a transient failure.
    if (
      !error &&
      !catalogError &&
      !workflowsError &&
      workflowSlugMatches(workflows, automationSlug)
    ) {
      return (
        <Navigate
          to="/dashboard/$id/workflows/$workflowId"
