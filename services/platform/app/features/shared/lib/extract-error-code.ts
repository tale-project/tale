/**
 * Duck-typed extraction of a `AppError` payload's `code` field.
 *
 * Vite chunk splitting can produce multiple `AppError` class copies that
 * break `instanceof`, so we never rely on the class identity. We peek at the
 * `data.code` property the server sets via `throw new AppError({code, ...})`.
 *
...
export function extractErrorCode(err: unknown): string | null {
  ...
  // message "Rate limit exceeded for ...". Convex wraps non-AppError throws
