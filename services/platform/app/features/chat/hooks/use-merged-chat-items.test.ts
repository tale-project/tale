...
describe('useMergedChatItems — inline job cards', () => {
  function makeJob(
    id: string,
    messageId: string | undefined,
    startedAt: number,
  ) {
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- minimal job double; the splicer reads messageId + startedAt
    return {
      _id: id,
  ...
  it('splices jobs after the spawning turn, ordered by startedAt', () => {
    const { result } = renderHook(() =>
      useMergedChatItems({
        ...emptyParams,
        messages: baseMessages,
        jobs: [makeJob('job2', 'a1', 2600), makeJob('job1', 'a1', 2500)],
      }),
    );
    const keys = result.current.messages.map(
      (i) => getMessageId(i) ?? `${i.type}:${getApprovalId(i)}`,
    );
    expect(keys).toEqual(['u1', 'a1', 'job:job1', 'job:job2', 'u2']);
  });

  it('never populates activeApproval (a job needs no user action)', () => {
    const { result } = renderHook(() =>
      useMergedChatItems({
        ...emptyParams,
        messages: baseMessages,
        jobs: [makeJob('job1', 'a1', 2500)],
      }),
    );
    expect(result.current.activeApproval).toBeNull();
  });

  it('skips unlinked jobs and jobs anchored to unloaded messages', () => {
    const { result } = renderHook(() =>
      useMergedChatItems({
        ...emptyParams,
        messages: baseMessages,
        jobs: [
          makeJob('unlinked', undefined, 2500),
          makeJob('unloaded', 'not-loaded', 2500),
        ],
      }),
    );
    expect(result.current.messages.some((i) => i.type === 'job')).toBe(false);
  });
});
