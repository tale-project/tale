...
describe('useChatPendingState', () => {
  ...
  describe('effectivePending (synchronous derivation)', () => {
    ...
    it('returns true when isPending is true and no messages exist', () => {
      const { result } = renderHook(() =>
        useChatPendingState({
          isPending: true,
          setIsPending,
    ...
    it('returns true when isPending is true and uiMessages is undefined', () => {
      const { result } = renderHook(() =>
        useChatPendingState({
          isPending: true,
          setIsPending,
    ...
    it('returns true when no new assistant message has appeared (send → message gap)', () => {
      ...
      // No new assistant message yet — effectivePending stays true
      expect(result.current.effectivePending).toBe(true);
    });

    it('returns false synchronously when all assistant messages are terminal', () => {
      const userMsg = createUIMessage({
        id: 'msg-1',
        order: 0,
        role: 'user',
      ...
      // Assistant responds with a terminal message
      const completedMsg = createUIMessage({
        id: 'msg-2',
        order: 1,
        role: 'assistant',
      ...
      // effectivePending should be false immediately (same render frame)
      expect(result.current.effectivePending).toBe(false);
    });
    ...
    it('returns true when assistant message has pending status (tool call)', () => {
      const userMsg = createUIMessage({
        id: 'msg-1',
        order: 0,
        role: 'user',
    ...
    it('handles remount with ref null — uses fallback baseline', () => {
      const completedMsg = createUIMessage({
        id: 'msg-2',
        order: 1,
        role: 'assistant',
      ...
      // Remount: isPending true from context, ref starts null
      // Fallback baseline = max(0, 1-1) = 0, length (1) > 0 → checks terminal
      const { result } = renderHook(() =>
        useChatPendingState({
          isPending: true,
          setIsPending,
