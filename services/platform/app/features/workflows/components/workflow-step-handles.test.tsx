...
vi.mock('./workflow-callbacks-context', () => ({
  useWorkflowCallbacks: () => ({ onNodeClick: vi.fn() }),
}));
...
describe('workflow canvas handles are non-connectable (read-only)', () => {
  it('renders WorkflowStep handles as non-connectable', () => {
  ...
  it('renders WorkflowLoopContainer handles as non-connectable', () => {
    const { container } = renderInFlow(
      <WorkflowLoopContainer data={{ label: 'Loop', stepSlug: 'loop' }} />,
    );
