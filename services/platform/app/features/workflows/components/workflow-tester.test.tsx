...
vi.mock('@/lib/i18n/client', () => ({
  useT: () => ({
    t: (key: string, params?: Record<string, string>) =>
      ...
      params
        ? `workflows.${key} ${Object.values(params).join(' ')}`
        : `workflows.${key}`,
  }),
}));
...
vi.mock('./workflow-steps', () => ({
  WORKFLOW_PANEL_URL_DEFINITIONS: {
    panel: { default: null },
    step: { default: null },
  },
}));
...
vi.mock('./workflow-debug-controls', () => ({
  WorkflowDebugControls: (props: { currentStepName?: string }) => (
...
async function runExecute() {
  ...
  render(<WorkflowTester organizationId="org-1" workflowSlug="my-workflow" />);
  await user.click(
    screen.getByRole('button', { name: 'workflows.tester.execute' }),
  );
}

describe('WorkflowTester result feedback (#1484)', () => {
  ...
  it('shows which step failed and why after a failed run', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('workflows.tester.result.failed'),
      ).toBeInTheDocument(),
    );
    expect(
      screen.getByText('workflows.tester.result.failedAtStep Send Email'),
    ).toBeInTheDocument();
  ...
  it('shows a success state when the run completes', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('workflows.tester.result.completed'),
      ).toBeInTheDocument(),
    );
  });
  ...
  it('shows a running state with the current step', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('workflows.tester.result.running'),
      ).toBeInTheDocument(),
    );
    expect(
      screen.getByText('workflows.tester.result.runningStep Fetch Data'),
    ).toBeInTheDocument();
  });
});

describe('WorkflowTester debug mode (#1490)', () => {
  async function runDebug() {
    ...
    await user.click(
      screen.getByRole('button', { name: 'workflows.tester.debug.button' }),
    );
  }
...
describe('WorkflowTester per-step feedback (#1484)', () => {
  const failedRun = () => {
    h.executionStatus = {
      data: {
        status: 'failed',
  ...
  it('lists executed steps in start order with the failing step error inline', async () => {
    ...
    expect(
      screen.queryByText('workflows.tester.result.failedAtStep Send Email'),
  ...
  it('opens the failing step editor when its row is clicked', async () => {
    ...
    const stepButton = await screen.findByRole('button', {
      name: 'workflows.tester.result.openStep Send Email',
    });
  ...
  it('shows an attempts counter for retried/looped steps', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('workflows.steps.execution.attempts 3'),
      ).toBeInTheDocument(),
    );
  });
  ...
  it('falls back to the run-level error and reason when no step failed', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('workflows.tester.result.errorCode.start_failure'),
      ).toBeInTheDocument(),
    );
    expect(
