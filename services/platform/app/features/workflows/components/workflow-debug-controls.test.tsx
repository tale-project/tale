...
describe('WorkflowDebugControls (#1490)', () => {
  ...
  it('shows the paused step and the variables inspector', () => {
    ...
    expect(
      screen.getByText('workflows.tester.debug.pausedAt Send email'),
    ).toBeInTheDocument();
  ...
  it('sends a step event when Step is clicked', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'workflows.tester.debug.step' }),
    );
  ...
  it('sends a continue event when Continue is clicked', async () => {
    ...
    await user.click(
      screen.getByRole('button', {
        name: 'workflows.tester.debug.continue',
      }),
    );
  ...
  it('cancels the execution when Stop is clicked', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'workflows.tester.debug.stop' }),
    );
  ...
  it('shows a loading state while variables are being fetched', () => {
    ...
    expect(
      screen.getByText('workflows.tester.debug.variablesLoading'),
    ).toBeInTheDocument();
  ...
  it('shows an error state when the variables fetch fails', () => {
    ...
    expect(
      screen.getByText('workflows.tester.debug.variablesError'),
    ).toBeInTheDocument();
  });
});
