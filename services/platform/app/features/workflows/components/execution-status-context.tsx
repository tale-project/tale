...
/**
 * Shared "viewed execution" state for the workflow detail page.
 *
 ...
 * `WorkflowCallbacksProvider` for the render-loop rationale).
 *
 * Kept separate from `WORKFLOW_PANEL_URL_DEFINITIONS` so closing a side
 * panel (`clearAll` on the panel definitions) does not drop the viewed run.
 */
export const WORKFLOW_EXECUTION_URL_DEFINITIONS = {
...
export function ExecutionStatusProvider({
   ...
   *  workflow route). When omitted, falls back to the `execution` URL param
