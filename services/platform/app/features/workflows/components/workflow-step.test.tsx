...
vi.mock('./workflow-callbacks-context', () => ({
  useWorkflowCallbacks: () => ({
...
describe('WorkflowStep execution status badge (#1487)', () => {
  it('renders no badge when no execution is viewed', () => {
    render(<WorkflowStep data={baseData} />);
  ...
  it('shows a running badge and highlights the card', () => {
    ...
    expect(
      screen.getByRole('button', {
        name: 'workflows.steps.execution.nodeBadgeLabel workflows.steps.execution.status.running',
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', {
        name: 'workflows.step.openStep Fetch data',
      }),
  ...
  it('opens a popover with the output preview for a successful node', async () => {
    ...
    expect(
      screen.getByText('workflows.steps.execution.outputTruncated'),
    ).toBeInTheDocument();
  });
  ...
  describe('accessibility', () => {
    it('has no critical accessibility violations with a badge', async () => {
      ...
      const { container } = render(<WorkflowStep data={baseData} />);
