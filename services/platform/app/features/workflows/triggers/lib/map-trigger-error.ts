 ...
 * `workflows` namespace (`triggers.errors.*`). The trigger mutations raise
 ...
 *
 * `t` must be scoped to the `workflows` namespace (`useT('workflows')`).
 */
export function mapTriggerError(
  ...
  switch (convexErrorCode(err)) {
    ...
    case 'AUTOMATION_OWNED_WORKFLOW':
      return t('triggers.errors.automationOwnedWorkflow');
