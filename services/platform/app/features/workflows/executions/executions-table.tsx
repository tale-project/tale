...
export function ExecutionsTable({
  ...
  const { data: count } = useApproxExecutionCount(workflowId);
  ...
  const { data: searchResult } = useSearchExecution(
    isSearching
      ? { wfDefinitionId: workflowId, searchTerm: searchTerm, numItems: 1 }
      : undefined,
  );
  ...
  const handleStatusChange = useCallback(
    (values: string[]) => {
      void navigate({
        to: location.pathname,
        search: (prev) => ({ ...prev, status: values[0] || undefined }),
      });
    },
  ...
  const handleTriggeredByChange = useCallback(
    (values: string[]) => {
      void navigate({
        to: location.pathname,
        search: (prev) => ({ ...prev, triggeredBy: values[0] || undefined }),
      });
    },
    [navigate, location.pathname],
  );
