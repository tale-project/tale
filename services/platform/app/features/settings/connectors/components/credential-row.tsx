...
export function CredentialRow({
  ...
  const handleMakeDefault = async () => {
    try {
      ...
      console.error('connectors: set default credential failed', err);
      failToast(err);
    }
  };
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      console.error('connectors: toggle credential status failed', err);
      failToast(err);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      toast({ title: t('connectors.credential.deletedToast') });
      setDeleteOpen(false);
    } catch (err) {
      console.error('connectors: delete credential failed', err);
      failToast(err);
    }
  };
  ...
  const replaceLabel =
    credential.authMethod === 'api-key'
      ? t('connectors.replace.apiKeyTitle')
      : credential.authMethod === 'bearer'
        ? t('connectors.replace.tokenTitle')
        : t('connectors.replace.basicTitle');
  ...
  const actions: EntityRowAction[] = [
    ...
    {
      key: 'make-default',
      label: t('connectors.credential.makeDefault'),
      icon: Star,
    ...
    },
    {
      key: 'toggle-status',
      label: disabled
        ? t('connectors.credential.enable')
        : t('connectors.credential.disable'),
      icon: Power,
    ...
    {
      key: 'edit',
      label: t('connectors.credential.edit'),
      icon: Pencil,
    ...
    {
      key: 'delete',
      label: t('connectors.credential.delete'),
      icon: Trash2,
  ...
  return (
    <li className="flex items-center justify-between gap-4 px-4 py-3">
      <div className="flex min-w-0 flex-col gap-1.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {credential.isDefault && (
            <Badge variant="blue">{t('connectors.credential.default')}</Badge>
          )}
          {status !== null && (
      ...
      <EntityRowActions
        actions={actions}
        ariaLabel={t('connectors.credential.actionsLabel', {
      ...
      <DeleteDialog
        ...
        title={t('connectors.credential.deleteTitle')}
        description={t('connectors.credential.deleteBody', {
          name: credential.name,
        })}
        warning={
          credential.isDefault
            ? t('connectors.credential.deleteDefaultWarning')
            : undefined
        }
