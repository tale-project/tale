...
import type { PlatformRole, RoleMappingRule, SsoProvider } from '@/lib/shared/schemas/sso_providers';
...
export function SSOConfigDialog({ open, onOpenChange, organizationId, existingProvider }: SSOConfigDialogProps) {
	...
	const [roleMappingRules, setRoleMappingRules] = useState<RoleMappingRule[]>(DEFAULT_MAPPING_RULES);
	...
	const hasChanges = useMemo(() => {
		...
		return roleMappingRules.some((curr, i) => {
			...
			return curr.source !== origRule.source || curr.pattern !== origRule.pattern || curr.targetRole !== origRule.targetRole;
		});
	}, [
		issuer,
		clientId,
			...
			getFullConfig({})
				.then((config) => {
					if (config) {
						...
						const rules =
							config.roleMappingRules.length > 0
								? config.roleMappingRules.filter(
										(r): r is RoleMappingRule => r.source === 'jobTitle' || r.source === 'appRole',
									)
						...
						setEnableOneDriveAccess(entraFeatures?.enableOneDriveAccess ?? false);
						originalConfigRef.current = {
							...
							enableOneDriveAccess: entraFeatures?.enableOneDriveAccess ?? false,
						};
					}
					setClientSecret('');
				})
	...
	const handleSave = async () => {
		...
		try {
			...
			toast({
				title: isConnected ? t('integrations.sso.updateSuccessful') : t('integrations.sso.configureSuccessful'),
			...
			toast({
				title: t('integrations.sso.configureFailed'),
				description: error instanceof Error ? error.message : t('integrations.sso.configureError'),
				variant: 'destructive',
			});
		} finally {
	...
	const handleDisconnect = async () => {
		...
		try {
			...
			toast({
				...
				description: error instanceof Error ? error.message : t('integrations.sso.disconnectError'),
				variant: 'destructive',
			});
		} finally {
			setIsSubmitting(false);
		...
		try {
			...
			if (result.valid) {
				...
				toast({
					...
					description: result.error || t('integrations.sso.testFailedDescription'),
					variant: 'destructive',
				});
			}
		} catch (error) {
			setTestResult({
				valid: false,
				error: error instanceof Error ? error.message : t('integrations.sso.testError'),
			});
			toast({
				title: t('integrations.sso.testFailed'),
				description: error instanceof Error ? error.message : t('integrations.sso.testError'),
				variant: 'destructive',
			});
		} finally {
			setIsTesting(false);
	...
	const addMappingRule = () => {
		setRoleMappingRules([...roleMappingRules, { source: 'jobTitle', pattern: '', targetRole: 'member' }]);
	};
	...
	const updateMappingRule = (index: number, updates: Partial<RoleMappingRule>) => {
		setRoleMappingRules(roleMappingRules.map((rule, i) => (i === index ? { ...rule, ...updates } : rule)));
	};

	const footer = isConnected ? (
		<>
			<Button variant="destructive" onClick={handleDisconnect} disabled={isSubmitting} className="flex-1">
				{isSubmitting ? t('integrations.disconnecting') : t('integrations.disconnect')}
			</Button>
			<Button onClick={handleSave} disabled={isSubmitting || !issuer || !clientId || !hasChanges} className="flex-1">
		...
		<>
			...
			<Button onClick={handleSave} className="flex-1" disabled={isSubmitting || !issuer || !clientId || !clientSecret}>
				{isSubmitting ? t('integrations.sso.configuring') : t('integrations.sso.configure')}
			</Button>
		</>
	);
	...
	return (
		<FormDialog
			...
			large
		>
			{isConnected && <StatusIndicator variant="success">{t('integrations.sso.connectedToEntra')}</StatusIndicator>}
			...
			<Stack gap={4}>
				...
				<Stack gap={3}>
					...
					/>
					<Description className="text-xs">{t('integrations.sso.issuerHelp')}</Description>
				</Stack>
				...
				<Stack gap={3}>
					<Input
						...
						placeholder={isLoadingConfig ? tCommon('actions.loading') : 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx'}
						value={clientId}
						onChange={(e) => setClientId(e.target.value)}
						disabled={isSubmitting || isLoadingConfig}
					/>
					<Description className="text-xs">{t('integrations.sso.clientIdHelp')}</Description>
				</Stack>
				...
				<Stack gap={3}>
					<Input
						...
						disabled={isSubmitting || isLoadingConfig}
					/>
					<Description className="text-xs">{t('integrations.sso.clientSecretHelp')}</Description>
				</Stack>
						...
						<HStack gap={1} align="center">
							{testResult.valid ? (
								<>
									...
									<span className="text-sm text-green-600">{t('integrations.sso.testPassed')}</span>
								</>
							) : (
								<>
									<XCircle className="h-4 w-4 text-red-600" />
									<span className="text-sm text-red-600">{t('integrations.sso.testFailed')}</span>
								</>
							)}
						</HStack>
					)}
				...
				<Stack gap={3}>
					<HStack justify="between" className="py-2">
						<Label htmlFor="onedrive-access-toggle">{t('integrations.sso.oneDriveAccessLabel')}</Label>
						<Switch
					...
					<Description className="text-xs">{t('integrations.sso.oneDriveAccessHelp')}</Description>
				</Stack>

				<Stack gap={3}>
					<HStack justify="between" className="py-2">
						<Label htmlFor="auto-provision-team-toggle">{t('integrations.sso.autoProvisionTeamLabel')}</Label>
						<Switch
					...
					<Description className="text-xs">{t('integrations.sso.autoProvisionTeamHelp')}</Description>
				</Stack>
				...
				{autoProvisionTeam && (
					<Stack gap={3}>
						<Input
							...
							disabled={isSubmitting || isLoadingConfig}
						/>
						<Description className="text-xs">{t('integrations.sso.excludeGroupsHelp')}</Description>
					</Stack>
				)}

				<Stack gap={3}>
					<HStack justify="between" className="py-2">
						<Label htmlFor="auto-provision-role-toggle">{t('integrations.sso.autoProvisionRoleLabel')}</Label>
						<Switch
					...
					<Description className="text-xs">{t('integrations.sso.autoProvisionRoleHelp')}</Description>
				</Stack>

				{autoProvisionRole && (
					<Stack gap={3}>
						<Label>{t('integrations.sso.roleMappingRulesLabel')}</Label>
						<Description className="text-xs">{t('integrations.sso.roleMappingRulesHelp')}</Description>

						<Stack gap={0} className="divide-y divide-border">
							{roleMappingRules.map((rule, index) => (
								<HStack key={index} gap={2} align="center" className="flex-wrap py-3 first:pt-0 last:pb-0">
									<Select
										value={rule.source}
										onValueChange={(value) =>
											updateMappingRule(index, { source: value as 'jobTitle' | 'appRole' })
										}
										disabled={isSubmitting || isLoadingConfig}
										className="w-28 shrink-0"
										options={[
											{ value: 'jobTitle', label: t('integrations.sso.sourceJobTitle') },
											{ value: 'appRole', label: t('integrations.sso.sourceAppRole') },
										]}
									/>
									...
									<Input
										...
										value={rule.pattern}
										onChange={(e) => updateMappingRule(index, { pattern: e.target.value })}
										disabled={isSubmitting || isLoadingConfig}
				...
				<Stack gap={3}>
					<Label htmlFor="default-role-select">{t('integrations.sso.defaultRoleLabel')}</Label>
					<Select
						value={defaultRole}
