...
export function SSOConfigDialog({
  ...
  // @ts-expect-error TS2589 - deep type instantiation in Convex actions
  ...
  const hasChanges = useMemo(() => {
    ...
    return roleMappingRules.some((curr, i) => {
      ...
      return curr.source !== origRule.source ||
             curr.pattern !== origRule.pattern ||
             curr.targetRole !== origRule.targetRole;
    });
  }, [issuer, clientId, clientSecret, autoProvisionTeam, excludeGroups, autoProvisionRole, roleMappingRules, defaultRole, enableOneDriveAccess]);
  ...
  useEffect(() => {
    if (open && existingProvider) {
      ...
      getFullConfig({})
        .then((config) => {
          if (config) {
            const excludeGroupsStr = config.excludeGroups.join(', ');
            const rules = config.roleMappingRules.length > 0 ? config.roleMappingRules : DEFAULT_MAPPING_RULES;
            setIssuer(config.issuer);
            setClientId(config.clientId);
            setAutoProvisionTeam(config.autoProvisionTeam);
            ...
            setEnableOneDriveAccess(config.enableOneDriveAccess);
            originalConfigRef.current = {
              issuer: config.issuer,
              clientId: config.clientId,
              autoProvisionTeam: config.autoProvisionTeam,
              excludeGroups: excludeGroupsStr,
              autoProvisionRole: config.autoProvisionRole,
              roleMappingRules: rules,
              defaultRole: config.defaultRole,
              enableOneDriveAccess: config.enableOneDriveAccess,
            };
          }
          setClientSecret('');
        })
  ...
  const updateMappingRule = (index: number, updates: Partial<RoleMappingRule>) => {
    setRoleMappingRules(
      roleMappingRules.map((rule, i) => (i === index ? { ...rule, ...updates } : rule)),
    );
  };
              ...
              {roleMappingRules.map((rule, index) => (
                <HStack key={index} gap={2} align="center" className="flex-wrap py-3 first:pt-0 last:pb-0">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateMappingRule(index, { targetRole: value as PlatformRole })
                    }
                    disabled={isSubmitting || isLoadingConfig}
                    className="w-28 shrink-0"
