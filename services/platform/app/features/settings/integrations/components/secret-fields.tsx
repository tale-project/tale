...
export function isSecretDraftComplete(
  method: IntegrationAuthMethodName,
  draft: SecretDraft,
): boolean {
  if (method === 'basic') {
...
export function SecretFields({
  ...
  if (method === 'basic') {
    return (
      <>
        <Input
          label={t('integrations.dialog.username')}
        ...
        />
        <Input
          label={t('integrations.dialog.password')}
          type="password"
  ...
  return (
    <Input
      label={
        method === 'api-key'
          ? t('integrations.dialog.apiKey')
          : t('integrations.dialog.token')
      }
      type="password"
      value={value.token}
      onChange={(e) => onChange({ ...value, token: e.target.value })}
      description={
        method === 'bearer' ? t('integrations.dialog.tokenHelp') : undefined
      }
      disabled={disabled}
      required
    />
