 ...
 * object storage) into an admin-facing message. Duck-types `ConvexError.data`
 * because Vite chunk splitting can produce multiple `ConvexError` class copies
 * that break `instanceof` — same rationale as `deployment-errors.ts`, which
 * this mirrors (the code set differs: these actions gate on org membership and
 * validate a single connection, not the deployment file).
 */
...
export function mapOrgResidencyError(err: unknown, t: Translator): string {
  ...
  switch (code) {
    ...
    case 'ORG_NOT_FOUND':
    case 'ORG_FORBIDDEN':
      return t('orgDataResidency.errors.forbidden');
    case 'INVALID_CONNECTION':
      ...
      return serverMessage ?? t('orgDataResidency.errors.invalidConnection');
    case 'INVALID_CREDENTIALS':
      return t('orgDataResidency.errors.credentialsPair');
    case 'CREDENTIALS_REQUIRED':
      return t('orgDataResidency.errors.credentialsRequired');
    default:
      return fallback;
  }
}
