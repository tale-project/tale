...
const useConvexMutation = vi.fn((..._args: unknown[]) => ({
  mutate: vi.fn(),
  isPending: false,
}));
vi.mock('@/app/hooks/use-convex-mutation', () => ({
  useConvexMutation: (...args: unknown[]) => useConvexMutation(...args),
}));
...
describe('product mutation hooks', () => {
  it('opts out of the shared generic toast on create (the dialog toasts itself)', () => {
    ...
    expect(useConvexMutation).toHaveBeenCalledWith(
      expect.anything(),
  ...
  it('opts out of the shared generic toast on update and delete', () => {
    ...
    expect(useConvexMutation).toHaveBeenLastCalledWith(
      expect.anything(),
      expect.objectContaining({ errorToast: false }),
    );
