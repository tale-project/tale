...
import { appScope } from '@/lib/shared/schemas/apps';
import { formatBytes } from '@/lib/utils/format-bytes';

import type { ParsedAppBundle } from '../utils/parse-app-bundle';

interface PreviewStepProps {
  parsedBundle: ParsedAppBundle;
}
    ...
    <Stack gap={4} className="min-w-0 overflow-hidden">
      <Stack gap={1}>
        <HStack gap={2} align="center" className="flex-wrap">
          ...
          <Badge variant={scope === 'project' ? 'blue' : 'slate'}>
            {scope === 'project'
              ? t('upload.scopeProject', { defaultValue: 'Project app' })
              : t('upload.scopeOrg', { defaultValue: 'Org app' })}
          </Badge>
        </HStack>
      ...
      <Stack gap={2}>
        <HStack gap={2} align="center" justify="between">
          <Text variant="label">
            {t('upload.bundleFiles', { defaultValue: 'Bundle files' })}
          </Text>
