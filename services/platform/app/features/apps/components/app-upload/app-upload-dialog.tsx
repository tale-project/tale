...
import {
  useGenerateAppUploadUrl,
  useRecordAppUploadIntent,
  useUploadAppBundle,
} from '@/app/features/apps/hooks/upload-mutations';
import { toast } from '@/app/hooks/use-toast';
import { toId } from '@/convex/lib/type_cast_helpers';
import { useT } from '@/lib/i18n/client';
import { fetchJson } from '@/lib/utils/type-utils';

import { useUploadApp } from './hooks/use-upload-app';
...
export function AppUploadDialog({
  open,
  onOpenChange,
  organizationId,
  onUploaded,
}: AppUploadDialogProps) {
  const { t } = useT('apps');
  const { t: tCommon } = useT('common');

  const { mutateAsync: generateUploadUrl } = useGenerateAppUploadUrl();
  const { mutateAsync: recordIntent } = useRecordAppUploadIntent();
  const { mutateAsync: uploadFn } = useUploadAppBundle();

  const state = useUploadApp();
    ...
    async (force: boolean) => {
      ...
      try {
        ...
        if (landed) {
          ...
          toast({
            title: force
              ? t('upload.replaceSuccess', { defaultValue: 'App replaced' })
              : t('upload.uploadSuccess', { defaultValue: 'App uploaded' }),
            description: t('upload.installHint', {
              defaultValue: 'Install it from the Apps list when you’re ready.',
            }),
            variant: 'success',
          });
        ...
        toast({
          title: t('upload.uploadFailed', {
            defaultValue: 'Failed to upload app',
          }),
        ...
        <>
          ...
          >
            {state.isSubmitting
              ...
              : t('upload.submit', { defaultValue: 'Upload app' })}
          </Button>
        </>
      )}
    </>
  ...
  return (
    <>
      <Dialog
        ...
        title={t('upload.dialogTitle', { defaultValue: 'Upload private app' })}
        size="xl"
        footer={footer}
      ...
      <ConfirmDialog
        ...
        title={t('upload.replaceTitle', {
          defaultValue: 'Replace existing app?',
        })}
        description={t('upload.replaceDescription', {
          defaultValue:
            'A private app named "{slug}" already exists. Uploading will overwrite its current contents.',
