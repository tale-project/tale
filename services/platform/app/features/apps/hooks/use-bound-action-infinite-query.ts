...
export function useBoundActionInfiniteQuery(
  ...
  // `$config:` reference resolves to `undefined`; gate on a fully-bound result so
  // an unconfigured app shows a prompt instead of firing a call that fails arg
  // validation. `page`/`cursor` are plain values spread on per fetch, not args
  // sentinels, so they don't affect resolution.
