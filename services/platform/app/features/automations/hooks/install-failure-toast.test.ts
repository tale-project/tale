...
describe('notifyOnInstallFailure', () => {
  ...
  it('shows a destructive toast and logs when the action rejects', async () => {
    ...
    const err = new Error('Automation "x" not found in the catalog');

    notifyOnInstallFailure(
      Promise.reject(err),
      'Couldn’t install the automation',
    );
    ...
    expect(toastSpy).toHaveBeenCalledWith({
      title: 'Couldn’t install the automation',
      description: 'Automation "x" not found in the catalog',
      variant: 'destructive',
    });
  ...
  it('omits the description when the rejection is not an Error', async () => {
    ...
    notifyOnInstallFailure(
      Promise.reject('boom'),
      'Couldn’t reinstall the automation',
    );
    await Promise.resolve();
    await Promise.resolve();

    expect(toastSpy).toHaveBeenCalledWith({
      title: 'Couldn’t reinstall the automation',
