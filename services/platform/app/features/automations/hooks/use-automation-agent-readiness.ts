...
/** Narrow the `getAutomationAgentReadiness` (`v.any()`) result to its agents array. */
...
/**
 * Per-agent readiness for an automation's bundled agents (the agent half of the install
 * readiness; the integration half is `getAutomationInstallState`). Backed by the
 * `getAutomationAgentReadiness` action, which returns facts for BOTH auth modes
 ...
 */
export function useAutomationAgentReadiness(
