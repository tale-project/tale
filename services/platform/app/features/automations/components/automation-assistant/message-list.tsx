...
export function MessageList({
  ...
  // Find the latest active (pending/executing) approval
  const activeApproval = useMemo((): ActiveApprovalItem | null => {
    const candidates: ActiveApprovalItem[] = [];

    for (const a of workflowUpdateApprovals) {
      if (a.status === 'pending' || a.status === 'executing') {
        candidates.push({ type: 'workflow_update', data: a });
      }
    }
    for (const a of workflowCreationApprovals) {
      if (a.status === 'pending' || a.status === 'executing') {
        candidates.push({ type: 'workflow_creation', data: a });
      }
    }
    ...
    return candidates[0];
  }, [workflowUpdateApprovals, workflowCreationApprovals]);
    ...
    >
      ...
      {activeApproval && (
        <div className="flex justify-start">
          {activeApproval.type === 'workflow_update' ? (
            <WorkflowUpdateApprovalCard
              approvalId={activeApproval.data._id}
              organizationId={organizationId}
              status={activeApproval.data.status}
              metadata={activeApproval.data.metadata}
              executedAt={activeApproval.data.executedAt}
              executionError={activeApproval.data.executionError}
