...
describe('ExternalList — filter-aware pagination', () => {
  ...
  it('renders more than the old 50-row cap (no silent truncation)', () => {
  ...
  it('keeps the loaded rows visible AND surfaces an inline error when a later page fetch fails', () => {
    ...
    expect(screen.getByText(/automations\.list\.error/)).toBeInTheDocument();
  ...
  it('reads an unresolved $state. binding as "awaiting selection", not config', () => {
    hookReturn = hook({ needsConfig: true });
    ...
    render(
      <ExternalList
        ...
        source={{
          ...
          args: { issueId: '$state.selectedIssue' },
        }}
      />,
    );
