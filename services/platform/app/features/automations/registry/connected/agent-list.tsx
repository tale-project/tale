...
/**
 * Connected `AgentList` block — shows the automation's agents (the team doing the work)
 * with entry points to configure them. Binds the allowlisted `listAutomationAgents`
 * action (one-shot), scoped to THIS automation's own agents dir — automation agents are
 * automation-scoped and never appear in the global `/agents`. Each agent's identity is
 * the composite slug `<automation>/<name>` (passed to the dialogs / save); the card
...
export interface AgentListProps {
  title?: string;
  /**
   * Optional subset/order of the automation's agents to show (matched against either
   * the composite slug `<automation>/<name>` or the bare short name); empty = all of
   * the automation's agents.
   */
  agents?: string[];
