...
/**
 * Connected `WorkflowDag` block — the automation's workflow display, which **fully
 * reuses the global workflow canvas** (`WorkflowSteps`), not a bespoke
 * per-automation map, driven by the same `useReadWorkflow` the editor uses. Three modes:
...
import {
  ...
  WorkflowSteps,
} from '@/app/features/workflows/components/workflow-steps';
 ...
 *  the workflow editor route's projection. */
 ...
 *  shape — the same projection the workflow editor route does. */
function mapSteps(
  config: unknown,
...
export function WorkflowDag({
  ...
  // (WorkflowSteps writes panel/step; the side panel reads it). The chat and
  ...
  if (read.error) {
    ...
    // cleanly inside the automation page rather than collapsing.
    const canvas = (
      <Row gap={0} align="stretch" className="h-[28rem] w-full">
        <WorkflowSteps steps={steps} hasActiveTrigger={false} />
      </Row>
    );
    body = executionId ? (
