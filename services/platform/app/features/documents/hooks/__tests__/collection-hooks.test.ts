...
vi.mock('@/lib/collections/entities/documents', () => ({
  createDocumentsCollection: vi.fn(),
}));
...
import { useDocumentCollection, useDocuments } from '../collections';
...
describe('useDocuments', () => {
  ...
  it('returns data from live query', () => {
    ...
    const result = useDocuments(mockCollection as never);
    expect(result.documents).toBe(items);
  ...
  it('returns data even while loading', () => {
    ...
    const result = useDocuments(mockCollection as never);
    expect(result.documents).toBe(mockData);
