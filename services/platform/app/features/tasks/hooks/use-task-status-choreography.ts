...
export interface ChoreographedTask {
  _id: Id<'tasks'>;
 ...
 */
export function useTaskStatusChoreography(organizationId: string) {
  ...
  return useCallback(
    async (
      ...
      const resolved = resolveTaskSubjectContract(task, automations);
      ...
      if (task.status === 'in_progress') {
        ...
        runActive = run != null && isActiveExecutionStatus(run.status);
      }
      let hasFiles = false;
      if (
        to === 'in_progress' &&
