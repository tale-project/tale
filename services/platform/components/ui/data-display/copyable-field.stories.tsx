...
## Usage
\`\`\`tsx
import { CopyableField, CopyableText } from '@/components/ui/data-display/copyable-field';

<CopyableField value="abc123def456" label="API Key" />
<CopyableText value="user_123" />
\`\`\`
...
export const CopyableTextDefault: Story = {
  render: () => <CopyableText value="user_12345" />,
  name: 'CopyableText - Default',
};

export const CopyableTextInContext: Story = {
  render: () => (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <span className="text-sm text-muted-foreground">User ID:</span>
        <CopyableText value="usr_abc123" />
      </div>
      <div className="flex items-center gap-2">
        <span className="text-sm text-muted-foreground">Order ID:</span>
        <CopyableText value="ord_xyz789" />
      </div>
      <div className="flex items-center gap-2">
        <span className="text-sm text-muted-foreground">Transaction:</span>
        <CopyableText value="txn_def456" />
      </div>
    </div>
  ),
  name: 'CopyableText - In Context',
  parameters: {
    docs: {
      description: {
        story: 'CopyableText is useful for inline ID values.',
      },
    },
  },
};
