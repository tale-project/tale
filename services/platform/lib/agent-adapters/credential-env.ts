...
/** OpenAI-compat gateway keys (Hermes / Codex) that may linger after a
 * runtime switch. */
const OPENAI_COMPAT_GATEWAY_SESSION_KEYS = [
  'OPENAI_BASE_URL',
  'OPENAI_API_KEY',
  'OPENROUTER_API_KEY',
] as const;
...
export function collectScrubCredentialEnvKeys(
  ...
  if (!gatewayRun || byo) {
    ...
    for (const k of OPENAI_COMPAT_GATEWAY_SESSION_KEYS) keys.add(k);
