 ...
 * Convex surface under `convex/integrations/` is the thin wiring that supplies
...
const catalog = new Map<string, IntegrationConnector>();
...
/**
 * Load every shipped connector under `<systemRoot>/integrations/`, register
 ...
 */
export function loadConnectorCatalog(
  systemRoot: string,
): IntegrationConnector[] {
 ...
 */
export interface NativeIntegrationContext extends IntegrationContext {
...
const natives = new Map<string, NativeIntegrationImpl>();
...
export interface IntegrationInvocationRecord {
  organizationId: string;
  connector: string;
  action: string;
...
export interface IntegrationAuditSink {
  record(entry: IntegrationInvocationRecord): Promise<void>;
}
...
export type IntegrationHostFactory = (
 ...
 *    vendors without passing through the integration layer at all.
...
function callerPolicy(caller: IntegrationCaller): CallerPolicy {
  switch (caller.kind) {
    case 'user':
...
function validatorFor(action: IntegrationAction): ValidateFunction {
...
function resolveConnector(slug: string): IntegrationConnector {
  if (catalog.size === 0) {
    throw new IntegrationError(
      'CATALOG_UNAVAILABLE',
 ...
 * any side effect; on failure the rejection is an {@link IntegrationError}
 * carrying the connector, the action, and what to do next.
 */
export async function executeIntegrationAction(
  args: ExecuteIntegrationActionArgs,
): Promise<IntegrationDispatchResult> {
  const { input, credentialRef, caller, ctx } = args;
  const mode: IntegrationMode = ctx.mode ?? 'mock';

  if (!ctx.organizationId) {
    throw new IntegrationError(
      'ORGANIZATION_REQUIRED',
      'an integration invocation must name the organization it acts for',
   ...
   */
  const record = async (
    outcome: IntegrationInvocationRecord['outcome'],
    ...
    try {
      ...
      console.warn(
        `[integrations] ${nodeType}: audit record failed (${outcome})`,
        cause,
      );
    }
  };
