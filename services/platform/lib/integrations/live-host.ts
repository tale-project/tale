...
import type {
  IntegrationContext,
  IntegrationHostCapabilities,
  IntegrationHttpRequest,
  IntegrationHttpResponse,
} from '../engine/core/slots';
import type { IntegrationConnector } from '../shared/schemas/integrations';
...
export interface IntegrationBlobSink {
  store(args: {
    data: string;
...
export function hostMatchesAllowEntry(
  host: string,
  entry: string,
  endpointMode: IntegrationConnector['endpointMode'],
): boolean {
...
export function checkConnectorRequestUrl(
  ...
  if (parsed.protocol !== 'https:') {
    throw new IntegrationError(
      'INSECURE_SCHEME',
      `integration requests are https only, got "${parsed.protocol}//" for ${parsed.host}`,
      {
  ...
  try {
    ...
    throw new IntegrationError(
      'BLOCKED_HOST',
      `host "${parsed.hostname}" is not reachable from an integration (private, link-local, or cloud-metadata address)`,
      { connector: connector.name, cause },
    );
  }
...
function asIntegrationError(
  ...
  if (error instanceof SafeFetchError) {
    if (error.kind === 'response_too_large') {
      return new IntegrationError('RESPONSE_TOO_LARGE', error.message, {
        ...where,
 ...
 * key — to produce the full {@link IntegrationContext}.
 */
export function createLiveHost(
  ...
  function buildHeaders(
    ...
    for (const [name, value] of Object.entries(supplied ?? {})) {
      if (name.toLowerCase() === 'authorization') {
        console.warn(
          `[integrations] ${connector.name}${action ? `.${action}` : ''}: connector body set its own Authorization header; the host-injected credential is used instead`,
        );
        continue;
      }
      headers[name] = value;
  ...
  async function request(
    ...
    try {
      ...
      throw asIntegrationError(error, connector, action, target);
    }
  }

  const http: IntegrationContext['http'] = {
  ...
  const files: IntegrationContext['files'] | undefined = blobs
    ? {
