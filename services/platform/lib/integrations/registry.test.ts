...
describe('connector registry', () => {
  ...
  it('exposes each action to the engine with its schema and signature', () => {
    ...
    expect(def?.kind).toBe('integration');
    expect(def?.outputKind).toBe('structured');
    expect(def?.integration?.inputSchema).toMatchObject({ type: 'object' });
    expect(def?.integration?.outputSignature).toContain('number');
  });

  it('marks write actions as effectful and read actions as not', () => {
    expect(
      nodeTypes().get(nodeTypeFor('github', 'create_issue'))?.integration
        ?.hasEffect,
    ).toBe(true);
    expect(
      nodeTypes().get(nodeTypeFor('tavily', 'search'))?.integration?.hasEffect,
    ).toBe(false);
  });
