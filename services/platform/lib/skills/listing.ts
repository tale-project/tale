...
/**
 * The skills `viewer` may see in this organization: every `org`-visible
 * bundle plus the viewer's own `private` ones. Failures are reported
 * unfiltered — a broken bundle is an operator problem regardless of who it
...
/**
 * The skills an agent turn may reach: what the acting member can see,
 * narrowed by an optional hard allowlist. An allowlist is a closed set — a
 * slug it does not name is unreachable even when the member could see it,
 * and a slug that no longer exists simply contributes nothing. Passing
 * `undefined` means "no allowlist configured", which leaves every visible
 * skill reachable.
 */
export function resolveSkillsForAgent(
  ...
  const visible = skills.filter((skill) => canViewSkill(skill.meta, viewer));
