...
```
  ...
  connector calls ──────▶│  …/mock/<integration>/*     → Prism      │  ← one instance per spec
  (rewritten, see below) └─────────────────────────────────────────┘
```
...
- **Integration connectors** hardcode their upstream base URLs, so they are
  redirected at the sandbox HTTP seam: when `TALE_MOCK_INTEGRATIONS_BASE` is set
  (the Playwright config sets it to the gateway), `convex/node_only/integration_sandbox/
helpers/mock_rewrite.ts` maps a known upstream origin (`api.github.com`,
  `slack.com`, `*.atlassian.net`, …) to `<base>/mock/<integration>/…`. That host
...
- `src/contract/integrations.test.ts` — `resolveMockUrl` rewrite mapping + each
  integration serving the exact shape its connector parses.
...
1. Read the connector (`examples/default/integrations/<name>/connector.ts`) for
...
sandbox `/v1/execute` contract — it makes no external AI/integration calls, so it
already runs offline and does not need the gateway. If that suite ever drives
chat or integration flows, add an `api-mocks` service to `compose.test.yml` and
set `TALE_MOCK_INTEGRATIONS_BASE` + the provider `baseUrl` on the convex service.
