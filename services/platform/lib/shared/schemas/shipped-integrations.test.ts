/**
 * The shipped integration catalog is valid, complete, and executable.
 *
 * This walks every `configs/platform/system/integrations/<slug>/connector.yml`
...
const INTEGRATIONS_DIR = path.join(
  ...
  '../../../../../configs/platform/system/integrations',
);
...
function loadConnector(slug: string): IntegrationConnector {
  const file = path.join(INTEGRATIONS_DIR, slug, 'connector.yml');
...
const presentSlugs = readdirSync(INTEGRATIONS_DIR)
  .filter((entry) => statSync(path.join(INTEGRATIONS_DIR, entry)).isDirectory())
  .sort();

describe('shipped integration catalog', () => {
