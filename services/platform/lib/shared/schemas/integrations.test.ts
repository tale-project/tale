...
describe('integrationConnectorSchema', () => {
  it('accepts a full connector with mixed auth, yaml-js + native + mock-only actions', () => {
    const github = integrationConnectorSchema.parse(GITHUB);
  ...
  it('accepts a native-backed connector', () => {
    const mailbox = integrationConnectorSchema.parse(MAILBOX);
  ...
  it('defaults oauth2 scopes, tags, and allowedHosts to empty', () => {
    const c = integrationConnectorSchema.parse({
      name: 'tavily',
      displayName: 'Tavily',
  ...
  it('defaults the bearer scheme to Bearer and accepts a vendor scheme', () => {
    ...
    const discord = integrationConnectorSchema.parse({
      ...(GITHUB as IntegrationConnector),
      name: 'discord',
  ...
  it('rejects a scheme that is not a single header token', () => {
    expect(
      integrationConnectorSchema.safeParse({
        ...(GITHUB as IntegrationConnector),
  ...
  it('defaults endpointMode to fixed and accepts per-credential', () => {
    ...
    const confluence = integrationConnectorSchema.parse({
      name: 'confluence',
  ...
  it('rejects duplicate auth methods', () => {
    ...
    expect(integrationConnectorSchema.safeParse(bad).success).toBe(false);
  });
  ...
  it('requires a mock on every action', () => {
    ...
    expect(integrationConnectorSchema.safeParse(noMock).success).toBe(false);
  });
  ...
  it('requires an object-typed input schema', () => {
    ...
    expect(integrationConnectorSchema.safeParse(badInput).success).toBe(false);
  });
  ...
  it('rejects a malformed native impl id', () => {
    const bad = {
      ...(MAILBOX as IntegrationConnector),
      actions: [
        {
          ...(MAILBOX as IntegrationConnector).actions[0],
  ...
  it('rejects unknown top-level and action fields (strict)', () => {
    const github = integrationConnectorSchema.parse(GITHUB);
    expect(
      integrationConnectorSchema.safeParse({ ...github, extra: 1 }).success,
    ).toBe(false);
  });
});
