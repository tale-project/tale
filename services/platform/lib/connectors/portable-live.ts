 ...
 *    `/api/connectors/hostcall` endpoint (bearer: a one-run capability
...
export interface PortableHostCall {
  ...
  url: string;
  /** One-run capability token (see connectors/hostcall_token.ts). */
  token: string;
}
...
const PORTABLE_CTX_PRELUDE = `ctx = (function (d) {
  async function hostHttp(method, url, req) {
    ...
    try {
      ...
      throw new Error('connector host-call unreachable: ' + (err && err.message ? err.message : String(err)));
    }
    var out = null;
    try { out = await res.json(); } catch (err) {
      throw new Error('connector host-call returned a non-JSON response (status ' + res.status + ')');
    }
    if (out && out.error) {
      throw new Error(String(out.error.message || out.error.code || 'connector host-call failed'));
    }
    if (!res.ok) {
      throw new Error('connector host-call failed (status ' + res.status + ')');
    }
    var parsed = null;
    return {
      status: out.status,
