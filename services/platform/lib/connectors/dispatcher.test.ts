...
import {
  ...
  type ConnectorInvocationRecord,
...
const DEMO: Connector = connectorSchema.parse({
  name: 'demo',
  displayName: 'Demo',
...
function auditSink(): ConnectorAuditSink & {
  records: ConnectorInvocationRecord[];
} {
  const records: ConnectorInvocationRecord[] = [];
  return {
    records,
    record: async (entry) => {
      records.push(entry);
...
describe('native backends', () => {
  ...
  it('calls a registered impl with the org and the mediated context', async () => {
    const seen: Array<{ input: unknown; ctx: NativeConnectorContext }> = [];
...
describe('caller modes', () => {
  ...
  it('fails a system call whose record could not be written', async () => {
    const audit: ConnectorAuditSink = {
      record: async () => {
