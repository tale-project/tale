/**
 * The connector layer's issue-code catalog — one place naming every refusal
...
/** Every code the connector layer can emit, with the invariant it guards. */
export const CONNECTOR_CODES = {
  ...
  INSECURE_SCHEME: 'live connector traffic is https only',
...
/** The refusal metadata an ConnectorError carries. @public */
export interface ConnectorErrorExtras {
  connector?: string;
  action?: string;
