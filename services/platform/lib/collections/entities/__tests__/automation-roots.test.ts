...
vi.mock('@/convex/_generated/api', () => ({
  api: {
    wf_definitions: {
      queries: {
        listAutomationRoots: 'listAutomationRoots-ref',
      },
    },
  },
}));
...
describe('createAutomationRootsCollection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('uses _id as the collection key', () => {
    ...
    expect(getKey({ _id: 'root-abc' })).toBe('root-abc');
  });
