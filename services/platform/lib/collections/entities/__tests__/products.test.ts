...
const mockQueryClient = {} as Parameters<typeof createProductsCollection>[1];
const mockConvexQueryFn = vi.fn();
const mockConvexClient = {} as Parameters<typeof createProductsCollection>[3];
...
describe('createProductsCollection', () => {
  ...
  it('uses _id as the collection key', () => {
    ...
    expect(getKey({ _id: 'product-abc' })).toBe('product-abc');
  });
