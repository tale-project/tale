...
import { createVendorsCollection } from '../vendors';

const mockQueryCollectionOptions = vi.mocked(queryCollectionOptions);
const mockQueryClient = {} as Parameters<typeof createVendorsCollection>[1];
const mockConvexQueryFn = vi.fn();
const mockConvexClient = {} as Parameters<typeof createVendorsCollection>[3];

describe('createVendorsCollection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('creates collection options with correct id and query', () => {
    createVendorsCollection(
      'org-123',
      mockQueryClient,
      mockConvexQueryFn,
      mockConvexClient,
  ...
  it('uses _id as the collection key', () => {
    ...
    expect(getKey({ _id: 'vendor-abc' })).toBe('vendor-abc');
  });
