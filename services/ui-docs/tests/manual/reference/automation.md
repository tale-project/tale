...
| Chrome a11y (axe): demo tag, nav tree, header strip, outline, neighbours, 404 | `app/**/*.test.tsx` | focus order and visible focus rings by hand, colour contrast (axe's contrast rule is off in jsdom) |
...
| Print | — | **manual-only** — no print stylesheet yet (`BL-4`) |
