...
// The site renders in English, but the chrome catalog stays complete in every
// locale the repo ships — `packages` layers the two design-system catalogs
// under this service's own keys, so a `@tale/ui` control or a marketing demo
// frame finds its strings without the service duplicating them.
