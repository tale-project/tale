...
| A failed re-read keeps the rows; a sign-in that ran out is told from a dropped connection | `app/lib/api.test.ts` + `app/lib/use-gateway.test.ts` + `app/components/accounts-screen.test.tsx` + `app/components/add-account-dialog.test.tsx` | the real gate and the page load back through it (`ACCT-46`), and the warning's place on screen (`ACCT-45`) — the specs pin the codes, the kept rows, which warning shows and what its button does, but only a browser holds a redirect as an opaque answer |
...
| OAuth URL construction, callback parsing, PKCE, the three ways a grant comes back | `backend/providers/{anthropic,openai,oauth}.test.ts` + `backend/accounts.test.ts` + `backend/routes.test.ts` | nothing below the vendor's own page — the authorize URLs, the paste shapes, the S256 derivation, the device code and its polling, the loopback redirect chosen from `Origin`, and `/callback` sending the browser back are all asserted. The consent itself is `ACCT-3`, `ACCT-4`, `ACCT-39` |
