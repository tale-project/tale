...
| `ACCT-30` | The reset is in the row, not behind a hover. "When does this come back" is the question a spent plan raises, and a figure only a mouse can reach does not answer it — for a touch reader it does not answer it at all. The tooltip keeps the exact instant, so hovering still buys precision the relative phrase cannot carry. |
