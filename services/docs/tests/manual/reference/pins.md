...
| `NAV-F3`, `NAV-A3` | TanStack Router marks a `Link` active by **prefix** and gives an active link `aria-current="page"` for free, so before 2026-09-14 every ancestor row of a deep page claimed to be the current page. The rows now pass `activeOptions={{ exact: true }}`; `smoke.spec.ts` asserts a single marked row. The same prefix rule made every link to a locale home (`/de`, `/fr`) — the logo, the **Previous** card of a locale's first page, the 404's way back — claim the current page on every German and French page until 2026-09-15; every link in the shared `@tale/ui/docs/*` frame is exact now, and `smoke.spec.ts` counts one marked link on `/de`. |
| `NAV-F13` | The strip used to wrap its fixed `h-13` row in a bordered parent, so it stood 53 px beside the rail's 52 px logo row and the line under it stepped down a pixel at the rail's edge — most visible with the page actions in the strip. The shared `DocsHeader` is the `h-13` box itself, border included; `page-header.spec.ts` and `docs-layout.browser.test.tsx` compare the two bars' heights and bottoms. |
