/**
 * Generates the connector catalog icons from a single canonical source:
 * Iconify's `logos` collection (full-color brand marks).
 *
 * Each connector renders a 24x24 SVG with a baked white rounded tile so dark
 ...
 *
 * Run: bun scripts/generate-connector-icons.ts
 */
...
/** Tile geometry shared by every connector icon. */
const TILE = 24;
/** Edge of the centered content box; 15/24 ≈ the existing 62.5% ratio. */
const MARK_BOX = 15;

/**
 * Connector slug → Iconify `logos` icon name. Only brands that exist in the
 * set live here; adding a new logos-backed connector is one line + a re-run.
 */
...
async function main(): Promise<void> {
  ...
  for (const slug of slugs) {
    ...
    const outPath = join(CONNECTORS_DIR, slug, 'icon.svg');
    await Bun.write(outPath, svg);
    console.log(`✓ ${slug} ← logos:${iconName}`);
  }
  console.log(`\nGenerated ${slugs.length} connector icons.`);
}
