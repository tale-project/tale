...
/** Promise cache keyed by index URL. A `Map` is the only race-safe shape:
 *  concurrent callers for different indexes (one per locale) don't collide,
 *  and concurrent callers for the same index dedupe onto a single in-flight
 *  fetch. */
...
function isSerializedIndex(
  value: unknown,
): value is Pick<SerializedIndex, 'index'> {
  return typeof value === 'object' && value !== null && 'index' in value;
}

/**
 * Fetch the static MiniSearch index at `indexUrl`, rehydrate it via
 * `MiniSearch.loadJSONAsync`, and cache the result. Subsequent calls for the
 * same URL return the cached instance.
 *
...
export async function loadIndex(
  ...
  const promise = (async () => {
    ...
    if (!response.ok) {
      throw new Error(
        `[search] failed to load ${indexUrl}: ${response.status}`,
      );
    }
    ...
    if (!contentType.includes('json')) {
      throw new Error(
        `[search] ${indexUrl} returned non-JSON (${contentType || 'unknown'}) — ` +
          `the index is likely missing or served from the wrong base path`,
      );
    }
    const payload: unknown = await response.json();
    if (!isSerializedIndex(payload)) {
      throw new Error(`[search] ${indexUrl} is not a search index`);
    }
  ...
  promise.catch(() => {
    if (indexCache.get(indexUrl) === promise) indexCache.delete(indexUrl);
  });

  indexCache.set(indexUrl, promise);
  return promise;
}

/** Test-only — drops every cached index so a fresh fetch runs next call. */
...
export async function search(
  ...
  if (!trimmed) return [];
  const ms = await loadIndex(indexUrl);
