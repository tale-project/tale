...
/** One entry per dialog key — typed without the `{} as Record<K, V>` cast. */
function buildRecord<K extends string, V>(
  keys: readonly K[],
  valueFn: (key: K) => V,
): Record<K, V> {
  const result: Partial<Record<K, V>> = {};
  for (const key of keys) result[key] = valueFn(key);
  // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- every key is assigned in the loop above
