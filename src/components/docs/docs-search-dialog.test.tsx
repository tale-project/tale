...
import type { StaticSearchHit } from '@tale/ui/search/static-index/types';
...
const RECENTS_KEY = 'tale.docs.recentSearches.v1';
...
function renderDialog(props: Partial<DocsSearchDialogProps> = {}) {
...
describe('DocsSearchDialog', () => {
  it('shows the ancestors the site resolves for a result', async () => {
    vi.spyOn(client, 'search').mockResolvedValue([
      makeResult({
        id: 'de:platform/projects/tasks',
        title: 'Task result',
        url: '/de/platform/projects/tasks',
      }),
    ]);
    const { user } = renderDialog({
      breadcrumb: () => ['Plattform', 'Projekte'],
    });
    await user.type(
  ...
  it('falls back to the URL trail when the site resolves no ancestors', async () => {
    vi.spyOn(client, 'search').mockResolvedValue([
      makeResult({ id: 'en:platform/rag', title: 'RAG', url: '/platform/rag' }),
    ]);
    const { user } = renderDialog({
      breadcrumb: () => undefined,
      sectionLabel: (key) => (key === 'platform' ? 'Platform guides' : key),
    });
    await user.type(screen.getByPlaceholderText('Search documentation'), 'rag');
    await waitFor(() =>
      expect(screen.getAllByText('Platform guides').length).toBeGreaterThan(0),
    );
  });

  it('queries the index URL it is given', async () => {
    const searchSpy = vi.spyOn(client, 'search').mockResolvedValue([]);
    const { user } = renderDialog({ indexUrl: '/docs/search-index-fr.json' });
    await user.type(screen.getByPlaceholderText('Search documentation'), 'rag');
    await waitFor(() =>
      expect(searchSpy).toHaveBeenCalledWith(
        '/docs/search-index-fr.json',
        'rag',
      ),
    );
  });
  ...
  it('shows the skeleton while the first search is in flight', async () => {
    let resolve: (rows: StaticSearchHit[]) => void = () => {};
    const searchSpy = vi
      .spyOn(client, 'search')
      .mockImplementation(
        () => new Promise<StaticSearchHit[]>((r) => (resolve = r)),
      );
    const { user } = renderDialog();
    await user.type(
      screen.getByPlaceholderText('Search documentation'),
